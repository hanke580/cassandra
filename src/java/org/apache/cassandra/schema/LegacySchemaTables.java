...
public class LegacySchemaTables {
    ...
    private static final CFMetaData Columnfamilies = compile(COLUMNFAMILIES, "table definitions", "CREATE TABLE %s (" + "keyspace_name text," + "columnfamily_name text," + "bloom_filter_fp_chance double," + "caching text," + // post-2.1 UUID cfid
    ...
    private static CFMetaData compile(String name, String description, String schema) {
        return CFMetaData.compile(String.format(schema, name), SystemKeyspace.NAME).comment(description).gcGraceSeconds((int) TimeUnit.DAYS.toSeconds(7));
    }
    ...
    public static void saveSystemKeyspaceSchema() {
        ...
        for (String table : ALL) {
            executeOnceInternal(String.format("DELETE FROM system.%s USING TIMESTAMP ? WHERE keyspace_name = ?", table), timestamp, keyspace.name);
        }
    ...
    public static Collection<KSMetaData> readSchemaFromSystemTables() {
        ...
        for (Row partition : serializedSchema) {
            ...
            for (UDFunction function : createFunctionsFromFunctionsPartition(readSchemaPartitionForKeyspace(FUNCTIONS, partition.key)).values()) org.apache.cassandra.cql3.functions.Functions.addOrReplaceFunction(function);
            // Will be moved away in #6717
            for (UDAggregate aggregate : createAggregatesFromAggregatesPartition(readSchemaPartitionForKeyspace(AGGREGATES, partition.key)).values()) org.apache.cassandra.cql3.functions.Functions.addOrReplaceFunction(aggregate);
        }
        return keyspaces;
    }

    public static void truncateSchemaTables() {
        for (String table : ALL) getSchemaCFS(table).truncateBlocking();
    }

    private static void flushSchemaTables() {
        for (String table : ALL) SystemKeyspace.forceBlockingFlush(table);
    }
    ...
    private static List<Row> getSchemaPartitionsForTable(String schemaTableName) {
        ...
        return getSchemaCFS(schemaTableName).getRangeSlice(new Range<RowPosition>(minToken.minKeyBound(), minToken.maxKeyBound()), null, new IdentityQueryFilter(), Integer.MAX_VALUE, System.currentTimeMillis());
    }
        ...
        for (Row partition : getSchemaPartitionsForTable(schemaTableName)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(getSchemaPartitionsForTable(schemaTableName), partition, "partition").toJsonString());
    ...
    public static synchronized void mergeSchema(Collection<Mutation> mutations, boolean doFlush) throws IOException {
        ...
        for (String keyspaceToDrop : keyspacesToDrop) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(keyspacesToDrop, keyspaceToDrop, "keyspaceToDrop").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static Set<String> mergeKeyspaces(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, ColumnFamily> entry : diff.entriesOnlyOnRight().entrySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(diff.entriesOnlyOnRight().entrySet(), entry, "entry").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(diff.entriesDiffering().entrySet(), entry, "entry").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (Row row : created) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(created, row, "row").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (String name : altered) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(altered, name, "name").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static void mergeTables(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            ...
            ColumnFamily post = entry.getValue().rightValue();
            if (pre.hasColumns() && post.hasColumns()) {
                MapDifference<String, CFMetaData> delta = Maps.difference(Schema.instance.getKSMetaData(keyspaceName).cfMetaData(), createTablesFromTablesPartition(new Row(entry.getKey(), post)));
        ...
        for (CFMetaData cfm : created) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(created, cfm, "cfm").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (CFMetaData cfm : altered) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(altered, cfm, "cfm").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (CFMetaData cfm : dropped) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(dropped, cfm, "cfm").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static void mergeTypes(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (UserType type : created) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(created, type, "type").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (UserType type : altered) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(altered, type, "type").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (UserType type : dropped) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(dropped, type, "type").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static void mergeFunctions(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, ColumnFamily> entry : diff.entriesOnlyOnRight().entrySet()) if (entry.getValue().hasColumns())
    ...
    private static Mutation makeCreateKeyspaceMutation(KSMetaData keyspace, long timestamp, boolean withTablesAndTypesAndFunctions) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Keyspaces, "org.apache.cassandra.schema.LegacySchemaTables.Keyspaces").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    public static Mutation makeDropKeyspaceMutation(KSMetaData keyspace, long timestamp) {
        Mutation mutation = new Mutation(SystemKeyspace.NAME, getSchemaKSKey(keyspace.name));
        for (String schemaTable : ALL) mutation.delete(schemaTable, timestamp);
    ...
    private static KSMetaData createKeyspaceFromSchemaPartition(Row partition) {
        ...
        return new KSMetaData(row.getString("keyspace_name"), AbstractReplicationStrategy.getClass(row.getString("strategy_class")), fromJsonMap(row.getString("strategy_options")), row.getBoolean("durable_writes"));
    }
    ...
    private static void addTypeToSchemaMutation(UserType type, long timestamp, Mutation mutation) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Usertypes, "org.apache.cassandra.schema.LegacySchemaTables.Usertypes").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    private static UserType createTypeFromRow(UntypedResultSet.Row row) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.db.marshal.UTF8Type.class, org.apache.cassandra.db.marshal.UTF8Type.instance, "org.apache.cassandra.db.marshal.UTF8Type.instance").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
        ...
        for (String rawColumn : rawColumns) columns.add(ByteBufferUtil.bytes(rawColumn));
        List<AbstractType<?>> types = new ArrayList<>(rawTypes.size());
        for (String rawType : rawTypes) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(rawTypes, rawType, "rawType").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static void addTableToSchemaMutation(CFMetaData table, long timestamp, boolean withColumnsAndTriggers, Mutation mutation) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Columnfamilies, "org.apache.cassandra.schema.LegacySchemaTables.Columnfamilies").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    public static Mutation makeUpdateTableMutation(KSMetaData keyspace, CFMetaData oldTable, CFMetaData newTable, long timestamp) {
        ...
        for (ColumnDefinition column : columnDiff.entriesOnlyOnLeft().values()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(columnDiff.entriesOnlyOnLeft().values(), column, "column").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (ColumnDefinition column : columnDiff.entriesOnlyOnRight().values()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(columnDiff.entriesOnlyOnRight().values(), column, "column").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (ByteBuffer name : columnDiff.entriesDiffering().keySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(columnDiff.entriesDiffering().keySet(), name, "name").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    public static Mutation makeDropTableMutation(KSMetaData keyspace, CFMetaData table, long timestamp) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.db.SystemKeyspace.class, org.apache.cassandra.db.SystemKeyspace.BuiltIndexes, "org.apache.cassandra.db.SystemKeyspace.BuiltIndexes").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(keyspace, keyspace.name, "keyspace.name").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    private static CFMetaData createTableFromTableRow(UntypedResultSet.Row result) {
        ...
        for (TriggerDefinition trigger : createTriggersFromTriggersPartition(serializedTriggers)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(createTriggersFromTriggersPartition(serializedTriggers), trigger, "trigger").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    public static CFMetaData createTableFromTableRowAndColumnRows(UntypedResultSet.Row result, UntypedResultSet serializedColumnDefinitions) {
        ...
        for (ColumnDefinition cd : columnDefs) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(columnDefs, cd, "cd").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static Map<ColumnIdentifier, Long> convertDroppedColumns(Map<String, Long> raw) {
        ...
        for (Map.Entry<String, Long> entry : raw.entrySet()) converted.put(new ColumnIdentifier(entry.getKey(), true), entry.getValue());
        return converted;
    }
    ...
    private static void addColumnToSchemaMutation(CFMetaData table, ColumnDefinition column, long timestamp, Mutation mutation) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Columns, "org.apache.cassandra.schema.LegacySchemaTables.Columns").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    private static List<ColumnDefinition> createColumnsFromColumnRows(UntypedResultSet rows, String keyspace, String table, AbstractType<?> rawComparator, boolean isSuper) {
        ...
        for (UntypedResultSet.Row row : rows) columns.add(createColumnFromColumnRow(row, keyspace, table, rawComparator, isSuper));
        return columns;
    }
    ...
    private static ColumnDefinition createColumnFromColumnRow(UntypedResultSet.Row row, String keyspace, String table, AbstractType<?> rawComparator, boolean isSuper) {
        ...
        AbstractType<?> comparator = kind == ColumnDefinition.Kind.REGULAR ? getComponentComparator(rawComparator, componentIndex) : UTF8Type.instance;
    ...
    private static void addTriggerToSchemaMutation(CFMetaData table, TriggerDefinition trigger, long timestamp, Mutation mutation) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Triggers, "org.apache.cassandra.schema.LegacySchemaTables.Triggers").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    private static void addFunctionToSchemaMutation(UDFunction function, long timestamp, Mutation mutation) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Functions, "org.apache.cassandra.schema.LegacySchemaTables.Functions").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    private static UDFunction createFunctionFromFunctionRow(UntypedResultSet.Row row) {
        ...
        if (row.has("argument_names")) {
            ...
            for (String arg : row.getList("argument_names", UTF8Type.instance)) argNames.add(new ColumnIdentifier(arg, true));
        }
        ...
        if (row.has("argument_types")) {
            ...
            for (String type : row.getList("argument_types", UTF8Type.instance)) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActiveStatic.get()) {
                        isSerializeLoggingActiveStatic.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(row.getList("argument_types", UTF8Type.instance), type, "type").toJsonString());
                        isSerializeLoggingActiveStatic.set(false);
                    }
                }
    ...
    private static void addAggregateToSchemaMutation(UDAggregate aggregate, long timestamp, Mutation mutation) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.schema.LegacySchemaTables.class, org.apache.cassandra.schema.LegacySchemaTables.Aggregates, "org.apache.cassandra.schema.LegacySchemaTables.Aggregates").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
        ...
        if (types == null) {
            ...
            for (String type : types) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActiveStatic.get()) {
                        isSerializeLoggingActiveStatic.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(types, type, "type").toJsonString());
                        isSerializeLoggingActiveStatic.set(false);
                    }
                }
                argTypes.add(parseType(type));
    ...
    public static ByteBuffer functionSignatureWithTypes(AbstractFunction fun) {
        ...
        for (AbstractType<?> argType : fun.argTypes()) strList.add(argType.asCQL3Type().toString());
