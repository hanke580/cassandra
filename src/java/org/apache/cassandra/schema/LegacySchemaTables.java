...
public class LegacySchemaTables {
    ...
    private static Row readSchemaPartitionForKeyspace(String schemaTableName, DecoratedKey keyspaceKey) {
        ...
        return ((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(keyspaceKey, getSchemaCFS(schemaTableName).getColumnFamily(filter)), 20));
    }
    ...
    private static Row readSchemaPartitionForTable(String schemaTableName, String keyspaceName, String tableName) {
        ...
        return ((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(key, cells), 21));
    }
    ...
    private static Set<String> mergeKeyspaces(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            ...
            post.hasColumns()) {
                created.add(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 22)));
            }
        }
    ...
    private static void mergeTables(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            ...
            if (pre.hasColumns() && post.hasColumns()) {
                ...
                created.addAll(createTablesFromTablesPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 23))).values());
            }
        }
        for (CFMetaData cfm : created) Schema.instance.addTable(cfm);
        for (CFMetaData cfm : altered) Schema.instance.updateTable(cfm.ksName, cfm.cfName);
        for (CFMetaData cfm : dropped) Schema.instance.dropTable(cfm.ksName, cfm.cfName);
    }
    ...
    private static void mergeTypes(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            ...
            if (pre.hasColumns() && post.hasColumns()) {
                ...
                created.addAll(createTypesFromPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 24))).values());
            }
        }
    ...
    private static void mergeFunctions(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            ...
            ColumnFamily post = entry.getValue().rightValue();
            if (pre.hasColumns() && post.hasColumns()) {
                MapDifference<ByteBuffer, UDFunction> delta = Maps.difference(createFunctionsFromFunctionsPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), pre), 25))), createFunctionsFromFunctionsPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 26))));
                dropped.addAll(delta.entriesOnlyOnLeft().values());
                created.addAll(delta.entriesOnlyOnRight().values());
                ...
                dropped.addAll(createFunctionsFromFunctionsPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), pre), 27))).values());
            } else if (post.hasColumns()) {
                created.addAll(createFunctionsFromFunctionsPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 28))).values());
            }
        }
    ...
    private static void mergeAggregates(Map<DecoratedKey, ColumnFamily> before, Map<DecoratedKey, ColumnFamily> after) {
        ...
        for (Map.Entry<DecoratedKey, MapDifference.ValueDifference<ColumnFamily>> entry : diff.entriesDiffering().entrySet()) {
            ...
            ColumnFamily post = entry.getValue().rightValue();
            if (pre.hasColumns() && post.hasColumns()) {
                MapDifference<ByteBuffer, UDAggregate> delta = Maps.difference(createAggregatesFromAggregatesPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), pre), 29))), createAggregatesFromAggregatesPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 30))));
                dropped.addAll(delta.entriesOnlyOnLeft().values());
                created.addAll(delta.entriesOnlyOnRight().values());
                ...
                dropped.addAll(createAggregatesFromAggregatesPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), pre), 31))).values());
            } else if (post.hasColumns()) {
                created.addAll(createAggregatesFromAggregatesPartition(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(entry.getKey(), post), 32))).values());
            }
        }
    ...
    private static Mutation makeCreateKeyspaceMutation(KSMetaData keyspace, long timestamp, boolean withTablesAndTypesAndFunctions) {
        Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(SystemKeyspace.NAME, getSchemaKSKey(keyspace.name)), 33));
    ...
    public static Mutation makeDropKeyspaceMutation(KSMetaData keyspace, long timestamp) {
        Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(SystemKeyspace.NAME, getSchemaKSKey(keyspace.name)), 34));
    ...
    private static UserType createTypeFromRow(UntypedResultSet.Row row) {
        ...
        for (String rawType : rawTypes) types.add(parseType(rawType));
    ...
    public static CFMetaData createTableFromTableRowAndColumnRows(UntypedResultSet.Row result, UntypedResultSet serializedColumnDefinitions) {
        ...
        CFMetaData cfm = ((CFMetaData) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new CFMetaData(ksName, cfName, cfType, comparator, cfId), 35));
        cfm.isDense(isDense);
    ...
    private static UDFunction createFunctionFromFunctionRow(UntypedResultSet.Row row) {
        ...
        if (row.has("argument_types"))
            for (String type : row.getList("argument_types", UTF8Type.instance)) argTypes.add(parseType(type));
