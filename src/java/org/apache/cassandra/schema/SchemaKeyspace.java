...
public final class SchemaKeyspace {
    ...
    private static final CFMetaData DroppedColumns = compile(DROPPED_COLUMNS, "dropped column registry", "CREATE TABLE %s (" + "keyspace_name text," + "table_name text," + "column_name text," + "dropped_time timestamp," + "kind text," + "type text," + "PRIMARY KEY ((keyspace_name), table_name, column_name))");
    ...
    public static void saveSystemKeyspacesSchema() {
        ...
        for (String schemaTable : ALL) {
            ...
            for (String systemKeyspace : Schema.LOCAL_SYSTEM_KEYSPACE_NAMES) executeOnceInternal(query, timestamp, systemKeyspace);
        }
    ...
    private static void addDroppedColumnToSchemaMutation(CFMetaData table, CFMetaData.DroppedColumn column, long timestamp, Mutation mutation) {
        ...
        adder.add("dropped_time", new Date(TimeUnit.MICROSECONDS.toMillis(column.droppedTime))).add("kind", null != column.kind ? column.kind.toString().toLowerCase() : null).add("type", expandUserTypes(column.type).asCQL3Type().toString()).build();
    }
    ...
    static void addFunctionToSchemaMutation(UDFunction function, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = new RowUpdateBuilder(Functions, timestamp, mutation).clustering(function.name().name, functionArgumentsList(function));
    ...
    static void addAggregateToSchemaMutation(UDAggregate aggregate, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = new RowUpdateBuilder(Aggregates, timestamp, mutation).clustering(aggregate.name().name, functionArgumentsList(aggregate));
    ...
    private static CFMetaData.DroppedColumn createDroppedColumnFromRow(UntypedResultSet.Row row) {
        ...
        ColumnDefinition.Kind kind = row.has("kind") ? ColumnDefinition.Kind.valueOf(row.getString("kind").toUpperCase()) : null;
        /*
    ...
    private static AbstractType<?> expandUserTypes(AbstractType<?> original) {
        ...
        if (original instanceof TupleType) {
            return ((TupleType) org.zlab.ocov.tracker.Runtime.update(new TupleType(expandUserTypes(((TupleType) original).allTypes())), 197, original));
        }
        if (original instanceof ListType<?>) {
            return ((org.apache.cassandra.db.marshal.ListType) org.zlab.ocov.tracker.Runtime.update(ListType.getInstance(expandUserTypes(((ListType<?>) original).getElementsType()), original.isMultiCell()), 198, original));
        }
        if (original instanceof MapType<?, ?>) {
            MapType<?, ?> mt = (MapType<?, ?>) original;
            return ((org.apache.cassandra.db.marshal.MapType) org.zlab.ocov.tracker.Runtime.update(MapType.getInstance(expandUserTypes(mt.getKeysType()), expandUserTypes(mt.getValuesType()), mt.isMultiCell()), 199, original));
        }
        if (original instanceof SetType<?>) {
            return ((org.apache.cassandra.db.marshal.SetType) org.zlab.ocov.tracker.Runtime.update(SetType.getInstance(expandUserTypes(((SetType<?>) original).getElementsType()), original.isMultiCell()), 200, original));
        }
