...
public final class SchemaKeyspace {
     ...
     */
    public static final ImmutableList<String> ALL_FOR_DIGEST = ImmutableList.of(KEYSPACES, TABLES, COLUMNS, DROPPED_COLUMNS, TRIGGERS, VIEWS, TYPES, FUNCTIONS, AGGREGATES, INDEXES);
    ...
    private static final CFMetaData Tables = compile(TABLES, "table definitions", "CREATE TABLE %s (" + "keyspace_name text," + "table_name text," + "bloom_filter_fp_chance double," + "caching frozen<map<text, text>>," + "comment text," + "compaction frozen<map<text, text>>," + "compression frozen<map<text, text>>," + "crc_check_chance double," + "dclocal_read_repair_chance double," + "default_time_to_live int," + "extensions frozen<map<text, blob>>," + // SUPER, COUNTER, DENSE, COMPOUND
    "flags frozen<set<text>>," + "gc_grace_seconds int," + "id uuid," + "max_index_interval int," + "memtable_flush_period_in_ms int," + "min_index_interval int," + "read_repair_chance double," + "speculative_retry text," + "PRIMARY KEY ((keyspace_name), table_name))");
    ...
    private static CFMetaData compile(String name, String description, String schema) {
        return CFMetaData.compile(String.format(schema, name), NAME).comment(description).gcGraceSeconds((int) TimeUnit.DAYS.toSeconds(7));
    }
    ...
    public static void saveSystemKeyspacesSchema() {
        ...
        for (String schemaTable : ALL) {
            ...
            for (String systemKeyspace : Schema.SYSTEM_KEYSPACE_NAMES) executeOnceInternal(query, timestamp, systemKeyspace);
        }
    ...
    public static Mutation makeCreateKeyspaceMutation(String name, KeyspaceParams params, long timestamp) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Keyspaces, timestamp, name).clustering(), 265));
        return adder.add(KeyspaceParams.Option.DURABLE_WRITES.toString(), params.durableWrites).frozenMap(KeyspaceParams.Option.REPLICATION.toString(), params.replication.asMap()).build();
    }
    ...
    public static Mutation makeDropKeyspaceMutation(KeyspaceMetadata keyspace, long timestamp) {
        ...
        Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(NAME, Keyspaces.decorateKey(getSchemaKSKey(keyspace.name))), 266));
        for (CFMetaData schemaTable : ALL_TABLE_METADATA) mutation.add(PartitionUpdate.fullPartitionDelete(schemaTable, mutation.key(), timestamp, nowInSec));
        return mutation;
    }
    ...
    static void addTypeToSchemaMutation(UserType type, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Types, timestamp, mutation).clustering(type.getNameAsString()), 267)).frozenList("field_names", type.fieldNames().stream().map(SchemaKeyspace::bbToString).collect(toList())).frozenList("field_types", type.fieldTypes().stream().map(AbstractType::asCQL3Type).map(CQL3Type::toString).collect(toList()));
        adder.build();
    }
    ...
    static void addTableToSchemaMutation(CFMetaData table, long timestamp, boolean withColumnsAndTriggers, Mutation mutation) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Tables, timestamp, mutation).clustering(table.cfName), 268));
        addTableParamsToSchemaMutation(table.params, adder);
        adder.add("id", table.cfId).frozenSet("flags", CFMetaData.flagsToStrings(table.flags())).build();
    ...
    public static Mutation makeDropTableMutation(KeyspaceMetadata keyspace, CFMetaData table, long timestamp) {
        ...
        for (CFMetaData.DroppedColumn column : table.getDroppedColumns().values()) dropDroppedColumnFromSchemaMutation(table, column, timestamp, mutation);
        for (TriggerMetadata trigger : table.getTriggers()) dropTriggerFromSchemaMutation(table, trigger, timestamp, mutation);
        for (IndexMetadata index : table.getIndexes()) dropIndexFromSchemaMutation(table, index, timestamp, mutation);
        return mutation;
    }

    private static void addColumnToSchemaMutation(CFMetaData table, ColumnDefinition column, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Columns, timestamp, mutation).clustering(table.cfName, column.name.toString()), 269));
        AbstractType<?> type = column.type;
        if (type instanceof ReversedType)
            type = ((ReversedType) type).baseType;
        adder.add("column_name_bytes", column.name.bytes).add("kind", column.kind.toString().toLowerCase()).add("position", column.position()).add("clustering_order", column.clusteringOrder().toString().toLowerCase()).add("type", type.asCQL3Type().toString()).build();
    }
    ...
    private static void addDroppedColumnToSchemaMutation(CFMetaData table, CFMetaData.DroppedColumn column, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(DroppedColumns, timestamp, mutation).clustering(table.cfName, column.name), 270));
        adder.add("dropped_time", new Date(TimeUnit.MICROSECONDS.toMillis(column.droppedTime))).add("type", expandUserTypes(column.type).asCQL3Type().toString()).build();
    }

    private static void addTriggerToSchemaMutation(CFMetaData table, TriggerMetadata trigger, long timestamp, Mutation mutation) {
        ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Triggers, timestamp, mutation).clustering(table.cfName, trigger.name).frozenMap("options", Collections.singletonMap("class", trigger.classOption)), 271)).build();
    }
    ...
    private static void addViewToSchemaMutation(ViewDefinition view, long timestamp, boolean includeColumns, Mutation mutation) {
        RowUpdateBuilder builder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Views, timestamp, mutation).clustering(view.viewName), 272));
        CFMetaData table = view.metadata;
        builder.add("include_all_columns", view.includeAllColumns).add("base_table_id", view.baseTableId).add("base_table_name", view.baseTableMetadata().cfName).add("where_clause", view.whereClause).add("id", table.cfId);
    ...
    public static Mutation makeUpdateViewMutation(Mutation mutation, ViewDefinition oldView, ViewDefinition newView, long timestamp) {
        ...
        // columns that are no longer needed
        for (ColumnDefinition column : columnDiff.entriesOnlyOnLeft().values()) dropColumnFromSchemaMutation(oldView.metadata, column, timestamp, mutation);
        // newly added columns
    ...
    private static void addIndexToSchemaMutation(CFMetaData table, IndexMetadata index, long timestamp, Mutation mutation) {
        RowUpdateBuilder builder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Indexes, timestamp, mutation).clustering(table.cfName, index.name), 273));
    ...
    static void addFunctionToSchemaMutation(UDFunction function, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Functions, timestamp, mutation).clustering(function.name().name, functionArgumentsList(function)), 274));
        adder.add("body", function.body()).add("language", function.language()).add("return_type", function.returnType().asCQL3Type().toString()).add("called_on_null_input", function.isCalledOnNullInput()).frozenList("argument_names", function.argNames().stream().map((c) -> bbToString(c.bytes)).collect(toList()));
        adder.build();
    }

    private static List<String> functionArgumentsList(AbstractFunction fun) {
        return fun.argTypes().stream().map(AbstractType::asCQL3Type).map(CQL3Type::toString).collect(toList());
    }
    ...
    static void addAggregateToSchemaMutation(UDAggregate aggregate, long timestamp, Mutation mutation) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Aggregates, timestamp, mutation).clustering(aggregate.name().name, functionArgumentsList(aggregate)), 275));
        adder.add("return_type", aggregate.returnType().asCQL3Type().toString()).add("state_func", aggregate.stateFunction().name().name).add("state_type", aggregate.stateType().asCQL3Type().toString()).add("final_func", aggregate.finalFunction() != null ? aggregate.finalFunction().name().name : null).add("initcond", aggregate.initialCondition() != null ? // must use the frozen state type here, as 'null' for unfrozen collections may mean 'empty'
        aggregate.stateType().freeze().asCQL3Type().toCQLLiteral(aggregate.initialCondition(), Server.CURRENT_VERSION) : null).build();
    }
    ...
    private static Views fetchViews(String keyspaceName, Types types) {
        ...
        for (UntypedResultSet.Row row : query(query, keyspaceName)) views.add(fetchView(keyspaceName, row.getString("view_name"), types));
        return views.build();
    }
    ...
    private static UDFunction createUDFFromRow(UntypedResultSet.Row row, Types types) {
        ...
        if (existing instanceof UDFunction) {
            ...
            if (// arg types checked in Functions.find call
            udf.argNames().equals(argNames) && udf.returnType().equals(returnType) && !udf.isAggregate() && udf.language().equals(language) && udf.body().equals(body) && udf.isCalledOnNullInput() == calledOnNullInput) {
    ...
    private static UDAggregate createUDAFromRow(UntypedResultSet.Row row, Functions functions, Types types) {
        ...
        FunctionName name = new FunctionName(ksName, functionName);
        List<AbstractType<?>> argTypes = row.getFrozenList("argument_types", UTF8Type.instance).stream().map(t -> parse(ksName, t, types)).collect(toList());
        AbstractType<?> returnType = parse(ksName, row.getString("return_type"), types);
    ...
    public static synchronized void mergeSchema(Collection<Mutation> mutations) {
        ...
        // updated keyspaces
        for (Map.Entry<String, MapDifference.ValueDifference<KeyspaceMetadata>> diff : keyspacesDiff.entriesDiffering().entrySet()) updateKeyspace(diff.getKey(), diff.getValue().leftValue(), diff.getValue().rightValue());
    }
    ...
    private static void updateKeyspace(String keyspaceName, KeyspaceMetadata keyspaceBefore, KeyspaceMetadata keyspaceAfter) {
        ...
        for (MapDifference.ValueDifference<UserType> diff : typesDiff.entriesDiffering().values()) Schema.instance.updateType(diff.rightValue());
        for (MapDifference.ValueDifference<CFMetaData> diff : tablesDiff.entriesDiffering().values()) Schema.instance.updateTable(diff.rightValue());
        for (MapDifference.ValueDifference<ViewDefinition> diff : viewsDiff.entriesDiffering().values()) Schema.instance.updateView(diff.rightValue());
        for (MapDifference.ValueDifference<UDFunction> diff : udfsDiff.entriesDiffering().values()) Schema.instance.updateFunction(diff.rightValue());
        for (MapDifference.ValueDifference<UDAggregate> diff : udasDiff.entriesDiffering().values()) Schema.instance.updateAggregate(diff.rightValue());
    }
    ...
    private static AbstractType<?> expandUserTypes(AbstractType<?> original) {
        if (original instanceof UserType) {
            return ((TupleType) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new TupleType(expandUserTypes(((UserType) original).fieldTypes())), 276));
        }
        if (original instanceof TupleType) {
            return ((TupleType) org.zlab.ocov.tracker.Runtime.update(new TupleType(expandUserTypes(((TupleType) original).allTypes())), 132, original));
        }
        if (original instanceof ListType<?>) {
            return ((org.apache.cassandra.db.marshal.ListType) org.zlab.ocov.tracker.Runtime.update(ListType.getInstance(expandUserTypes(((ListType<?>) original).getElementsType()), original.isMultiCell()), 133, original));
        }
        if (original instanceof MapType<?, ?>) {
            MapType<?, ?> mt = (MapType<?, ?>) original;
            return ((org.apache.cassandra.db.marshal.MapType) org.zlab.ocov.tracker.Runtime.update(MapType.getInstance(expandUserTypes(mt.getKeysType()), expandUserTypes(mt.getValuesType()), mt.isMultiCell()), 134, original));
        }
        if (original instanceof SetType<?>) {
            return ((org.apache.cassandra.db.marshal.SetType) org.zlab.ocov.tracker.Runtime.update(SetType.getInstance(expandUserTypes(((SetType<?>) original).getElementsType()), original.isMultiCell()), 135, original));
        }
