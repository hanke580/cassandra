...
public final class LegacySchemaMigrator {
    ...
    public static void migrate() {
        ...
        logger.info("Moving {} keyspaces from legacy schema tables to the new schema keyspace ({})", keyspaces.size(), SchemaKeyspace.NAME);
    ...
    private static void migrateBuiltIndexesForTable(Table table) {
        table.metadata.getIndexes().forEach((index) -> migrateIndexBuildStatus(table.metadata.ksName, table.metadata.cfName, index));
    }
    ...
    static void unloadLegacySchemaTables() {
        ...
        for (CFMetaData table : LegacySchemaTables) systemTables = systemTables.without(table.cfName);
    ...
    private static long readKeyspaceTimestamp(String keyspaceName) {
        String query = format("SELECT writeTime(durable_writes) AS timestamp FROM %s.%s WHERE keyspace_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_KEYSPACES);
    ...
    private static KeyspaceParams readKeyspaceParams(String keyspaceName) {
        String query = format("SELECT * FROM %s.%s WHERE keyspace_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_KEYSPACES);
    ...
    private static Collection<Table> readTables(String keyspaceName) {
        String query = format("SELECT columnfamily_name FROM %s.%s WHERE keyspace_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_COLUMNFAMILIES);
    ...
    private static long readTableTimestamp(String keyspaceName, String tableName) {
        String query = format("SELECT writeTime(type) AS timestamp FROM %s.%s WHERE keyspace_name = ? AND columnfamily_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_COLUMNFAMILIES);
    ...
    private static CFMetaData decodeTableMetadata(String tableName, UntypedResultSet.Row tableRow, UntypedResultSet columnRows, UntypedResultSet triggerRows) {
        ...
        CFMetaData cfm = CFMetaData.create(ksName, cfName, cfId, isDense, isCompound, isSuper, isCounter, // legacy schema did not contain views
        false, columnDefs, DatabaseDescriptor.getPartitioner());
    ...
    private static boolean calculateIsDense(AbstractType<?> comparator, UntypedResultSet columnRows, boolean isSuper) {
        ...
        for (UntypedResultSet.Row columnRow : columnRows) if ("clustering_key".equals(columnRow.getString("type")))
    ...
    private static void addDefinitionForUpgrade(List<ColumnDefinition> defs, String ksName, String cfName, boolean isStaticCompactTable, boolean isSuper, AbstractType<?> rawComparator, AbstractType<?> subComparator, AbstractType<?> defaultValidator) {
        ...
        if (isSuper) {
            ...
            AbstractType<?> compactColumnType = Boolean.getBoolean("cassandra.init_dense_table_compact_value_as_bytes") ? BytesType.instance : EmptyType.instance;
    ...
    private static boolean hasKind(Iterable<UntypedResultSet.Row> defs, ColumnDefinition.Kind kind) {
        for (UntypedResultSet.Row row : defs) if (deserializeKind(row.getString("type")) == kind)
            return true;
        return false;
    }
    ...
    private static Collection<Type> readTypes(String keyspaceName) {
        String query = format("SELECT type_name FROM %s.%s WHERE keyspace_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_USERTYPES);
    ...
    private static long readTypeTimestamp(String keyspaceName, String typeName) {
        ...
        Slices slices = Slices.with(comparator, ((org.apache.cassandra.db.Slice) org.zlab.ocov.tracker.Runtime.update(Slice.make(comparator, typeName), 195, keyspaceName, typeName)));
        ...
        org.zlab.ocov.tracker.Runtime.update(command, 196, keyspaceName, typeName);
    ...
    private static UserType readTypeMetadata(String keyspaceName, String typeName) {
        String query = format("SELECT * FROM %s.%s WHERE keyspace_name = ? AND type_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_USERTYPES);
    ...
    private static Collection<Function> readFunctions(String keyspaceName) {
        String query = format("SELECT function_name, signature FROM %s.%s WHERE keyspace_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_FUNCTIONS);
    ...
    private static long readFunctionTimestamp(String keyspaceName, String functionName, List<String> signature) {
        String query = format("SELECT writeTime(return_type) AS timestamp " + "FROM %s.%s " + "WHERE keyspace_name = ? AND function_name = ? AND signature = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_FUNCTIONS);
    ...
    private static UDFunction readFunctionMetadata(String keyspaceName, String functionName, List<String> signature) {
        String query = format("SELECT * FROM %s.%s WHERE keyspace_name = ? AND function_name = ? AND signature = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_FUNCTIONS);
    ...
    private static Collection<Aggregate> readAggregates(Functions functions, String keyspaceName) {
        String query = format("SELECT aggregate_name, signature FROM %s.%s WHERE keyspace_name = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_AGGREGATES);
    ...
    private static long readAggregateTimestamp(String keyspaceName, String aggregateName, List<String> signature) {
        String query = format("SELECT writeTime(return_type) AS timestamp " + "FROM %s.%s " + "WHERE keyspace_name = ? AND aggregate_name = ? AND signature = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_AGGREGATES);
    ...
    private static UDAggregate readAggregateMetadata(Functions functions, String keyspaceName, String functionName, List<String> signature) {
        String query = format("SELECT * FROM %s.%s WHERE keyspace_name = ? AND aggregate_name = ? AND signature = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_AGGREGATES);
        UntypedResultSet.Row row = query(query, keyspaceName, functionName, signature).one();
        FunctionName name = new FunctionName(keyspaceName, functionName);
