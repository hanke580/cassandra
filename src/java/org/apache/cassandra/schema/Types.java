...
public final class Types implements Iterable<UserType> {
    ...
    public static final class Builder {
        ...
        public Types build() {
            return ((Types) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Types(this), 262));
        }
    ...
    public static final class RawBuilder {
        ...
        public Types build() {
            ...
            Types types = ((Types) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Types(new HashMap<>()), 263));
        ...
        private static final class RawUDT {
            ...
            UserType prepare(String keyspace, Types types) {
                List<ByteBuffer> preparedFieldNames = fieldNames.stream().map(ByteBufferUtil::bytes).collect(toList());
                List<AbstractType<?>> preparedFieldTypes = fieldTypes.stream().map(t -> t.prepareInternal(keyspace, types).getType()).collect(toList());
                return ((UserType) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new UserType(keyspace, bytes(name), preparedFieldNames, preparedFieldTypes), 264));
            }
