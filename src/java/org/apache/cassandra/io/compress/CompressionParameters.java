...
public class CompressionParameters {
    ...
    // Unrecognized options, can be use by the compressor
    ...
    public int chunkLength() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.chunkLength, "this.chunkLength").toJsonString());
            ...
            for (String provided : compressionOptions.keySet()) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActiveStatic.get()) {
                        ...
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(compressionOptions.keySet(), provided, "provided").toJsonString());
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(29, 1597).append(sstableCompressor).append(chunkLength).append(otherOptions).toHashCode();
    }
