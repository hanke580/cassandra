...
public class BigTableWriter extends SSTableWriter {
    ...
    public BigTableWriter(Descriptor descriptor, Long keyCount, Long repairedAt, CFMetaData metadata, MetadataCollector metadataCollector, SerializationHeader header, LifecycleTransaction txn) {
        super(descriptor, keyCount, repairedAt, metadata, metadataCollector, header);
        // must track before any files are created
        txn.trackNew(this);
        if (compression) {
            dataFile = SequentialWriter.open(getFilename(), descriptor.filenameFor(Component.COMPRESSION_INFO), metadata.params.compression, metadataCollector);
    ...
    class IndexWriter extends AbstractTransactional implements Transactional {
        ...
        protected void doPrepare() {
            ...
            try (IndexSummary summary = iwriter.summary.build(getPartitioner())) {
                org.zlab.ocov.tracker.Runtime.update(summary, 141);
