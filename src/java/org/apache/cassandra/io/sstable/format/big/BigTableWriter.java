...
public class BigTableWriter extends SSTableWriter {
    ...
    public BigTableWriter(Descriptor descriptor, Long keyCount, Long repairedAt, CFMetaData metadata, MetadataCollector metadataCollector, SerializationHeader header, LifecycleNewTracker lifecycleNewTracker) {
        super(descriptor, keyCount, repairedAt, metadata, metadataCollector, header);
        // must track before any files are created
        lifecycleNewTracker.trackNew(this);
        if (compression) {
            dataFile = SequentialWriter.open(getFilename(), descriptor.filenameFor(Component.COMPRESSION_INFO), metadata.compressionParams(), metadataCollector);
    ...
    public RowIndexEntry append(UnfilteredRowIterator iterator) {
        ...
        try (UnfilteredRowIterator collecting = Transformation.apply(iterator, new StatsCollector(metadataCollector))) {
            ...
            org.zlab.ocov.tracker.Runtime.update(index, 206, iterator);
            RowIndexEntry entry = RowIndexEntry.create(startPosition, collecting.partitionLevelDeletion(), index);
            org.zlab.ocov.tracker.Runtime.update(entry, 207, iterator);
    ...
    private SSTableReader openFinal(Descriptor desc, SSTableReader.OpenReason openReason) {
        ...
        SSTableReader sstable = SSTableReader.internalOpen(desc, components, this.metadata, ifile, dfile, indexSummary, iwriter.bf.sharedCopy(), maxDataAge, stats, openReason, header);
        sstable.first = getMinimalKey(first);
    ...
    class IndexWriter extends AbstractTransactional implements Transactional {
        ...
        protected void doPrepare() {
            ...
            try (IndexSummary summary = iwriter.summary.build(getPartitioner())) {
                org.zlab.ocov.tracker.Runtime.update(summary, 208);
