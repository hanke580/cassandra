...
public class SSTableRewriter extends Transactional.AbstractTransactional implements Transactional {
    ...
    // true for operations that are performed without Cassandra running (prevents updates of Tracker)
    ...
    public SSTableRewriter(ColumnFamilyStore cfs, LifecycleTransaction transaction, long maxAge, boolean isOffline, long preemptiveOpenInterval) {
        ...
        for (SSTableReader sstable : this.transaction.originals()) fileDescriptors.put(sstable.descriptor, CLibrary.getfd(sstable.getFilename()));
        this.cfs = cfs;
        this.maxAge = maxAge;
                ...
                if (save) {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            ...
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(row, row.key, "row.key").toJsonString());
            ...
            if (isOffline) {
                for (SSTableReader reader : transaction.originals()) {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(transaction.originals(), reader, "reader").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
    ...
    private void moveStarts(SSTableReader newReader, DecoratedKey lowerbound) {
        ...
        for (Map.Entry<DecoratedKey, RowIndexEntry> cacheKey : cachedKeys.entrySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(cachedKeys.entrySet(), cacheKey, "cacheKey").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        ...
        for (SSTableReader sstable : transaction.originals()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(transaction.originals(), sstable, "sstable").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        ...
        if (writer == null || writer.getFilePointer() == 0) {
            if (writer != null) {
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.writer, "this.writer").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
