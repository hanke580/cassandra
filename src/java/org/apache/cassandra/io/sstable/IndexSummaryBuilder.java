...
public class IndexSummaryBuilder implements AutoCloseable {
    ...
    public IndexSummaryBuilder(long expectedKeys, int minIndexInterval, int samplingLevel) {
        ...
        if (maxExpectedEntries > Integer.MAX_VALUE) {
            ...
            logger.warn("min_index_interval of {} is too low for {} expected keys; using interval of {} instead", minIndexInterval, expectedKeys, effectiveMinInterval);
    ...
    public IndexSummary build(IPartitioner partitioner, ReadableBoundary boundary) {
        ...
        assert count > 0;
        return ((IndexSummary) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexSummary(partitioner, offsets.currentBuffer().sharedCopy(), count, entries.currentBuffer().sharedCopy(), entriesLength, sizeAtFullSampling, minIndexInterval, samplingLevel), 8));
    }
