    ...
    public IndexSummaryBuilder maybeAddEntry(DecoratedKey decoratedKey, long indexStart, long indexEnd, long dataEnd) throws IOException {
        if (keysWritten == nextSamplePosition) {
            if ((entries.length() + getEntrySize(decoratedKey)) <= Integer.MAX_VALUE) {
                ...
                logger.error("Memory capacity of index summary exceeded (2GiB), index summary will not cover full sstable, " + "you should increase min_sampling_level");
            }
        } else if (dataEnd != 0 && keysWritten + 1 == nextSamplePosition) {
            // this is the last key in this summary interval, so stash it
            ReadableBoundary boundary = ((ReadableBoundary) org.zlab.ocov.tracker.Runtime.update(new ReadableBoundary(decoratedKey, indexEnd, dataEnd, (int) (offsets.length() / 4), entries.length()), 29, decoratedKey, indexStart, indexEnd, dataEnd));
