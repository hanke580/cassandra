...
public class IndexSummaryBuilder implements AutoCloseable {
    ...
    public IndexSummary build(IPartitioner partitioner, ReadableBoundary boundary) {
        ...
        assert count > 0;
        return ((IndexSummary) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexSummary(partitioner, offsets.currentBuffer().sharedCopy(), count, entries.currentBuffer().sharedCopy(), entriesLength, sizeAtFullSampling, minIndexInterval, samplingLevel), 288));
    }
