    ...
    public ByteBuffer[] asByteBuffers(long offset, long length) {
        ...
        if (size() == 0) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.io.util.Memory.class, org.apache.cassandra.io.util.Memory.NO_BYTE_BUFFERS, "org.apache.cassandra.io.util.Memory.NO_BYTE_BUFFERS").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
