...
public class ColumnCondition {
        ...
        public boolean appliesTo(Composite rowPrefix, ColumnFamily current, final long now) throws InvalidRequestException {
            ...
            if (column.type instanceof MapType) {
                ...
                if (column.type.isMultiCell()) {
                    ...
                    ByteBuffer mapElementValue = cell.isLive(now) ? mapType.getSerializer().getSerializedValue(cell.value(), collectionElement, mapType.getKeysType()) : null;
            ...
            if (column.type.isMultiCell()) {
                ByteBuffer columnValue = getListItem(collectionColumns(current.metadata().comparator.create(rowPrefix, column), current, now), getListIndex(collectionElement));
                ...
                Cell cell = current.getColumn(current.metadata().comparator.create(rowPrefix, column));
                ByteBuffer listElementValue = cell.isLive(now) ? listType.getSerializer().getElement(cell.value(), getListIndex(collectionElement)) : null;
    ...
    static class ElementAccessInBound extends Bound {
        ...
        public boolean appliesTo(Composite rowPrefix, ColumnFamily current, final long now) throws InvalidRequestException {
            ...
            if (column.type.isMultiCell()) {
                ByteBuffer columnValue = ElementAccessBound.getListItem(collectionColumns(nameType.create(rowPrefix, column), current, now), ElementAccessBound.getListIndex(collectionElement));
                for (ByteBuffer value : inValues) {
    ...
    public static class CollectionInBound extends Bound {
        ...
        private CollectionInBound(ColumnCondition condition, QueryOptions options) throws InvalidRequestException {
            ...
            if (condition.inValues == null) {
                ...
                for (Term value : condition.inValues) this.inValues.add(value.bind(options));
            }
        }
