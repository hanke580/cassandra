...
public final class DropFunctionStatement extends SchemaAlteringStatement {
    ...
    public void checkAccess(ClientState state) throws UnauthorizedException, InvalidRequestException {
        ...
        if (function == null) {
            if (!ifExists)
                throw new InvalidRequestException(String.format("Unconfigured function %s.%s(%s)", functionName.keyspace, functionName.name, Joiner.on(",").join(argRawTypes)));
        } else {
    ...
    public void validate(ClientState state) {
        ...
        if (!argsPresent && olds != null && olds.size() > 1)
            throw new InvalidRequestException(String.format("'DROP FUNCTION %s' matches multiple function definitions; " + "specify the argument types by issuing a statement like " + "'DROP FUNCTION %s (type, type, ...)'. Hint: use cqlsh " + "'DESCRIBE FUNCTION %s' command to find all overloads", functionName, functionName, functionName));
    }
