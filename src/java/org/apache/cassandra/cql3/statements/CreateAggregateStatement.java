...
public final class CreateAggregateStatement extends SchemaAlteringStatement {
    ...
    public Prepared prepare() {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.stateFunc, "this.stateFunc").toJsonString());
        ...
        if (finalFunc != null) {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.finalFunc, "this.finalFunc").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public void checkAccess(ClientState state) throws UnauthorizedException, InvalidRequestException {
        ...
        if (Functions.find(functionName, argTypes) != null && orReplace) {
            ...
            state.ensureHasPermission(Permission.ALTER, FunctionResource.function(functionName.keyspace, functionName.name, argTypes));
        } else {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
        ...
        for (Function referencedFunction : stateFunction.getFunctions()) state.ensureHasPermission(Permission.EXECUTE, referencedFunction);
        if (finalFunction != null)
            for (Function referencedFunction : finalFunction.getFunctions()) state.ensureHasPermission(Permission.EXECUTE, referencedFunction);
    }
    ...
    public boolean announceMigration(boolean isLocalOnly) throws RequestValidationException {
        ...
        if (old != null) {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.returnType, "this.returnType").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            if (!old.returnType().isValueCompatibleWith(returnType))
                throw new InvalidRequestException(String.format("Cannot replace aggregate %s, the new return type %s is not compatible with the return type %s of existing function", functionName, returnType.asCQL3Type(), old.returnType().asCQL3Type()));
        }
    ...
    private static String stateFuncSig(FunctionName stateFuncName, CQL3Type.Raw stateTypeRaw, List<CQL3Type.Raw> argRawTypes) {
        ...
        for (CQL3Type.Raw argRawType : argRawTypes) sb.append(", ").append(argRawType);
        sb.append(')');
        return sb.toString();
    }
