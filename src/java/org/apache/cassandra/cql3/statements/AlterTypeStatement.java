...
public abstract class AlterTypeStatement extends SchemaAlteringStatement {
    ...
    public boolean announceMigration(boolean isLocalOnly) throws InvalidRequestException, ConfigurationException {
        ...
        for (UserType ut : ksm.userTypes.getAllTypes().values()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(ksm.userTypes.getAllTypes().values(), ut, "ut").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    private static int getIdxOfField(UserType type, ColumnIdentifier field) {
        for (int i = 0; i < type.size(); i++) if (field.bytes.equals(type.fieldName(i)))
            return i;
        return -1;
    }
    ...
    private boolean updateDefinition(CFMetaData cfm, ColumnDefinition def, String keyspace, ByteBuffer toReplace, UserType updated) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(def, def.type, "def.type").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        ...
        if (type instanceof UserType) {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(ut, ut.name, "ut.name").toJsonString());
    ...
    private static class AddOrAlter extends AlterTypeStatement {
        ...
        private UserType doAdd(UserType toUpdate) throws InvalidRequestException {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(toUpdate, toUpdate.name, "toUpdate.name").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
