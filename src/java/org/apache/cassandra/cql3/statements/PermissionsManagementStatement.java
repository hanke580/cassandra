...
public abstract class PermissionsManagementStatement extends AuthorizationStatement {
    ...
    public void validate(ClientState state) throws RequestValidationException {
        ...
        if (resource instanceof FunctionResource && SystemKeyspace.NAME.equals(((FunctionResource) resource).getKeyspace())) {
    ...
    public void checkAccess(ClientState state) throws UnauthorizedException {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.permissions, "this.permissions").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        ...
        for (Permission p : permissions) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(permissions, p, "p").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
