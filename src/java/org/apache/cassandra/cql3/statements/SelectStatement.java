...
public class SelectStatement implements CQLStatement {
    ...
    public Iterable<Function> getFunctions() {
        return Iterables.concat(selection.getFunctions(), restrictions.getFunctions(), limit != null ? limit.getFunctions() : Collections.<Function>emptySet());
    }
    ...
    private ResultMessage.Rows execute(Pageable command, QueryOptions options, int limit, long now, QueryState state) throws RequestValidationException, RequestExecutionException {
        ...
        if (command == null) {
            ...
            rows = command instanceof Pageable.ReadCommands ? StorageProxy.read(((Pageable.ReadCommands) command).commands, options.getConsistency(), state.getClientState()) : StorageProxy.getRangeSlice((RangeSliceCommand) command, options.getConsistency());
        }
    ...
    private ColumnSlice makeStaticSlice() {
        ...
        return isReversed ? new ColumnSlice(cfm.comparator.staticPrefix().end(), Composites.EMPTY) : new ColumnSlice(Composites.EMPTY, cfm.comparator.staticPrefix().end());
    }
    ...
    private IDiskAtomFilter makeFilter(QueryOptions options, int limit) throws InvalidRequestException {
        ...
        if (parameters.isDistinct) {
            ...
            ColumnSlice staticSlice = selection.containsStaticColumns() && !restrictions.usesSecondaryIndexing() ? makeStaticSlice() : null;
            ...
            if (startBounds.size() == 1) {
                ...
                if (isReversed) {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(staticSlice, staticSlice.start, "staticSlice.start").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
                    return slice.includes(cfm.comparator.reverseComparator(), staticSlice.start) ? sliceFilter(new ColumnSlice(slice.start, staticSlice.finish), limit, toGroup) : sliceFilter(new ColumnSlice[] { slice, staticSlice }, limit, toGroup);
                } else {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(staticSlice, staticSlice.finish, "staticSlice.finish").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
                    return slice.includes(cfm.comparator, staticSlice.finish) ? sliceFilter(new ColumnSlice(staticSlice.start, slice.finish), limit, toGroup) : sliceFilter(new ColumnSlice[] { staticSlice, slice }, limit, toGroup);
                }
            }
            ...
            if (// in case of IN () for the last column of the key
            cellNames == null)
                return null;
    ...
    private int updateLimitForQuery(int limit) {
        ...
        return restrictions.isNonCompositeSliceWithExclusiveBounds() && limit != Integer.MAX_VALUE ? limit + 1 : limit;
    }
    ...
    private SortedSet<CellName> getRequestedColumns(QueryOptions options) throws InvalidRequestException {
        ...
        for (Composite composite : restrictions.getClusteringColumnsAsComposites(options)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(restrictions.getClusteringColumnsAsComposites(options), composite, "composite").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (cfm.comparator.isCompound() && !cfm.isSuper()) {
                ...
                for (ColumnDefinition def : selection.getColumns()) {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(selection.getColumns(), def, "def").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
                ...
                for (ColumnDefinition def : cfm.regularColumns()) {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(cfm.regularColumns(), def, "def").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
                    columns.add(cfm.comparator.create(prefix, def));
    ...
    public static class RawStatement extends CFStatement {
        ...
        private StatementRestrictions prepareRestrictions(CFMetaData cfm, VariableSpecifications boundNames, Selection selection) throws InvalidRequestException {
            try {
                return new StatementRestrictions(cfm, whereClause, boundNames, selection.containsOnlyStaticColumns(), selection.containsACollection(), parameters.allowFiltering);
        ...
        private static void validateDistinctSelection(CFMetaData cfm, Selection selection, StatementRestrictions restrictions) throws InvalidRequestException {
            ...
            for (ColumnDefinition def : requestedColumns) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActiveStatic.get()) {
                        isSerializeLoggingActiveStatic.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(requestedColumns, def, "def").toJsonString());
                        isSerializeLoggingActiveStatic.set(false);
                    }
                }
        ...
        private boolean isReversed(CFMetaData cfm) throws InvalidRequestException {
            ...
            for (Map.Entry<ColumnIdentifier.Raw, Boolean> entry : parameters.orderings.entrySet()) {
                ...
                if (def == null)
                    handleUnrecognizedOrderingColumn(column);
                checkTrue(def.isClusteringColumn(), "Order by is currently only supported on the clustered columns of the PRIMARY KEY, got %s", column);
                checkTrue(i++ == def.position(), "Order by currently only support the ordering of columns following their declared order in the PRIMARY KEY");
        ...
        private SingleColumnRelation findInclusiveClusteringRelationForCompact(CFMetaData cfm) {
            for (Relation r : whereClause) {
                ...
                if (cfm.getColumnDefinition(rel.getEntity().prepare(cfm)).isClusteringColumn() && (rel.operator() == Operator.GT || rel.operator() == Operator.LT))
                    return rel;
            }
        ...
        public String toString() {
            return Objects.toStringHelper(this).add("name", cfName).add("selectClause", selectClause).add("whereClause", whereClause).add("isDistinct", parameters.isDistinct).toString();
        }
    }
