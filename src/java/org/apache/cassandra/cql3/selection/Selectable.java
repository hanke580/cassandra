...
public abstract class Selectable {
    ...
    public abstract Selector.Factory newSelectorFactory(CFMetaData cfm, List<ColumnDefinition> defs) throws InvalidRequestException;
    ...
    public static class WritetimeOrTTL extends Selectable {
        ...
        public Selector.Factory newSelectorFactory(CFMetaData cfm, List<ColumnDefinition> defs) throws InvalidRequestException {
            ...
            if (def.isPrimaryKeyColumn())
                throw new InvalidRequestException(String.format("Cannot use selection function %s on PRIMARY KEY part %s", isWritetime ? "writeTime" : "ttl", def.name));
            if (def.type.isCollection())
                throw new InvalidRequestException(String.format("Cannot use selection function %s on collections", isWritetime ? "writeTime" : "ttl"));
    ...
    public static class WithFunction extends Selectable {
        ...
        @Override
        public String toString() {
            return new StrBuilder().append(functionName).append("(").appendWithSeparators(args, ", ").append(")").toString();
        }
        ...
        public static class Raw implements Selectable.Raw {
            ...
            public WithFunction prepare(CFMetaData cfm) {
                ...
                for (Selectable.Raw arg : args) preparedArgs.add(arg.prepare(cfm));
