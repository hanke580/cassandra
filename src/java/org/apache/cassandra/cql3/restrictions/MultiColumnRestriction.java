...
public abstract class MultiColumnRestriction extends AbstractRestriction {
    ...
    public List<ColumnDefinition> getColumnDefs() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.columnDefs, "this.columnDefs").toJsonString());
    ...
    public static class EQ extends MultiColumnRestriction {
        ...
        public CompositesBuilder appendTo(CFMetaData cfm, CompositesBuilder builder, QueryOptions options) {
            ...
            for (int i = 0, m = values.size(); i < m; i++) {
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(values, values.get(i), "values.get(i)").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
        ...
        public boolean isNotReturningAnyRows(CFMetaData cfm, QueryOptions options) {
            ...
            return !cfm.comparator.isCompound() && !((Tuples.Value) value.bind(options)).getElements().get(0).hasRemaining();
        }
    }
    ...
    public static class Slice extends MultiColumnRestriction {
        ...
        public boolean isNotReturningAnyRows(CFMetaData cfm, QueryOptions options) {
            ...
            return !cfm.comparator.isCompound() && hasBound(Bound.END) && !componentBounds(Bound.END, options).get(0).hasRemaining();
        }
