...
public abstract class QueryOptions {
    ...
    public static final QueryOptions DEFAULT = new DefaultQueryOptions(ConsistencyLevel.ONE, Collections.<ByteBuffer>emptyList(), false, SpecificOptions.DEFAULT, Server.CURRENT_VERSION);
    ...
    static class DefaultQueryOptions extends QueryOptions {
        ...
        SpecificOptions getSpecificOptions() {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.options, "this.options").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            return options;
    ...
    static class OptionsWithNames extends QueryOptionsWrapper {
                ...
                for (int j = 0; j < names.size(); j++) {
                    if (name.equals(names.get(j))) {
                        ...
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(wrapped.getValues(), wrapped.getValues().get(j), "wrapped.getValues().get(j)").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
                        break;
        ...
        public List<ByteBuffer> getValues() {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.orderedValues, "this.orderedValues").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
