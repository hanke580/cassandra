...
public abstract class MergeIterator<In, Out> extends AbstractIterator<Out> implements IMergeIterator<In, Out> {
    ...
    private static final class ManyToOne<In, Out> extends MergeIterator<In, Out> {
        ...
        protected final void advance() {
            ...
            while ((candidate = candidates.pollFirst()) != null) if (candidate.advance())
                queue.add(candidate);
        }
    }
        ...
        public int compareTo(Candidate<In> that) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(that, that.item, "that.item").toJsonString());
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.item, "this.item").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
