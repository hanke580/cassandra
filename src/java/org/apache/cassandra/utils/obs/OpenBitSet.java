...
public class OpenBitSet implements IBitSet {
    ...
    // number of words (elements) used in the array
    private int wlen;
    ...
    public long[] getPage(int pageIdx) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.bits, "this.bits").toJsonString());
    ...
    public int getNumWords() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.wlen, "this.wlen").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        return wlen;
    ...
    public long cardinality() {
        long bitCount = 0L;
        for (int i = getPageCount(); i-- > 0; ) bitCount += BitUtil.pop_array(bits[i], 0, wlen);
        return bitCount;
    }
        ...
        for (int p = 0; p < pageCount; p++) {
            ...
            for (int i = 0; i < pageSize && bitLength-- > 0; i++) {
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(bits, bits[i], "bits[i]").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
            }
    ...
    public long serializedSize(TypeSizes type) {
        ...
        for (int p = 0; p < pageCount; p++) {
            long[] bits = getPage(p);
            for (int i = 0; i < pageSize && bitLength-- > 0; i++) // bucket
            size += type.sizeof(bits[i]);
        }
        return size;
    }
    ...
    public static OpenBitSet deserialize(DataInput in) throws IOException {
        ...
        for (int p = 0; p < pageCount; p++) {
            ...
            for (int i = 0; i < pageSize && bitLength-- > 0; i++) bits[i] = in.readLong();
        }
        return bs;
    }
}
