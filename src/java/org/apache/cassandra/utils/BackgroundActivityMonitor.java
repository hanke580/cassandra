...
public class BackgroundActivityMonitor {
    ...
    private long[] readAndCompute() throws IOException {
        ...
        for (int i = 0; i < returned.length; i++) returned[i] = Long.parseLong(tokenizer.nextToken());
        return returned;
    }
    ...
    private float compareAtIndex(long[] reading1, long[] reading2, int index) {
        ...
        if (diff == 0)
            return 0f;
        // yes it is hard coded to 100 [update
    ...
    public double getSeverity(InetAddress endpoint) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.gms.ApplicationState.class, org.apache.cassandra.gms.ApplicationState.SEVERITY, "org.apache.cassandra.gms.ApplicationState.SEVERITY").toJsonString());
    ...
    public class BackgroundActivityReporter implements Runnable {
        ...
        public void run() {
            ...
            if (!Gossiper.instance.isEnabled())
                return;
            // add manual severity setting.
            report += manual_severity.get();
