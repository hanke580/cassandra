...
public class MerkleTree implements Serializable {
    ...
    public static class MerkleTreeSerializer implements IVersionedSerializer<MerkleTree> {
        ...
        public void serialize(MerkleTree mt, DataOutputPlus out, int version) throws IOException {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(mt, mt.hashdepth, "mt.hashdepth").toJsonString());
        ...
        public long serializedSize(MerkleTree mt, int version) {
            ...
            1 + TypeSizes.NATIVE.sizeof(mt.maxsize) + TypeSizes.NATIVE.sizeof(mt.size) + TypeSizes.NATIVE.sizeof(mt.partitioner.getClass().getCanonicalName());
            // full range
            ...
            public void serialize(Inner inner, DataOutputPlus out, int version) throws IOException {
                if (inner.hash == null)
                    ...
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(inner, inner.hash, "inner.hash").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
    ...
    static class Leaf extends Hashable {
        ...
        private static class LeafSerializer implements IPartitionerDependentSerializer<Leaf> {
            ...
            public void serialize(Leaf leaf, DataOutputPlus out, int version) throws IOException {
                if (leaf.hash == null) {
                    ...
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(leaf, leaf.hash, "leaf.hash").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
                    out.write(leaf.hash);
            ...
            public long serializedSize(Leaf leaf, int version) {
                return leaf.hash == null ? TypeSizes.NATIVE.sizeof(-1) : TypeSizes.NATIVE.sizeof(leaf.hash().length) + leaf.hash().length;
            }
        }
    }
