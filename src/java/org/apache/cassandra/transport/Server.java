...
public class Server implements CassandraDaemon.Server {
    ...
    private static class Initializer extends ChannelInitializer {
        ...
        protected void initChannel(Channel channel) throws Exception {
            ...
            if (DatabaseDescriptor.getNativeTransportMaxConcurrentConnections() > 0 || DatabaseDescriptor.getNativeTransportMaxConcurrentConnectionsPerIp() > 0) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        ...
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.connectionLimitHandler, "org.apache.cassandra.transport.Server.Initializer.connectionLimitHandler").toJsonString());
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.frameEncoder, "org.apache.cassandra.transport.Server.Initializer.frameEncoder").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.frameDecompressor, "org.apache.cassandra.transport.Server.Initializer.frameDecompressor").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.frameCompressor, "org.apache.cassandra.transport.Server.Initializer.frameCompressor").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.messageDecoder, "org.apache.cassandra.transport.Server.Initializer.messageDecoder").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.messageEncoder, "org.apache.cassandra.transport.Server.Initializer.messageEncoder").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.server, "this.server").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.transport.Server.Initializer.class, org.apache.cassandra.transport.Server.Initializer.dispatcher, "org.apache.cassandra.transport.Server.Initializer.dispatcher").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    protected abstract static class AbstractSecureIntializer extends Initializer {
        ...
        protected final SslHandler createSslHandler() {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.encryptionOptions, "this.encryptionOptions").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    private static class EventNotifier extends MigrationListener implements IEndpointLifecycleSubscriber {
        ...
        private final Set<InetAddress> endpointsPendingJoinedNotification = Collections.newSetFromMap(new ConcurrentHashMap<InetAddress, Boolean>());
