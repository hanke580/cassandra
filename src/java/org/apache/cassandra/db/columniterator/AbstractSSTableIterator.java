...
abstract class AbstractSSTableIterator implements SliceableUnfilteredRowIterator {
    ...
    protected // file on every path where we created it.
    AbstractSSTableIterator(SSTableReader sstable, FileDataInput file, DecoratedKey key, RowIndexEntry indexEntry, ColumnFilter columnFilter, boolean isForThrift) {
    ...
    private static Row readStaticRow(SSTableReader sstable, FileDataInput file, SerializationHelper helper, Columns statics, boolean isForThrift, UnfilteredDeserializer deserializer) throws IOException {
        if (!sstable.descriptor.version.storeRows()) {
            if (!sstable.metadata.isCompactTable()) {
                assert deserializer != null;
                return deserializer.hasNext() && deserializer.nextIsStatic() ? (Row) deserializer.readNext() : Rows.EMPTY_STATIC_ROW;
            }
    ...
    public EncodingStats stats() {
        ...
        return ((EncodingStats) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new EncodingStats(sstable.getMinTimestamp(), sstable.getMinLocalDeletionTime(), sstable.getMinTTL()), 175));
    }
    ...
    protected static class IndexState {
        ...
        public void setToBlock(int blockIdx) throws IOException {
            ...
            if (!reader.version.storeRows() && blockIdx == 0 && reader.deserializer.hasNext() && reader.deserializer.nextIsStatic()) {
