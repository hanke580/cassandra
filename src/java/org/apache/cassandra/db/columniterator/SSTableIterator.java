...
public class SSTableIterator extends AbstractSSTableIterator {
    ...
    public SSTableIterator(SSTableReader sstable, DecoratedKey key, ColumnFilter columns, boolean isForThrift, SSTableReadsListener listener) {
    ...
    protected Reader createReader(RowIndexEntry indexEntry, FileDataInput file, boolean shouldCloseFile) {
        return indexEntry.isIndexed() ? ((ForwardIndexedReader) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ForwardIndexedReader(indexEntry, file, shouldCloseFile), 176)) : ((ForwardReader) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ForwardReader(file, shouldCloseFile), 177));
    }
    ...
    private class ForwardReader extends Reader {
        ...
        private Unfiltered handlePreSliceData() throws IOException {
            ...
            if (openMarker != null) {
                return ((RangeTombstoneBoundMarker) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstoneBoundMarker(sliceStart, openMarker), 178));
            }
            return null;
        }
        ...
        protected boolean hasNextInternal() throws IOException {
            ...
            if (openMarker != null) {
                next = ((RangeTombstoneBoundMarker) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstoneBoundMarker(end, openMarker), 179));
                return true;
            }
            return false;
        }
