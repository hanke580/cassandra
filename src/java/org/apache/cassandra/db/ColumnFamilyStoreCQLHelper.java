...
public class ColumnFamilyStoreCQLHelper {
    ...
    private static List<ColumnDefinition> getClusteringColumns(CFMetaData metadata) {
        ...
        if (!metadata.isStaticCompactTable())
            for (ColumnDefinition cd : metadata.clusteringColumns()) cds.add(cd);
        return cds;
    }
    ...
    private static List<ColumnDefinition> getPartitionColumns(CFMetaData metadata) {
        ...
        if (metadata.isDense()) {
            ...
            for (ColumnDefinition cd : metadata.partitionColumns().withoutStatics()) if (!cd.type.equals(EmptyType.instance))
                cds.add(cd);
        } else // "regular" columns are not exposed for static compact tables
        if (!metadata.isStaticCompactTable()) {
    ...
    public static String getCFMetadataAsCQL(CFMetaData metadata, boolean includeDroppedColumns) {
        ...
        if (clusteringColumns.size() > 0 || partitionKeyColumns.size() > 1) {
            sb.append(",\n\tPRIMARY KEY (");
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(partitionKeyColumns.size(), 1, ">", 19))) {
                sb.append("(");
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(clusteringColumns.size(), 0, ">", 20))) {
    ...
    public static List<String> getUserTypesAsCQL(CFMetaData metadata) {
        ...
        for (UserType type : types) typeStrings.add(toCQL(type));
        return typeStrings;
    }
    ...
    public static List<String> getIndexesAsCQL(CFMetaData metadata) {
        ...
        for (IndexMetadata indexMetadata : metadata.getIndexes()) indexes.add(toCQL(metadata.ksName, metadata.cfName, indexMetadata));
        return indexes;
    }
    ...
    private static String toCQL(String keyspace, String cf, IndexMetadata indexMetadata) {
        if (indexMetadata.isCustom()) {
            ...
            return String.format("CREATE CUSTOM INDEX %s ON %s.%s (%s) USING '%s'%s;", quoteIdentifier(indexMetadata.name), quoteIdentifier(keyspace), quoteIdentifier(cf), indexMetadata.options.get(IndexTarget.TARGET_OPTION_NAME), indexMetadata.options.get(IndexTarget.CUSTOM_INDEX_OPTION_NAME), options.isEmpty() ? "" : " WITH OPTIONS " + toCQL(options));
        } else {
            return String.format("CREATE INDEX %s ON %s.%s (%s);", quoteIdentifier(indexMetadata.name), quoteIdentifier(keyspace), quoteIdentifier(cf), indexMetadata.options.get(IndexTarget.TARGET_OPTION_NAME));
        }
    }
    ...
    private static String toCQL(ColumnDefinition cd, boolean isStaticCompactTable) {
        return String.format("%s %s%s", quoteIdentifier(cd.name.toString()), cd.type.asCQL3Type().toString(), cd.isStatic() && !isStaticCompactTable ? " static" : "");
    }
    ...
    private static String toCQLDrop(String keyspace, String cf, CFMetaData.DroppedColumn droppedColumn) {
        return String.format("ALTER TABLE %s.%s DROP %s USING TIMESTAMP %s;", quoteIdentifier(keyspace), quoteIdentifier(cf), quoteIdentifier(droppedColumn.name), droppedColumn.droppedTime);
    }

    private static void resolveUserType(UserType type, Set<UserType> typeSet, List<UserType> types) {
        for (AbstractType subType : type.fieldTypes()) if (!typeSet.contains(subType) && subType instanceof UserType)
    ...
    public static boolean isCqlCompatible(CFMetaData metaData) {
        if (metaData.isSuper())
            return false;
        if (metaData.isCompactTable() && metaData.partitionColumns().withoutStatics().size() > 1 && metaData.clusteringColumns().size() >= 1)
            return false;
        return true;
    }
}
