...
public class Columns extends AbstractCollection<ColumnMetadata> implements Collection<ColumnMetadata> {
    ...
    public static final ColumnMetadata FIRST_COMPLEX_STATIC = new ColumnMetadata("", "", ColumnIdentifier.getInterned(ByteBufferUtil.EMPTY_BYTE_BUFFER, UTF8Type.instance), SetType.getInstance(UTF8Type.instance, true), ColumnMetadata.NO_POSITION, ColumnMetadata.Kind.STATIC);

    public static final ColumnMetadata FIRST_COMPLEX_REGULAR = new ColumnMetadata("", "", ColumnIdentifier.getInterned(ByteBufferUtil.EMPTY_BYTE_BUFFER, UTF8Type.instance), SetType.getInstance(UTF8Type.instance, true), ColumnMetadata.NO_POSITION, ColumnMetadata.Kind.REGULAR);
    ...
    public Columns mergeTo(Columns other) {
        ...
        if (tree == other.columns)
            return other;
        return ((Columns) org.zlab.ocov.tracker.Runtime.update(new Columns(tree, findFirstComplexIdx(tree)), 25, other));
    }
    ...
    public static class Serializer {
        ...
        private static long encodeBitmap(Collection<ColumnMetadata> columns, Columns superset, int supersetCount) {
            ...
            for (ColumnMetadata column : columns) {
                ...
                org.zlab.ocov.tracker.Runtime.update(iter, 3, columns, superset, supersetCount);
        ...
        private void serializeLargeSubset(Collection<ColumnMetadata> columns, int columnCount, Columns superset, int supersetCount, DataOutputPlus out) throws IOException {
            ...
            if (columnCount < supersetCount / 2) {
                ...
                for (ColumnMetadata column : columns) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(iter, 2, columns, columnCount, superset, supersetCount, out);
