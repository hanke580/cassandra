...
public class MutationVerbHandler implements IVerbHandler<Mutation> {
    ...
    private static void forwardToLocalNodes(Mutation mutation, MessagingService.Verb verb, byte[] forwardBytes, InetAddress from) throws IOException {
        try (DataInputStream in = new DataInputStream(new FastByteArrayInputStream(forwardBytes))) {
            ...
            MessageOut<Mutation> message = new MessageOut<>(verb, mutation, Mutation.serializer).withParameter(Mutation.FORWARD_FROM, from.getAddress()).withParameter(MessagingService.FAILURE_CALLBACK_PARAM, MessagingService.ONE_BYTE);
            ...
            for (int i = 0; i < size; i++) {
                ...
                org.zlab.net.tracker.Runtime.record("sendOneWay", 11, message, id, address);
            }
        }
    }
}
