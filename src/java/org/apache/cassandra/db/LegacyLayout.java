...
public abstract class LegacyLayout {
    ...
    private static LegacyCellName decodeForSuperColumn(CFMetaData metadata, Clustering clustering, ByteBuffer subcol) {
        ...
        if (def != null) {
            ...
            return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(clustering, def, null), 107));
        }
        def = metadata.compactValueColumn();
        assert def != null && def.type instanceof MapType;
        return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(clustering, def, subcol), 108));
    }
    ...
    public static LegacyCellName decodeCellName(CFMetaData metadata, ByteBuffer cellname, boolean readAllAsDynamic) throws UnknownColumnException {
        ...
        if (metadata.isSuper()) {
            return ((org.apache.cassandra.db.LegacyLayout.LegacyCellName) org.zlab.ocov.tracker.Runtime.update(decodeForSuperColumn(metadata, clustering, CompositeType.extractComponent(cellname, 1)), 44, metadata, cellname, readAllAsDynamic));
        }
        if (metadata.isDense() || (metadata.isCompactTable() && readAllAsDynamic)) {
            return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(clustering, metadata.compactValueColumn(), null), 109));
        }
        ...
        if (column == null) {
            ...
            if (metadata.partitionColumns().isEmpty()) {
                return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(clustering, null, null), 110));
            }
        ...
        if (!column.hasRemaining()) {
            return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(clustering, null, null), 111));
        }
        ...
        if ((def == null) || def.isPrimaryKeyColumn()) {
            ...
            if (metadata.isCompactTable()) {
                return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(new Clustering(column), metadata.compactValueColumn(), null), 112));
            }
            if (def == null)
        ...
        return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(def.isStatic() ? Clustering.STATIC_CLUSTERING : clustering, def, collectionElement), 113));
    }
    ...
    public static LegacyBound decodeBound(CFMetaData metadata, ByteBuffer bound, boolean isStart) {
        ...
        if (!metadata.isCompound()) {
            ...
            // with that first.
            return ((LegacyBound) org.zlab.ocov.tracker.Runtime.update(new LegacyBound(isStart ? Slice.Bound.inclusiveStartOf(bound) : Slice.Bound.inclusiveEndOf(bound), false, null), 45, metadata, bound, isStart));
        }
        ...
        return ((LegacyBound) org.zlab.ocov.tracker.Runtime.update(new LegacyBound(sb, metadata.isCompound() && CompositeType.isStaticName(bound), collectionName), 46, metadata, bound, isStart));
    }
    ...
    public static ByteBuffer encodeBound(CFMetaData metadata, Slice.Bound bound, boolean isStart) {
        ...
        org.zlab.ocov.tracker.Runtime.update(builder, 47, metadata, bound, isStart);
        for (int i = 0; i < clustering.size(); i++) {
            builder.add(clustering.get(i));
            org.zlab.ocov.tracker.Runtime.update(builder, 48, metadata, bound, isStart);
        }
        if (isStart)
    ...
    public static Clustering decodeClustering(CFMetaData metadata, ByteBuffer value) {
        ...
        return ((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(components.subList(0, Math.min(csize, components.size())).toArray(new ByteBuffer[csize])), 114));
    }
    ...
    public static LegacyUnfilteredPartition fromUnfilteredRowIterator(ReadCommand command, UnfilteredRowIterator iterator) {
        ...
        org.zlab.ocov.tracker.Runtime.update(pair, 49, command, iterator);
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(cells.size(), maxCellsPerPartition, ">", 13)))
        ...
        if (info.hasRanges()) {
            ...
            while (rangeTombstoneIterator.hasNext()) {
                ...
                LegacyLayout.LegacyBound start = ((LegacyLayout.LegacyBound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyLayout.LegacyBound(slice.start(), false, null), 115));
                LegacyLayout.LegacyBound end = ((LegacyLayout.LegacyBound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyLayout.LegacyBound(slice.end(), false, null), 116));
                rtl.add(start, end, rt.deletionTime().markedForDeleteAt(), rt.deletionTime().localDeletionTime());
                org.zlab.ocov.tracker.Runtime.update(rtl, 50, command, iterator);
            }
        }
        return ((LegacyUnfilteredPartition) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyUnfilteredPartition(info.getPartitionDeletion(), rtl, cells), 117));
    }
    ...
    public static void serializeAsLegacyPartition(ReadCommand command, UnfilteredRowIterator partition, DataOutputPlus out, int version) throws IOException {
        ...
        LegacyLayout.LegacyUnfilteredPartition legacyPartition = ((org.apache.cassandra.db.LegacyLayout.LegacyUnfilteredPartition) org.zlab.ocov.tracker.Runtime.update(LegacyLayout.fromUnfilteredRowIterator(command, partition), 51, command, partition, out, version));
    ...
    public static UnfilteredRowIterator deserializeLegacyPartition(DataInputPlus in, int version, SerializationHelper.Flag flag, ByteBuffer key) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(info, 52, in, version, flag, key);
        int size = in.readInt();
    ...
    private static UnfilteredRowIterator toUnfilteredRowIterator(CFMetaData metadata, DecoratedKey key, LegacyDeletionInfo delInfo, Iterator<LegacyCell> cells, boolean reversed, SerializationHelper helper) {
        ...
        org.zlab.ocov.tracker.Runtime.update(atoms, 54, metadata, key, delInfo, cells, reversed, helper);
        org.zlab.ocov.tracker.Runtime.update(atoms, 53, metadata, key, delInfo, cells, reversed, helper);
        Iterator<Row> rows = convertToRows(((CellGrouper) org.zlab.ocov.tracker.Runtime.update(new CellGrouper(metadata, helper), 55, metadata, key, delInfo, cells, reversed, helper)), atoms);
        Iterator<RangeTombstone> ranges = delInfo.deletionInfo.rangeIterator(reversed);
        return ((RowAndDeletionMergeIterator) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowAndDeletionMergeIterator(metadata, key, delInfo.deletionInfo.getPartitionDeletion(), ColumnFilter.all(metadata), staticRow, reversed, EncodingStats.NO_STATS, rows, ranges, true), 118));
    }
        ...
        while ((atom = readLegacyAtom(metadata, in, false)) != null) {
            if (atom.isCell()) {
                ...
                if (!columnsToFetch.contains(cell.name.encode(metadata)))
                    continue;
                foundOne = true;
                builder.addCell(((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(cell.name.column, cell.timestamp, cell.ttl, cell.localDeletionTime, cell.value, null), 119)));
            } else {
    ...
    private static Pair<LegacyRangeTombstoneList, Iterator<LegacyCell>> fromRowIterator(final CFMetaData metadata, final Iterator<Row> iterator, final Row staticRow) {
        LegacyRangeTombstoneList deletions = ((LegacyRangeTombstoneList) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyRangeTombstoneList(new LegacyBoundComparator(metadata.comparator), 10), 120));
    ...
    private static Pair<LegacyRangeTombstoneList, Iterator<LegacyCell>> fromRow(final CFMetaData metadata, final Row row) {
        ...
        // to legacy nodes
        LegacyRangeTombstoneList deletions = ((LegacyRangeTombstoneList) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyRangeTombstoneList(new LegacyBoundComparator(metadata.comparator), 10), 121));
        if (!row.deletion().isLive()) {
            ...
            LegacyBound start = ((LegacyLayout.LegacyBound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyLayout.LegacyBound(startBound, false, null), 122));
            LegacyBound end = ((LegacyLayout.LegacyBound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyLayout.LegacyBound(endBound, false, null), 123));
        ...
        for (ColumnData cd : row) {
            ...
            if (!delTime.isLive()) {
                ...
                Slice.Bound startBound = Slice.Bound.inclusiveStartOf(clustering);
                Slice.Bound endBound = Slice.Bound.inclusiveEndOf(clustering);
                LegacyLayout.LegacyBound start = ((LegacyLayout.LegacyBound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyLayout.LegacyBound(startBound, col.isStatic(), col), 124));
                LegacyLayout.LegacyBound end = ((LegacyLayout.LegacyBound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyLayout.LegacyBound(endBound, col.isStatic(), col), 125));
    ...
    private static LegacyCell makeLegacyCell(Clustering clustering, Cell cell) {
        ...
        LegacyCellName name = ((LegacyCellName) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LegacyCellName(clustering, cell.column(), path == null ? null : path.get(0)), 126));
    ...
    public static LegacyRangeTombstone readLegacyRangeTombstoneBody(CFMetaData metadata, DataInputPlus in, ByteBuffer boundname) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(dt, 56, metadata, in, boundname);
    ...
    public static class CellGrouper {
        ...
        public static CellGrouper staticGrouper(CFMetaData metadata, SerializationHelper helper) {
            return ((CellGrouper) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new CellGrouper(metadata, helper, true), 127));
        }
        ...
        public boolean addCell(LegacyCell cell) {
            ...
            if (column == null) {
                ...
                if (!cell.isTombstone())
                    ...
                    builder.addRowDeletion(Row.Deletion.regular(((DeletionTime) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DeletionTime(cell.timestamp, cell.localDeletionTime), 128))));
                } else
    ...
    public static class LegacyDeletionInfo {
        ...
        private static Slice.Bound staticBound(CFMetaData metadata, boolean isStart) {
            ...
            return isStart ? Slice.Bound.inclusiveStartOf(values) : Slice.Bound.inclusiveEndOf(values);
        }
    ...
    public static class LegacyRangeTombstoneList {
        ...
        private void serializeCompound(DataOutputPlus out, boolean isDense) throws IOException {
            ...
            for (int i = 0; i < size; i++) {
                ...
                org.zlab.ocov.tracker.Runtime.update(startBuilder, 57, out, isDense);
                if (end.collectionName != null)
                    endBuilder.add(end.collectionName.name.bytes);
                org.zlab.ocov.tracker.Runtime.update(endBuilder, 58, out, isDense);
        ...
        private long serializedSizeCompound(boolean isDense) {
            ...
            for (int i = 0; i < this.size; i++) {
                ...
                if (start.collectionName != null)
                    startBuilder.add(start.collectionName.name.bytes);
                org.zlab.ocov.tracker.Runtime.update(startBuilder, 59, isDense);
                if (end.collectionName != null)
                    endBuilder.add(end.collectionName.name.bytes);
                org.zlab.ocov.tracker.Runtime.update(endBuilder, 60, isDense);
