...
public abstract class LegacyLayout {
    ...
    private static LegacyCellName decodeForSuperColumn(CFMetaData metadata, Clustering clustering, ByteBuffer subcol) {
        ...
        if (def != null) {
            ...
            return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.update(new LegacyCellName(clustering, def, null), 70, metadata, clustering, subcol));
        }
    ...
    public static LegacyCellName decodeCellName(CFMetaData metadata, ByteBuffer cellname, boolean readAllAsDynamic) throws UnknownColumnException {
        ...
        org.zlab.ocov.tracker.Runtime.update(clustering, 71, metadata, cellname, readAllAsDynamic);
        if (metadata.isSuper()) {
            return ((org.apache.cassandra.db.LegacyLayout.LegacyCellName) org.zlab.ocov.tracker.Runtime.update(decodeForSuperColumn(metadata, clustering, CompositeType.extractComponent(cellname, 1)), 72, metadata, cellname, readAllAsDynamic));
        }
        ...
        if (column == null) {
            ...
            if (metadata.partitionColumns().isEmpty()) {
                return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.update(new LegacyCellName(clustering, null, null), 73, metadata, cellname, readAllAsDynamic));
            }
        ...
        if (!column.hasRemaining()) {
            return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.update(new LegacyCellName(clustering, null, null), 74, metadata, cellname, readAllAsDynamic));
        }
        ...
        if (metadata.isCompactTable()) {
            if (def == null || def.isPrimaryKeyColumn()) {
                ...
                return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.update(new LegacyCellName(new Clustering(column), metadata.compactValueColumn(), null), 75, metadata, cellname, readAllAsDynamic));
            }
        } else if (def == null) {
        ...
        return ((LegacyCellName) org.zlab.ocov.tracker.Runtime.update(new LegacyCellName(def.isStatic() ? Clustering.STATIC_CLUSTERING : clustering, def, collectionElement), 76, metadata, cellname, readAllAsDynamic));
    }
    ...
    private static LegacyBound decodeBound(CFMetaData metadata, ByteBuffer bound, boolean isStart, boolean isDeletion) {
        ...
        if (!metadata.isCompound()) {
            ...
            return ((LegacyBound) org.zlab.ocov.tracker.Runtime.update(new LegacyBound(isStart ? Slice.Bound.inclusiveStartOf(bound) : Slice.Bound.inclusiveEndOf(bound), false, null), 77, metadata, bound, isStart, isDeletion));
        }
        ...
        if (components.size() > clusteringSize) {
            ...
            if (components.size() > clusteringSize + 1) {
                ...
                collectionName = ((org.apache.cassandra.config.ColumnDefinition) org.zlab.ocov.tracker.Runtime.update(decodeBoundLookupComplexColumn(metadata, components, clusteringSize, isStatic), 78, metadata, bound, isStart, isDeletion));
        ...
        return ((LegacyBound) org.zlab.ocov.tracker.Runtime.update(new LegacyBound(sb, isStatic, collectionName), 79, metadata, bound, isStart, isDeletion));
    }
    ...
    public static ByteBuffer encodeBound(CFMetaData metadata, Slice.Bound bound, boolean isStart) {
        ...
        org.zlab.ocov.tracker.Runtime.update(builder, 80, metadata, bound, isStart);
        for (int i = 0; i < clustering.size(); i++) {
            builder.add(clustering.get(i));
            org.zlab.ocov.tracker.Runtime.update(builder, 81, metadata, bound, isStart);
        }
        if (isStart)
    ...
    public static LegacyUnfilteredPartition fromUnfilteredRowIterator(ReadCommand command, UnfilteredRowIterator iterator) {
        ...
        org.zlab.ocov.tracker.Runtime.update(pair, 82, command, iterator);
        ...
        if (info.hasRanges()) {
            ...
            while (rangeTombstoneIterator.hasNext()) {
                ...
                org.zlab.ocov.tracker.Runtime.update(rtl, 83, command, iterator);
            }
        }
    ...
    public static void serializeAsLegacyPartition(ReadCommand command, UnfilteredRowIterator partition, DataOutputPlus out, int version) throws IOException {
        ...
        LegacyLayout.LegacyUnfilteredPartition legacyPartition = ((org.apache.cassandra.db.LegacyLayout.LegacyUnfilteredPartition) org.zlab.ocov.tracker.Runtime.update(LegacyLayout.fromUnfilteredRowIterator(command, partition), 84, command, partition, out, version));
    ...
    public static UnfilteredRowIterator deserializeLegacyPartition(DataInputPlus in, int version, SerializationHelper.Flag flag, ByteBuffer key) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(info, 85, in, version, flag, key);
        int size = in.readInt();
    ...
    private static UnfilteredRowIterator toUnfilteredRowIterator(CFMetaData metadata, DecoratedKey key, LegacyDeletionInfo delInfo, Iterator<LegacyCell> cells, boolean reversed, SerializationHelper helper) {
        ...
        org.zlab.ocov.tracker.Runtime.update(atoms, 87, metadata, key, delInfo, cells, reversed, helper);
        org.zlab.ocov.tracker.Runtime.update(atoms, 86, metadata, key, delInfo, cells, reversed, helper);
        Iterator<Row> rows = convertToRows(((CellGrouper) org.zlab.ocov.tracker.Runtime.update(new CellGrouper(metadata, helper), 88, metadata, key, delInfo, cells, reversed, helper)), atoms);
    ...
    private static Row getNextRow(CellGrouper grouper, PeekingIterator<? extends LegacyAtom> cells) {
        if (!cells.hasNext())
            return null;
        org.zlab.ocov.tracker.Runtime.update(cells, 89, grouper, cells);
        grouper.reset();
        while (cells.hasNext() && grouper.addAtom(cells.peek())) {
            // We've added the cell already in the grouper, so just skip it
            cells.next();
        }
        org.zlab.ocov.tracker.Runtime.update(cells, 91, grouper, cells);
        org.zlab.ocov.tracker.Runtime.update(cells, 90, grouper, cells);
    ...
    private static Pair<LegacyRangeTombstoneList, Iterator<LegacyCell>> fromRow(final CFMetaData metadata, final Row row) {
        ...
        for (ColumnData cd : row) {
            ...
            if (!delTime.isLive()) {
                ...
                Slice.Bound startBound = isStatic ? LegacyDeletionInfo.staticBound(metadata, true) : Slice.Bound.inclusiveStartOf(clustering);
                Slice.Bound endBound = isStatic ? LegacyDeletionInfo.staticBound(metadata, false) : Slice.Bound.inclusiveEndOf(clustering);
    ...
    public static LegacyCell readLegacyCellBody(CFMetaData metadata, DataInput in, ByteBuffer cellname, int mask, SerializationHelper.Flag flag, boolean readAllAsDynamic) throws IOException, UnknownColumnException {
        ...
        if ((mask & COUNTER_MASK) != 0) {
            ...
            return ((LegacyCell) org.zlab.ocov.tracker.Runtime.update(new LegacyCell(LegacyCell.Kind.COUNTER, decodeCellName(metadata, cellname, readAllAsDynamic), value, ts, Cell.NO_DELETION_TIME, Cell.NO_TTL), 92, metadata, in, cellname, mask, flag, readAllAsDynamic));
        } else if ((mask & EXPIRATION_MASK) != 0) {
            int ttl = in.readInt();
            int expiration = in.readInt();
            long ts = in.readLong();
            ByteBuffer value = ByteBufferUtil.readWithLength(in);
            return ((LegacyCell) org.zlab.ocov.tracker.Runtime.update(new LegacyCell(LegacyCell.Kind.EXPIRING, decodeCellName(metadata, cellname, readAllAsDynamic), value, ts, expiration, ttl), 93, metadata, in, cellname, mask, flag, readAllAsDynamic));
        } else {
            long ts = in.readLong();
            ByteBuffer value = ByteBufferUtil.readWithLength(in);
            LegacyCellName name = decodeCellName(metadata, cellname, readAllAsDynamic);
            org.zlab.ocov.tracker.Runtime.update(name, 94, metadata, in, cellname, mask, flag, readAllAsDynamic);
    ...
    public static LegacyRangeTombstone readLegacyRangeTombstoneBody(CFMetaData metadata, DataInputPlus in, ByteBuffer boundname) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(dt, 95, metadata, in, boundname);
    ...
    public static class LegacyDeletionInfo {
        ...
        public static LegacyDeletionInfo deserialize(CFMetaData metadata, DataInputPlus in) throws IOException {
            ...
            LegacyDeletionInfo delInfo = new LegacyDeletionInfo(((MutableDeletionInfo) org.zlab.ocov.tracker.Runtime.update(new MutableDeletionInfo(topLevel), 96, metadata, in)));
    ...
    public static class LegacyRangeTombstoneList {
        ...
        private void serializeCompound(DataOutputPlus out, boolean isDense) throws IOException {
            ...
            for (int i = 0; i < size; i++) {
                ...
                org.zlab.ocov.tracker.Runtime.update(startBuilder, 97, out, isDense);
                CompositeType.Builder endBuilder = type.builder(end.isStatic);
                org.zlab.ocov.tracker.Runtime.update(endBuilder, 98, out, isDense);
                for (int j = 0; j < start.bound.clustering().size(); j++) {
                    startBuilder.add(start.bound.get(j));
                    org.zlab.ocov.tracker.Runtime.update(startBuilder, 99, out, isDense);
                    endBuilder.add(end.bound.get(j));
                    org.zlab.ocov.tracker.Runtime.update(endBuilder, 100, out, isDense);
                }
                if (start.collectionName != null) {
                    startBuilder.add(start.collectionName.name.bytes);
                    org.zlab.ocov.tracker.Runtime.update(startBuilder, 101, out, isDense);
                }
                if (end.collectionName != null) {
                    endBuilder.add(end.collectionName.name.bytes);
                    org.zlab.ocov.tracker.Runtime.update(endBuilder, 102, out, isDense);
                }
        ...
        private long serializedSizeCompound(boolean isDense) {
            ...
            for (int i = 0; i < this.size; i++) {
                ...
                org.zlab.ocov.tracker.Runtime.update(startBuilder, 103, isDense);
                CompositeType.Builder endBuilder = type.builder();
                org.zlab.ocov.tracker.Runtime.update(endBuilder, 104, isDense);
                for (int j = 0; j < start.bound.size(); j++) {
                    startBuilder.add(start.bound.get(j));
                    org.zlab.ocov.tracker.Runtime.update(startBuilder, 105, isDense);
                }
                for (int j = 0; j < end.bound.size(); j++) {
                    endBuilder.add(end.bound.get(j));
                    org.zlab.ocov.tracker.Runtime.update(endBuilder, 106, isDense);
                }
                if (start.collectionName != null) {
                    startBuilder.add(start.collectionName.name.bytes);
                    org.zlab.ocov.tracker.Runtime.update(startBuilder, 107, isDense);
                }
                if (end.collectionName != null) {
                    endBuilder.add(end.collectionName.name.bytes);
                    org.zlab.ocov.tracker.Runtime.update(endBuilder, 108, isDense);
                }
