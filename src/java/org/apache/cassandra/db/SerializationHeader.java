...
public class SerializationHeader {
    ...
    public static SerializationHeader makeWithoutStats(CFMetaData metadata) {
        return ((SerializationHeader) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SerializationHeader(true, metadata, metadata.partitionColumns(), EncodingStats.NO_STATS), 246));
    }
    ...
    public static SerializationHeader forKeyCache(CFMetaData metadata) {
        ...
        for (int i = 0; i < size; i++) clusteringTypes.add(BytesType.instance);
        return ((SerializationHeader) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SerializationHeader(false, BytesType.instance, clusteringTypes, PartitionColumns.NONE, EncodingStats.NO_STATS, Collections.<ByteBuffer, AbstractType<?>>emptyMap()), 247));
    }
    ...
    public static SerializationHeader make(CFMetaData metadata, Collection<SSTableReader> sstables) {
        ...
        EncodingStats.Collector stats = ((EncodingStats.Collector) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new EncodingStats.Collector(), 248));
        ...
        return ((SerializationHeader) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SerializationHeader(true, metadata, columns.build(), stats.get()), 249));
    }
    ...
    public SerializationHeader(boolean isForSSTable, CFMetaData metadata, PartitionColumns columns, EncodingStats stats) {
        this(isForSSTable, metadata.getKeyValidator(), typesOf(metadata.clusteringColumns()), columns, stats, null);
    }
    ...
    public DeletionTime readDeletionTime(DataInputPlus in) throws IOException {
        ...
        return ((DeletionTime) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DeletionTime(markedAt, localDeletionTime), 250));
    }
    ...
    public static class Component extends MetadataComponent {
        ...
        public SerializationHeader toHeader(CFMetaData metadata) {
            ...
            return ((SerializationHeader) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SerializationHeader(true, keyType, clusteringTypes, builder.build(), stats, typeMap), 251));
        }
