...
public abstract class PurgeFunction extends Transformation<UnfilteredRowIterator> {
    ...
    public PurgeFunction(boolean isForThrift, int nowInSec, int gcBefore, int oldestUnrepairedTombstone, boolean onlyPurgeRepairedTombstones, boolean enforceStrictLiveness) {
    ...
    protected UnfilteredRowIterator applyToPartition(UnfilteredRowIterator partition) {
        ...
        org.zlab.ocov.tracker.Runtime.update(purged, 83, partition);
        return purged;
    }
    ...
    protected RangeTombstoneMarker applyToMarker(RangeTombstoneMarker marker) {
        ...
        if (marker.isBoundary()) {
            ...
            if (shouldPurgeClose) {
                if (shouldPurgeOpen)
                    return null;
                return ((org.apache.cassandra.db.rows.RangeTombstoneBoundMarker) org.zlab.ocov.tracker.Runtime.update(boundary.createCorrespondingOpenMarker(reversed), 84, marker));
            }
