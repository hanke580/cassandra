...
public abstract class AbstractBTreePartition implements Partition, Iterable<Row> {
    ...
    public UnfilteredRowIterator unfilteredIterator(Holder current, ColumnFilter selection, Slices slices, boolean reversed) {
        ...
        return slices.size() == 1 ? sliceIterator(selection, slices.get(0), reversed, current, staticRow) : ((SlicesIterator) org.zlab.ocov.tracker.Runtime.update(new SlicesIterator(selection, slices, reversed, current, staticRow), 118, current, selection, slices, reversed));
    }
    ...
    private UnfilteredRowIterator sliceIterator(ColumnFilter selection, Slice slice, boolean reversed, Holder current, Row staticRow) {
        ...
        Iterator<Row> rowIter = ((org.apache.cassandra.utils.btree.BTreeSearchIterator) org.zlab.ocov.tracker.Runtime.update(BTree.slice(current.tree, metadata.comparator, start, true, end, true, desc(reversed)), 119, selection, slice, reversed, current, staticRow));
    ...
    protected static Holder build(UnfilteredRowIterator iterator, int initialRowCapacity, boolean ordered, BTree.Builder.QuickResolver<Row> quickResolver) {
        ...
        MutableDeletionInfo.Builder deletionBuilder = ((org.apache.cassandra.db.MutableDeletionInfo.Builder) org.zlab.ocov.tracker.Runtime.update(MutableDeletionInfo.builder(iterator.partitionLevelDeletion(), metadata.comparator, reversed), 120, iterator, initialRowCapacity, ordered, quickResolver));
    ...
    protected static Holder build(RowIterator rows, DeletionInfo deletion, boolean buildEncodingStats, int initialRowCapacity) {
        ...
        EncodingStats stats = buildEncodingStats ? ((org.apache.cassandra.db.rows.EncodingStats) org.zlab.ocov.tracker.Runtime.update(EncodingStats.Collector.collect(staticRow, BTree.iterator(tree), deletion), 121, rows, deletion, buildEncodingStats, initialRowCapacity)) : EncodingStats.NO_STATS;
