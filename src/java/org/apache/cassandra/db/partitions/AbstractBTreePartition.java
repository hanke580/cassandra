...
public abstract class AbstractBTreePartition implements Partition, Iterable<Row> {
    ...
    public UnfilteredRowIterator unfilteredIterator(Holder current, ColumnFilter selection, Slices slices, boolean reversed) {
        ...
        return slices.size() == 1 ? sliceIterator(selection, slices.get(0), reversed, current, staticRow) : ((SlicesIterator) org.zlab.ocov.tracker.Runtime.update(new SlicesIterator(selection, slices, reversed, current, staticRow), 70, current, selection, slices, reversed));
    }
    ...
    private UnfilteredRowIterator sliceIterator(ColumnFilter selection, Slice slice, boolean reversed, Holder current, Row staticRow) {
        ...
        Iterator<Row> rowIter = ((org.apache.cassandra.utils.btree.BTreeSearchIterator) org.zlab.ocov.tracker.Runtime.update(BTree.slice(current.tree, metadata.comparator, start, true, end, true, desc(reversed)), 71, selection, slice, reversed, current, staticRow));
    ...
    private RowAndDeletionMergeIterator merge(Iterator<Row> rowIter, Iterator<RangeTombstone> deleteIter, ColumnFilter selection, boolean reversed, Holder current, Row staticRow) {
        return ((RowAndDeletionMergeIterator) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowAndDeletionMergeIterator(metadata, partitionKey, current.deletionInfo.getPartitionDeletion(), selection, staticRow, reversed, current.stats, rowIter, deleteIter, canHaveShadowedData()), 153, this));
    }
    ...
    private abstract class AbstractIterator extends AbstractUnfilteredRowIterator {
        ...
        private AbstractIterator(Holder current, ColumnFilter selection, boolean isReversed) {
            this(current, AbstractBTreePartition.this.staticRow(current, selection, false), selection, isReversed);
        }
    ...
    public SliceableUnfilteredRowIterator sliceableUnfilteredIterator(ColumnFilter columns, boolean reversed) {
        return ((SliceableIterator) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SliceableIterator(columns, reversed), 154));
    }
    ...
    protected static Holder build(UnfilteredRowIterator iterator, int initialRowCapacity, boolean ordered) {
        ...
        org.zlab.ocov.tracker.Runtime.update(iterator, 72, iterator, initialRowCapacity, ordered);
        ...
        MutableDeletionInfo.Builder deletionBuilder = ((org.apache.cassandra.db.MutableDeletionInfo.Builder) org.zlab.ocov.tracker.Runtime.update(MutableDeletionInfo.builder(iterator.partitionLevelDeletion(), metadata.comparator, reversed), 73, iterator, initialRowCapacity, ordered));
        ...
        if (reversed)
            builder.reverse();
        return ((Holder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Holder(columns, builder.build(), deletionBuilder.build(), iterator.staticRow(), iterator.stats()), 155));
    }
    ...
    protected static Holder build(RowIterator rows, DeletionInfo deletion, boolean buildEncodingStats, int initialRowCapacity) {
        ...
        EncodingStats stats = buildEncodingStats ? ((org.apache.cassandra.db.rows.EncodingStats) org.zlab.ocov.tracker.Runtime.update(EncodingStats.Collector.collect(staticRow, BTree.iterator(tree), deletion), 74, rows, deletion, buildEncodingStats, initialRowCapacity)) : EncodingStats.NO_STATS;
        return ((Holder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Holder(columns, tree, deletion, staticRow, stats), 156));
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("[%s.%s] key=%s partition_deletion=%s columns=%s", metadata.ksName, metadata.cfName, metadata.getKeyValidator().getString(partitionKey().getKey()), partitionLevelDeletion(), columns()));
