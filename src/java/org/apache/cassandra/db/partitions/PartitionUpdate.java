...
public class PartitionUpdate extends AbstractBTreePartition {
    ...
    public static PartitionUpdate emptyUpdate(CFMetaData metadata, DecoratedKey key) {
        ...
        org.zlab.ocov.tracker.Runtime.update(deletionInfo, 122, metadata, key);
        Holder holder = ((Holder) org.zlab.ocov.tracker.Runtime.update(new Holder(PartitionColumns.NONE, BTree.empty(), deletionInfo, Rows.EMPTY_STATIC_ROW, EncodingStats.NO_STATS), 123, metadata, key));
    ...
    public static PartitionUpdate fullPartitionDelete(CFMetaData metadata, DecoratedKey key, long timestamp, int nowInSec) {
        MutableDeletionInfo deletionInfo = ((MutableDeletionInfo) org.zlab.ocov.tracker.Runtime.update(new MutableDeletionInfo(timestamp, nowInSec), 124, metadata, key, timestamp, nowInSec));
        Holder holder = ((Holder) org.zlab.ocov.tracker.Runtime.update(new Holder(PartitionColumns.NONE, BTree.empty(), deletionInfo, Rows.EMPTY_STATIC_ROW, EncodingStats.NO_STATS), 125, metadata, key, timestamp, nowInSec));
        return new PartitionUpdate(metadata, key, holder, deletionInfo, false);
    }
    ...
    public static PartitionUpdate singleRowUpdate(CFMetaData metadata, DecoratedKey key, Row row, Row staticRow) {
        MutableDeletionInfo deletionInfo = MutableDeletionInfo.live();
        Holder holder = new Holder(new PartitionColumns(staticRow == null ? Columns.NONE : Columns.from(staticRow.columns()), row == null ? Columns.NONE : Columns.from(row.columns())), row == null ? BTree.empty() : BTree.singleton(row), deletionInfo, staticRow == null ? Rows.EMPTY_STATIC_ROW : staticRow, EncodingStats.NO_STATS);
        return new PartitionUpdate(metadata, key, holder, deletionInfo, false);
    }
    ...
    public static PartitionUpdate fromPre30Iterator(UnfilteredRowIterator iterator) {
        return fromIterator(iterator, false, (a, b) -> {
            ...
            rowMergingLogger.warn(String.format("Merging rows from pre 3.0 iterator for partition key: %s", cfm.getKeyValidator().getString(iterator.partitionKey().getKey())));
    ...
    private static PartitionUpdate fromIterator(UnfilteredRowIterator iterator, boolean ordered, BTree.Builder.QuickResolver<Row> quickResolver) {
        ...
        org.zlab.ocov.tracker.Runtime.update(holder, 126, iterator, ordered, quickResolver);
    ...
    public static class PartitionUpdateSerializer {
        ...
        private static PartitionUpdate deserialize30(DataInputPlus in, int version, SerializationHelper.Flag flag) throws IOException {
            ...
            org.zlab.ocov.tracker.Runtime.update(header, 127, in, version, flag);
            if (header.isEmpty) {
                return ((org.apache.cassandra.db.partitions.PartitionUpdate) org.zlab.ocov.tracker.Runtime.update(emptyUpdate(metadata, header.key), 128, in, version, flag));
            }
            assert !header.isReversed;
            assert header.rowEstimate >= 0;
            MutableDeletionInfo.Builder deletionBuilder = ((org.apache.cassandra.db.MutableDeletionInfo.Builder) org.zlab.ocov.tracker.Runtime.update(MutableDeletionInfo.builder(header.partitionDeletion, metadata.comparator, false), 129, in, version, flag));
            ...
            return new PartitionUpdate(metadata, header.key, new Holder(header.sHeader.columns(), rows.build(), deletionInfo, header.staticRow, header.sHeader.stats()), deletionInfo, false);
        }
