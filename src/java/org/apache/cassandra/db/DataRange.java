...
public class DataRange {
    ...
    public static DataRange forTokenRange(Range<Token> tokenRange) {
        return forKeyRange(((org.apache.cassandra.dht.Range) org.zlab.ocov.tracker.Runtime.update(Range.makeRowRange(tokenRange), 119, tokenRange)));
    }
    ...
    public static DataRange forKeyRange(Range<PartitionPosition> keyRange) {
        return ((DataRange) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DataRange(keyRange, new ClusteringIndexSliceFilter(Slices.ALL, false)), 236));
    }
    ...
    public DataRange forPaging(AbstractBounds<PartitionPosition> range, ClusteringComparator comparator, Clustering lastReturned, boolean inclusive) {
        return ((Paging) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Paging(range, clusteringIndexFilter, comparator, lastReturned, inclusive), 237));
    }
    ...
    public DataRange forSubRange(AbstractBounds<PartitionPosition> range) {
        return ((DataRange) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DataRange(range, clusteringIndexFilter), 238));
    }
    ...
    public static class Paging extends DataRange {
        ...
        public DataRange forSubRange(AbstractBounds<PartitionPosition> range) {
            ...
            return range.left.equals(keyRange().left) ? ((Paging) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Paging(range, clusteringIndexFilter, comparator, lastReturned, inclusive), 239)) : ((DataRange) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DataRange(range, clusteringIndexFilter), 240));
        }
        ...
        @Override
        public String toString(CFMetaData metadata) {
            return String.format("range=%s (paging) pfilter=%s lastReturned=%s (%s)", keyRange.getString(metadata.getKeyValidator()), clusteringIndexFilter.toString(metadata), lastReturned.toString(metadata), inclusive ? "included" : "excluded");
        }
    }
    ...
    public static class Serializer {
        ...
        public DataRange deserialize(DataInputPlus in, int version, CFMetaData metadata) throws IOException {
            ...
            if (in.readBoolean()) {
                ...
                return ((Paging) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Paging(range, filter, comparator, lastReturned, inclusive), 241));
            } else {
                return ((DataRange) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DataRange(range, filter), 242));
            }
        }
