...
public class Memtable implements Comparable<Memtable> {
    ...
    private Collection<SSTableReader> writeSortedContents(File sstableDirectory) {
        ...
        try (SSTableTxnWriter writer = createFlushWriter(cfs.getSSTablePath(sstableDirectory), columnsCollector.get(), statsCollector.get())) {
            ...
            for (AtomicBTreePartition partition : partitions.values()) {
                ...
                org.zlab.ocov.tracker.Runtime.update(partition, 110, sstableDirectory);
            }
    ...
    // log and writer closed by SSTableTxnWriter
    ...
    public SSTableTxnWriter createFlushWriter(String filename, PartitionColumns columns, EncodingStats stats) {
        ...
        try {
            ...
            return new SSTableTxnWriter(txn, cfs.createSSTableMultiWriter(Descriptor.fromFilename(filename), partitions.size(), ActiveRepairService.UNREPAIRED_SSTABLE, sstableMetadataCollector, new SerializationHeader(true, cfs.metadata, columns, stats), txn));
        } catch (Throwable t) {
            if (txn != null)
                txn.close();
            throw t;
