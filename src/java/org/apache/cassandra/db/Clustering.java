...
public class Clustering extends AbstractClusteringPrefix {
    ...
    public Clustering copy(AbstractAllocator allocator) {
        ...
        for (int i = 0; i < size(); i++) newValues[i] = values[i] == null ? null : allocator.clone(values[i]);
        return ((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(newValues), 171));
    }
    ...
    public static class Serializer {
        ...
        public Clustering deserialize(DataInputPlus in, int version, List<AbstractType<?>> types) throws IOException {
            ...
            return ((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(values), 172));
        }
