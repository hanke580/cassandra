...
public class Keyspace {
    ...
    public static ColumnFamilyStore openAndGetStore(CFMetaData cfm) {
        return ((org.apache.cassandra.db.Keyspace) org.zlab.ocov.tracker.Runtime.update(open(cfm.ksName), 160, cfm)).getColumnFamilyStore(cfm.cfId);
    }
     ...
     */
    @Deprecated
    public CompletableFuture<?> apply(final Mutation mutation, final boolean writeCommitLog, boolean updateIndexes, boolean isClReplay, boolean isDeferrable, CompletableFuture<?> future) {
        ...
        if (requiresViewUpdate) {
            ...
            while (true) {
                ...
                if (lock == null) {
                    ...
                    if (isDroppable) {
                        for (UUID cfid : mutation.getColumnFamilyIds()) columnFamilyStores.get(cfid).metric.viewLockAcquireTime.update(acquireTime, TimeUnit.MILLISECONDS);
                    }
                    break;
                }
            }
