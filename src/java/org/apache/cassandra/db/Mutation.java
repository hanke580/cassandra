...
public class Mutation implements IMutation {
    ...
    public Mutation copy() {
        Mutation copy = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(keyspaceName, key, new HashMap<>(modifications)), 38));
        return copy;
    }
    ...
    public Mutation without(UUID cfId) {
        Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(keyspaceName, key), 39));
        for (Map.Entry<UUID, ColumnFamily> entry : modifications.entrySet()) if (!entry.getKey().equals(cfId))
    ...
    public static class MutationSerializer implements IVersionedSerializer<Mutation> {
        ...
        public void serialize(Mutation mutation, DataOutputPlus out, int version) throws IOException {
            ...
            out.writeInt(size);
            assert size > 0;
            for (Map.Entry<UUID, ColumnFamily> entry : mutation.modifications.entrySet()) ColumnFamily.serializer.serialize(entry.getValue(), out, version);
        }

        public Mutation deserialize(DataInput in, int version, ColumnSerializer.Flag flag) throws IOException {
            // will always be set from cf.metadata but javac isn't smart enough to see that
            String keyspaceName = null;
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(version, MessagingService.VERSION_20, "<", 2)))
            ...
            return ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(keyspaceName, key, modifications), 40));
        }
        ...
        public long serializedSize(Mutation mutation, int version) {
            ...
            for (Map.Entry<UUID, ColumnFamily> entry : mutation.modifications.entrySet()) size += ColumnFamily.serializer.serializedSize(entry.getValue(), TypeSizes.NATIVE, version);
            return size;
        }
    }
}
