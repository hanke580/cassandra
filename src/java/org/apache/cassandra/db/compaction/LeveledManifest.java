    ...
    public synchronized void replace(Collection<SSTableReader> removed, Collection<SSTableReader> added) {
        ...
        for (SSTableReader sstable : removed) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(removed, sstable, "sstable").toJsonString());
        ...
        for (SSTableReader ssTableReader : added) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(added, ssTableReader, "ssTableReader").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            add(ssTableReader);
                ...
                if (compactionCounter[i] > NO_COMPACTION_LIMIT) {
                    ...
                    for (SSTableReader sstable : getLevel(i)) {
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(getLevel(i), sstable, "sstable").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
        ...
        if (logger.isTraceEnabled()) {
            for (int i = 0; i < generations.length; i++) {
                if (!getLevel(i).isEmpty()) {
                    logger.trace("L{} contains {} SSTables ({} bytes) in {}", i, getLevel(i).size(), SSTableReader.getTotalBytes(getLevel(i)), this);
                }
            }
        }
    }
        ...
        if (level == 0) {
            ...
            for (SSTableReader sstable : ageSortedSSTables(remaining)) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(ageSortedSSTables(remaining), sstable, "sstable").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
    ...
    public synchronized Set<SSTableReader>[] getSStablesPerLevelSnapshot() {
        ...
        for (int i = 0; i < generations.length; i++) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.generations, "this.generations").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(sstablesPerLevel, sstablesPerLevel[i], "sstablesPerLevel[i]").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        }
    ...
    public Iterable<SSTableReader> getAllSSTables() {
        ...
        for (List<SSTableReader> generation : generations) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(generations, generation, "generation").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
