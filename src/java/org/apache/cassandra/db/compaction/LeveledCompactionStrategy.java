        ...
        for (SSTableReader sstable : ssTablesToGroup) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(ssTablesToGroup, sstable, "sstable").toJsonString());
    ...
    public ScannerList getScanners(Collection<SSTableReader> sstables, Range<Token> range) {
        ...
        for (SSTableReader sstable : sstables) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(sstables, sstable, "sstable").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
