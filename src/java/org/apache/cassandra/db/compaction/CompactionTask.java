...
public class CompactionTask extends AbstractCompactionTask {
    ...
    protected void runMayThrow() throws Exception {
        ...
        try (CompactionController controller = getCompactionController(transaction.originals())) {
            org.zlab.ocov.tracker.Runtime.update(controller, 143);
            ...
            logger.debug(String.format("Compacted (task id: %s, compaction history id: %s) %d sstables to [%s] to level=%d.  %,d bytes to %,d (~%d%% of original) in %,dms = %fMB/s.  %,d total partitions merged to %,d.  Partition merge counts were {%s}", taskId, taskTimeUUID, transaction.originals().size(), newSSTableNames.toString(), getLevel(), startsize, endsize, (int) (ratio * 100), dTime, mbps, mergeSummary.totalSourceRows, totalKeysWritten, mergeSummary.partitionMerge));
