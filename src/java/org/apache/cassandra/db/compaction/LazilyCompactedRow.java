    ...
    public RowIndexEntry write(long currentPosition, SequentialWriter dataFile) throws IOException {
        ...
        try {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.merger, "this.merger").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    protected class Reducer extends MergeIterator.Reducer<OnDiskAtom, OnDiskAtom> {
        ...
        protected OnDiskAtom getReduced() {
            if (tombstone != null) {
                ...
                tombstone = null;
                if (t.data.isGcAble(controller.gcBefore) && t.timestamp() < getMaxPurgeableTimestamp() || maxRowTombstone.markedForDeleteAt >= t.timestamp()) {
