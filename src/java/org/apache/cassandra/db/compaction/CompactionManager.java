...
public class CompactionManager implements CompactionManagerMBean {
    ...
    public List<Future<?>> submitBackground(final ColumnFamilyStore cfs) {
        ...
        logger.trace("Scheduling a background task check for {}.{} with {}", cfs.keyspace.getName(), cfs.name, cfs.getCompactionStrategy().getName());
    ...
    public boolean isCompacting(Iterable<ColumnFamilyStore> cfses) {
        for (ColumnFamilyStore cfs : cfses) if (!cfs.getTracker().getCompacting().isEmpty())
            return true;
        return false;
    }
    ...
    public AllSSTableOpStatus performScrub(final ColumnFamilyStore cfs, final boolean skipCorrupted, final boolean checkData, final boolean reinsertOverflowedTTLRows, int jobs) throws InterruptedException, ExecutionException {
    ...
    public void performAnticompaction(ColumnFamilyStore cfs, Collection<Range<Token>> ranges, Refs<SSTableReader> validatedForRepair, LifecycleTransaction txn, long repairedAt) throws InterruptedException, IOException {
        ...
        try {
            ...
            while (sstableIterator.hasNext()) {
                ...
                // and CASSANDRA-14423.
                if (// We never anti-compact already repaired SSTables
                sstable.isRepaired())
                ...
                for (Range<Token> r : normalizedRanges) {
                    if (r.contains(sstableBounds.left) && r.contains(sstableBounds.right)) {
                        ...
                        if (// don't notify if the SSTable was already repaired
    ...
    private void scrubOne(ColumnFamilyStore cfs, LifecycleTransaction modifier, boolean skipCorrupted, boolean checkData, boolean reinsertOverflowedTTLRows) throws IOException {
        ...
        try (Scrubber scrubber = ((Scrubber) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Scrubber(cfs, modifier, skipCorrupted, checkData, reinsertOverflowedTTLRows), 59))) {
    ...
    private void doCleanupOne(final ColumnFamilyStore cfs, LifecycleTransaction txn, CleanupStrategy cleanupStrategy, Collection<Range<Token>> ranges, boolean hasIndexes) throws IOException {
        ...
        long expectedBloomFilterSize = Math.max(cfs.metadata.getMinIndexInterval(), SSTableReader.getApproximateKeyCount(txn.originals()));
        ...
        try (SSTableRewriter writer = ((SSTableRewriter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SSTableRewriter(cfs, txn, sstable.maxDataAge, false), 60));
    ...
    public static SSTableWriter createWriter(ColumnFamilyStore cfs, File compactionFileLocation, long expectedBloomFilterSize, long repairedAt, SSTableReader sstable) {
        FileUtils.createDirectory(compactionFileLocation);
        return SSTableWriter.create(cfs.metadata, Descriptor.fromFilename(cfs.getTempSSTablePath(compactionFileLocation)), expectedBloomFilterSize, repairedAt, sstable.getSSTableLevel(), cfs.partitioner);
    }
    ...
    private void doValidationCompaction(ColumnFamilyStore cfs, Validator validator) throws IOException {
        ...
        try {
            ...
            if (logger.isTraceEnabled()) {
                ...
                logger.trace("Validation finished in {} msec, depth {} for {} keys, serialized size {} bytes for {}", duration, depth, numPartitions, MerkleTree.serializer.serializedSize(tree, 0), validator.desc);
            }
        } finally {
            if (sstables != null)
                sstables.release();
    ...
    private int antiCompactGroup(ColumnFamilyStore cfs, Collection<Range<Token>> ranges, LifecycleTransaction anticompactionGroup, long repairedAt) {
        ...
        AbstractCompactionStrategy strategy = cfs.getCompactionStrategy();
        try (SSTableRewriter repairedSSTableWriter = ((SSTableRewriter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SSTableRewriter(cfs, anticompactionGroup, groupMaxDataAge, false, false), 61));
            SSTableRewriter unRepairedSSTableWriter = ((SSTableRewriter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SSTableRewriter(cfs, anticompactionGroup, groupMaxDataAge, false, false), 62));
    ...
    private static class CleanupInfo extends CompactionInfo.Holder {
        ...
        public CompactionInfo getCompactionInfo() {
            try {
                return new CompactionInfo(sstable.metadata, OperationType.CLEANUP, scanner.getCurrentPosition(), scanner.getLengthInBytes(), cleanupCompactionId);
            } catch (Exception e) {
