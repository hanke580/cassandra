...
public class Scrubber implements Closeable {
    ...
    public Scrubber(ColumnFamilyStore cfs, LifecycleTransaction transaction, boolean skipCorrupted, OutputHandler outputHandler, boolean checkData, boolean reinsertOverflowedTTLRows) throws IOException {
        ...
        this.controller = transaction.isOffline() ? new ScrubController(cfs) : new CompactionController(cfs, Collections.singleton(sstable), CompactionManager.getDefaultGcBefore(cfs));
        ...
        this.expectedBloomFilterSize = Math.max(cfs.metadata.getMinIndexInterval(), hasIndexFile ? SSTableReader.getApproximateKeyCount(toScrub) : 0);
    ...
    public void scrub() {
        ...
        try (SSTableRewriter writer = ((SSTableRewriter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SSTableRewriter(cfs, transaction, sstable.maxDataAge, transaction.isOffline()), 55));
            ...
            while (!dataFile.isEOF()) {
                ...
                if (prevKey != null && indexFile != null) {
                    ...
                    if (dataFile.getFilePointer() < nextRowStart) {
                        ...
                        saveOutOfOrderRow(prevKey, SSTableIdentityIterator.createFragmentIterator(sstable, dataFile, prevKey, nextRowStart - dataFile.getFilePointer(), checkData), String.format("Row fragment detected after END_OF_ROW at key %s", prevKey));
                        if (dataFile.isEOF())
                            break;
                    }
                }
            ...
            if (!outOfOrderRows.isEmpty()) {
                ...
                try (SSTableWriter inOrderWriter = CompactionManager.createWriter(cfs, destination, expectedBloomFilterSize, repairedAt, sstable)) {
                    for (Row row : outOfOrderRows) inOrderWriter.append(row.key, row.cf);
    ...
    private boolean tryAppend(DecoratedKey prevKey, DecoratedKey key, SSTableRewriter writer) {
        ...
        OrderCheckerIterator atoms = new OrderCheckerIterator(getIterator(key), cfs.metadata.comparator.onDiskAtomComparator());
        if (prevKey != null && prevKey.compareTo(key) > 0) {
            saveOutOfOrderRow(prevKey, key, atoms);
            return false;
        }
        AbstractCompactedRow compactedRow = ((LazilyCompactedRow) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LazilyCompactedRow(controller, Collections.singletonList(atoms)), 56));
    ...
    private OnDiskAtomIterator getIterator(DecoratedKey key) {
        ...
        return reinsertOverflowedTTLRows ? new FixNegativeLocalDeletionTimeIterator(sstableIdentityIterator, outputHandler, negativeLocalDeletionInfoMetrics) : sstableIdentityIterator;
    }
    ...
    private void updateIndexKey() {
        ...
        try {
            ...
            nextRowPositionFromIndex = !indexAvailable() ? dataFile.length() : rowIndexEntrySerializer.deserialize(indexFile, sstable.descriptor.version).position;
        } catch (Throwable th) {
    ...
    void saveOutOfOrderRow(DecoratedKey key, OnDiskAtomIterator atoms, String message) {
        ...
        outOfOrderRows.add(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(key, cf), 57)));
    }
    ...
    void saveOutOfOrderRow(DecoratedKey key, OrderCheckerIterator atoms) {
        ...
        outOfOrderRows.add(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(key, atoms.getOutOfOrderCells()), 58)));
    }
    ...
    private static final class OrderCheckerIterator extends AbstractIterator<OnDiskAtom> implements OnDiskAtomIterator {
        ...
        private static ColumnFamily collectOutOfOrderCells(OnDiskAtom atom, OnDiskAtomIterator iterator) {
            ...
            cf.addAtom(atom);
            while (iterator.hasNext()) cf.addAtom(iterator.next());
            return cf;
        }
    }
    ...
    private static final class FixNegativeLocalDeletionTimeIterator extends AbstractIterator<OnDiskAtom> implements OnDiskAtomIterator {
        ...
        public FixNegativeLocalDeletionTimeIterator(OnDiskAtomIterator iterator, OutputHandler outputHandler, NegativeLocalDeletionInfoMetrics negativeLocalDeletionInfoMetrics) {
            this.iterator = iterator;
            this.outputHandler = outputHandler;
