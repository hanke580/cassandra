...
public class Scrubber implements Closeable {
    ...
    private boolean tryAppend(DecoratedKey prevKey, DecoratedKey key, SSTableRewriter writer) {
        ...
        OrderCheckerIterator sstableIterator = new OrderCheckerIterator(new RowMergingSSTableIterator(sstable, dataFile, key), cfs.metadata.comparator);
    ...
    private void updateIndexKey() {
        ...
        try {
            ...
            nextRowPositionFromIndex = !indexAvailable() ? dataFile.length() : rowIndexEntrySerializer.deserialize(indexFile).position;
        } catch (Throwable th) {
        ...
        protected Unfiltered doCompute() {
            ...
            while (iterator.hasNext()) {
                ...
                if (next.clustering().equals(peek.clustering()) && peek.isRow()) {
                    // Make sure that the peeked item was consumed.
                    iterator.next();
    ...
    private static final class OrderCheckerIterator extends AbstractIterator<Unfiltered> implements UnfilteredRowIterator {
        ...
        public EncodingStats stats() {
            return ((org.apache.cassandra.db.rows.EncodingStats) org.zlab.ocov.tracker.Runtime.update(iterator.stats(), 96));
        }
