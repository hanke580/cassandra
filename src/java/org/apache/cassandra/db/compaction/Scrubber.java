...
public class Scrubber implements Closeable {
        ...
        try (SSTableRewriter writer = new SSTableRewriter(cfs, transaction, sstable.maxDataAge, transaction.isOffline())) {
            ...
            if (!outOfOrderRows.isEmpty()) {
                ...
                try (SSTableWriter inOrderWriter = CompactionManager.createWriter(cfs, destination, expectedBloomFilterSize, repairedAt, sstable)) {
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            ...
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.outOfOrderRows, "this.outOfOrderRows").toJsonString());
                    ...
                    for (Row row : outOfOrderRows) {
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(outOfOrderRows, row, "row").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.newInOrderSstable, "this.newInOrderSstable").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
    ...
    public CompactionInfo.Holder getScrubInfo() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.scrubInfo, "this.scrubInfo").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        return scrubInfo;
    ...
    private static final class OrderCheckerIterator extends AbstractIterator<OnDiskAtom> implements OnDiskAtomIterator {
        ...
        public ColumnFamily getOutOfOrderCells() {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.outOfOrderCells, "this.outOfOrderCells").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
