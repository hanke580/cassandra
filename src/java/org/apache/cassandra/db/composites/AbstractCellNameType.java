...
public abstract class AbstractCellNameType extends AbstractCType implements CellNameType {
    ...
    private static class SparseRowIterator extends AbstractIterator<CQL3Row> implements CQL3Row.RowIterator {
        ...
        public SparseRowIterator(CFMetaData cfMetaData, CellNameType type, Iterator<Cell> cells, long now) {
            ...
            this.cells = cells;
            this.now = now;
            this.staticRow = hasNextCell() && nextCell.name().isStatic() ? computeNext() : null;
        }
        ...
        public CQL3Row getStaticRow() {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.staticRow, "this.staticRow").toJsonString());
        ...
        protected CQL3Row computeNext() {
            while (hasNextCell()) {
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.nextCell, "this.nextCell").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
