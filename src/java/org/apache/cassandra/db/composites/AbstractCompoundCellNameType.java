...
public abstract class AbstractCompoundCellNameType extends AbstractCellNameType {
    ...
    public AbstractType<?> asAbstractType() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.fullType, "this.fullType").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    private static class CompositeDeserializer implements CellNameType.Deserializer {
        ...
        public int compareNextTo(Composite composite) throws IOException {
            ...
            for (int i = 0; i < composite.size(); i++) {
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.nextComponents, "this.nextComponents").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
        ...
        private boolean hasComponent(int i) {
            while (i >= nextSize && deserializeOne()) continue;
            return i < nextSize;
        }
