...
public class CompoundSparseCellNameType extends AbstractCompoundCellNameType {
    ...
    private static Composite makeStaticPrefix(int size) {
        ...
        for (int i = 0; i < size; i++) elements[i] = ByteBufferUtil.EMPTY_BYTE_BUFFER;
    ...
    protected static CompoundCType makeCType(CompoundCType clusteringType, AbstractType<?> columnNameType, ColumnToCollectionType collectionType) {
        ...
        for (int i = 0; i < clusteringType.size(); i++) allSubtypes.add(clusteringType.subtype(i));
    ...
    public Composite staticPrefix() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.staticPrefix, "this.staticPrefix").toJsonString());
    ...
    public CellName rowMarker(Composite prefix) {
        // static columns don't really create rows, they shouldn't have a row marker
        assert !prefix.isStatic();
        if (prefix.isEmpty()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.db.composites.CompoundSparseCellNameType.class, org.apache.cassandra.db.composites.CompoundSparseCellNameType.rowMarkerNoPrefix, "org.apache.cassandra.db.composites.CompoundSparseCellNameType.rowMarkerNoPrefix").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
