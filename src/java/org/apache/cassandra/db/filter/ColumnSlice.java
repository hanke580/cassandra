    ...
    public boolean isAlwaysEmpty(CellNameType comparator, boolean reversed) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.finish, "this.finish").toJsonString());
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.start, "this.start").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        ...
        for (int i = 1; i < slices.length; i++) {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(s2, s2.start, "s2.start").toJsonString());
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(s2, s2.finish, "s2.finish").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
