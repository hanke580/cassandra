                ...
                if (!clause.isEmpty()) {
                    ...
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            ...
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.clause, "this.clause").toJsonString());
                    ...
                    for (IndexExpression expr : clause) {
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(clause, expr, "expr").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
        ...
        public boolean isSatisfiedBy(DecoratedKey rowKey, ColumnFamily data, Composite prefix, ByteBuffer collectionElement) {
            ...
            for (IndexExpression expression : clause) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(clause, expression, "expression").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
