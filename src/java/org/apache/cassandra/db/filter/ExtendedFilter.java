...
public abstract class ExtendedFilter {
    ...
    public String toString() {
        return Objects.toStringHelper(this).add("dataRange", dataRange).add("maxResults", maxResults).add("currentLimit", currentLimit).add("timestamp", timestamp).add("countCQL3Rows", countCQL3Rows).toString();
    }
    ...
    public static class WithClauses extends ExtendedFilter {
        ...
        private IDiskAtomFilter computeOptimizedFilter() {
            ...
            if (cfs.getComparator().isCompound() || dataRange instanceof DataRange.Paging)
                return null;
            // ok since not a paging range
            IDiskAtomFilter filter = dataRange.columnFilter(null);
            if (filter instanceof SliceQueryFilter) {
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(cfs.metric.maxRowSize.getValue(), DatabaseDescriptor.getColumnIndexSize(), "<", 19))) {
                    logger.trace("Expanding slice filter to entire row to cover additional expressions");
                    return ((SliceQueryFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SliceQueryFilter(ColumnSlice.ALL_COLUMNS_ARRAY, ((SliceQueryFilter) filter).reversed, Integer.MAX_VALUE), 88));
                }
            } else {
                ...
                if (!clause.isEmpty()) {
                    ...
                    for (IndexExpression expr : clause) columns.add(cfs.getComparator().cellFromByteBuffer(expr.column));
        ...
        public IDiskAtomFilter getExtraFilter(DecoratedKey rowKey, ColumnFamily data) {
             ...
             */
            assert !(cfs.getComparator().isCompound()) : "Sequential scan with filters is not supported (if you just created an index, you " + "need to wait for the creation to be propagated to all nodes before querying it)";
        ...
        private static boolean collectionSatisfies(ColumnDefinition def, ColumnFamily data, Composite prefix, IndexExpression expr) {
            ...
            while (iter.hasNext()) {
                Cell next = iter.next();
                if (type.nameComparator().compare(next.name().collectionElement(), key) == 0 && type.valueComparator().compare(next.value(), value) == 0)
                    return true;
            }
            return false;
        }
        ...
        @Override
        public String toString() {
            return Objects.toStringHelper(this).add("dataRange", dataRange).add("timestamp", timestamp).add("clause", clause).toString();
        }
    }
