...
public class SliceQueryFilter implements IDiskAtomFilter {
    ...
    public boolean hasStaticSlice(CFMetaData cfm) {
        for (ColumnSlice slice : slices) if (sliceIncludesStatics(slice, cfm))
            return true;
        return false;
    }
    ...
    public Pair<SliceQueryFilter, SliceQueryFilter> splitOutStaticSlice(CFMetaData cfm) {
        ...
        return Pair.create(new SliceQueryFilter(staticSliceEnd, Composites.EMPTY, true, count, compositesToGroup), new SliceQueryFilter(nonStaticSlices.toArray(new ColumnSlice[nonStaticSlices.size()]), true, count, compositesToGroup));
    }
    ...
    public Iterator<Cell> getColumnIterator(ColumnFamily cf) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.slices, "this.slices").toJsonString());
    ...
    public void collectReducedColumns(ColumnFamily container, Iterator<Cell> reducedColumns, DecoratedKey key, int gcBefore, long now) {
        ...
        while (reducedColumns.hasNext()) {
            ...
            if (respectTombstoneThresholds() && columnCounter.tombstones() > DatabaseDescriptor.getTombstoneFailureThreshold()) {
                ...
                throw new TombstoneOverwhelmingException(columnCounter.tombstones(), count, container.metadata().ksName, container.metadata().cfName, container.getComparator().getString(cell.name()), getSlicesInfo(container), container.metadata().getKeyValidator().getString(key.getKey()));
            }
    ...
    public boolean maySelectPrefix(CType type, Composite prefix) {
        for (ColumnSlice slice : slices) if (slice.includes(type, prefix))
            return true;
        return false;
    }
    ...
    public boolean shouldInclude(SSTableReader sstable) {
        ...
        if (minColumnNames.isEmpty() || maxColumnNames.isEmpty())
            return true;
        for (ColumnSlice slice : slices) if (slice.intersects(minColumnNames, maxColumnNames, comparator, reversed))
            return true;
        return false;
    }
    ...
    public boolean isFullyCoveredBy(ColumnFamily cf, long now) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.db.filter.ColumnSlice.class, org.apache.cassandra.db.filter.ColumnSlice.ALL_COLUMNS_ARRAY, "org.apache.cassandra.db.filter.ColumnSlice.ALL_COLUMNS_ARRAY").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public static class Serializer implements IVersionedSerializer<SliceQueryFilter> {
        ...
        public void serialize(SliceQueryFilter f, DataOutputPlus out, int version) throws IOException {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(f, f.slices, "f.slices").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            for (ColumnSlice slice : f.slices) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(f.slices, slice, "slice").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(f, f.reversed, "f.reversed").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(f, f.count, "f.count").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            int count = f.count;
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(f, f.compositesToGroup, "f.compositesToGroup").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        ...
        public SliceQueryFilter deserialize(DataInput in, int version) throws IOException {
            ...
            for (int i = 0; i < slices.length; i++) slices[i] = type.sliceSerializer().deserialize(in, version);
