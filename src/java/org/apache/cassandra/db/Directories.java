...
public class Directories {
        ...
        if (dataDirectory != null) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.dataPaths, "this.dataPaths").toJsonString());
            ...
            for (File dir : dataPaths) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(dataPaths, dir, "dir").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
        ...
        private void filter() {
            ...
            for (File location : dataPaths) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(dataPaths, location, "location").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
    ...
    private class TrueFilesSizeVisitor extends SimpleFileVisitor<Path> {
        ...
        private final Set<String> visited = newHashSet();
        ...
        TrueFilesSizeVisitor() {
            ...
            for (File file : sstableLister().listFiles()) builder.add(file.getName());
            alive = builder.build();
        }
        ...
        private boolean isAcceptable(Path file) {
            ...
            return pair != null && pair.left.ksname.equals(metadata.ksName) && pair.left.cfname.equals(metadata.cfName) && !visited.contains(fileName) && !alive.contains(fileName);
        }
