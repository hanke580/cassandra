    ...
    public Cell purge(DeletionPurger purger, int nowInSec) {
        if (!isLive(nowInSec)) {
            ...
            if (isExpiring()) {
                ...
                return ((org.apache.cassandra.db.rows.BufferCell) org.zlab.ocov.tracker.Runtime.update(BufferCell.tombstone(column, timestamp, localDeletionTime - ttl, path), 53, purger, nowInSec)).purge(purger, nowInSec);
            }
        }
        return this;
    }
