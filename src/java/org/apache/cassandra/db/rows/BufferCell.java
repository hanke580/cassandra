...
public class BufferCell extends AbstractCell {
    ...
    public static BufferCell live(CFMetaData metadata, ColumnDefinition column, long timestamp, ByteBuffer value, CellPath path) {
        ...
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, NO_TTL, NO_DELETION_TIME, value, path), 65));
    }
    ...
    public static BufferCell expiring(ColumnDefinition column, long timestamp, int ttl, int nowInSec, ByteBuffer value, CellPath path) {
        assert ttl != NO_TTL;
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, ttl, nowInSec + ttl, value, path), 66));
    }
    ...
    public static BufferCell tombstone(ColumnDefinition column, long timestamp, int nowInSec, CellPath path) {
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, NO_TTL, nowInSec, ByteBufferUtil.EMPTY_BYTE_BUFFER, path), 67));
    }
    ...
    public Cell withUpdatedColumn(ColumnDefinition newColumn) {
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(newColumn, timestamp, ttl, localDeletionTime, value, path), 68));
    }

    public Cell withUpdatedValue(ByteBuffer newValue) {
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, ttl, localDeletionTime, newValue, path), 69));
    }

    public Cell copy(AbstractAllocator allocator) {
        if (!value.hasRemaining())
            return this;
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, ttl, localDeletionTime, allocator.clone(value), path == null ? null : path.copy(allocator)), 70));
    }
    ...
    public Cell markCounterLocalToBeCleared() {
        ...
        return marked == value() ? this : ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, ttl, localDeletionTime, marked, path), 71));
    }
    ...
    public Cell purge(DeletionPurger purger, int nowInSec) {
        if (!isLive(nowInSec)) {
            ...
            if (isExpiring()) {
                ...
                return ((org.apache.cassandra.db.rows.BufferCell) org.zlab.ocov.tracker.Runtime.update(BufferCell.tombstone(column, timestamp, localDeletionTime - ttl, path), 27, purger, nowInSec)).purge(purger, nowInSec);
            }
        }
        return this;
    }
    ...
    public Cell updateAllTimestamp(long newTimestamp) {
        return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, isTombstone() ? newTimestamp - 1 : newTimestamp, ttl, localDeletionTime, value, path), 72));
    }
    ...
    static class Serializer implements Cell.Serializer {
        ...
        public Cell deserialize(DataInputPlus in, LivenessInfo rowLiveness, ColumnDefinition column, SerializationHeader header, SerializationHelper helper) throws IOException {
            ...
            return ((BufferCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCell(column, timestamp, ttl, localDeletionTime, value, path), 73));
        }
