...
public class ComplexColumnData extends ColumnData implements Iterable<Cell> {
    ...
    private ComplexColumnData transformAndFilter(DeletionTime newDeletion, Function<? super Cell, ? extends Cell> function) {
        ...
        if (newDeletion == DeletionTime.LIVE && BTree.isEmpty(transformed))
            return null;
        return ((ComplexColumnData) org.zlab.ocov.tracker.Runtime.update(new ComplexColumnData(column, transformed, newDeletion), 54, newDeletion, function));
    }

    public ComplexColumnData updateAllTimestamp(long newTimestamp) {
        DeletionTime newDeletion = complexDeletion.isLive() ? complexDeletion : ((DeletionTime) org.zlab.ocov.tracker.Runtime.update(new DeletionTime(newTimestamp - 1, complexDeletion.localDeletionTime()), 55, newTimestamp));
