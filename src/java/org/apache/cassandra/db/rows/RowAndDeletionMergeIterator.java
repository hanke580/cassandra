...
public class RowAndDeletionMergeIterator extends AbstractUnfilteredRowIterator {
    ...
    public RowAndDeletionMergeIterator(CFMetaData metadata, DecoratedKey partitionKey, DeletionTime partitionLevelDeletion, ColumnFilter selection, Row staticRow, boolean isReversed, EncodingStats stats, Iterator<Row> rows, Iterator<RangeTombstone> ranges, boolean removeShadowedData) {
    ...
    private RangeTombstoneMarker closeOpenedRange() {
        ...
        if (nextRange != null && comparator.compare(closeBound(openRange), openBound(nextRange)) == 0) {
            ...
            marker = ((RangeTombstoneBoundMarker) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstoneBoundMarker(closeBound(toClose), toClose.deletionTime()), 88));
        }
        return marker;
    }
    ...
    private RangeTombstoneMarker openRange() {
        ...
        openRange = consumeNextRange();
        return ((RangeTombstoneBoundMarker) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstoneBoundMarker(openBound(openRange), openRange.deletionTime()), 89));
    }
}
