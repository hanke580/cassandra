...
public class UnfilteredRowIteratorSerializer {
    ...
    public Header deserializeHeader(CFMetaData metadata, ColumnFilter selection, DataInputPlus in, int version, SerializationHelper.Flag flag) throws IOException {
        ...
        if ((flags & IS_EMPTY) != 0) {
            SerializationHeader sh = ((SerializationHeader) org.zlab.ocov.tracker.Runtime.update(new SerializationHeader(false, metadata, PartitionColumns.NONE, EncodingStats.NO_STATS), 63, metadata, selection, in, version, flag));
            return ((Header) org.zlab.ocov.tracker.Runtime.update(new Header(sh, key, isReversed, true, null, null, 0), 64, metadata, selection, in, version, flag));
        }
        ...
        SerializationHeader header = ((org.apache.cassandra.db.SerializationHeader) org.zlab.ocov.tracker.Runtime.update(SerializationHeader.serializer.deserializeForMessaging(in, metadata, selection, hasStatic), 65, metadata, selection, in, version, flag));
        DeletionTime partitionDeletion = hasPartitionDeletion ? ((org.apache.cassandra.db.DeletionTime) org.zlab.ocov.tracker.Runtime.update(header.readDeletionTime(in), 66, metadata, selection, in, version, flag)) : DeletionTime.LIVE;
