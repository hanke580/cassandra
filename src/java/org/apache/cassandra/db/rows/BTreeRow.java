...
public class BTreeRow extends AbstractRow {
    ...
    public Row filter(ColumnFilter filter, DeletionTime activeDeletion, boolean setActiveDeletionToRow, CFMetaData metadata) {
        ...
        if (mayHaveShadowed) {
            ...
            newDeletion = setActiveDeletionToRow ? ((org.apache.cassandra.db.rows.Row.Deletion) org.zlab.ocov.tracker.Runtime.update(Deletion.regular(activeDeletion), 47, filter, activeDeletion, setActiveDeletionToRow, metadata)) : Deletion.LIVE;
        }
    ...
    public Row updateAllTimestamp(long newTimestamp) {
        LivenessInfo newInfo = primaryKeyLivenessInfo.isEmpty() ? primaryKeyLivenessInfo : ((org.apache.cassandra.db.LivenessInfo) org.zlab.ocov.tracker.Runtime.update(primaryKeyLivenessInfo.withUpdatedTimestamp(newTimestamp), 48, newTimestamp));
    ...
    private Row transformAndFilter(LivenessInfo info, Deletion deletion, Function<ColumnData, ColumnData> function) {
        ...
        return ((BTreeRow) org.zlab.ocov.tracker.Runtime.update(new BTreeRow(clustering, info, deletion, transformed, minDeletionTime), 49, info, deletion, function));
    }
    ...
    public static class Builder implements Row.Builder {
        ...
        public Row build() {
            ...
            Row row = ((BTreeRow) org.zlab.ocov.tracker.Runtime.update(new BTreeRow(clustering, primaryKeyLivenessInfo, deletion, btree, minDeletionTime), 50));
            reset();
            return row;
        }
    }
