...
public class CollationController {
    ...
    public ColumnFamily getTopLevelColumns(boolean copyOnHeap) {
        return filter.filter instanceof NamesQueryFilter && cfs.metadata.getDefaultValidator() != CounterColumnType.instance ? collectTimeOrderedData(copyOnHeap) : collectAllData(copyOnHeap);
    }
    ...
    private ColumnFamily collectTimeOrderedData(boolean copyOnHeap) {
        ...
        try {
            ...
            QueryFilter reducedFilter = ((QueryFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new QueryFilter(filter.key, filter.cfName, namesFilter.withUpdatedColumns(filterColumns), filter.timestamp), 77));
            ...
            if (sstablesIterated > cfs.getMinimumCompactionThreshold() && onlyUnrepaired && !cfs.isAutoCompactionDisabled() && cfs.getCompactionStrategy().shouldDefragment()) {
                // !!WARNING!!   if we stop copying our data to a heap-managed object,
                //               we will need to track the lifetime of this mutation as well
                Tracing.trace("Defragmenting requested data");
                final Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(cfs.keyspace.getName(), filter.key.getKey(), returnCF.cloneMe()), 78));
    ...
    private ColumnFamily collectAllData(boolean copyOnHeap) {
        ...
        try {
            ...
            for (Object iter : iterators) if (iter instanceof Closeable)
