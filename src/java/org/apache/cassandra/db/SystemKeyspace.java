...
public final class SystemKeyspace {
    ...
    private static final CFMetaData BuiltViews = compile(BUILT_VIEWS, "built views", "CREATE TABLE %s (" + "keyspace_name text," + "view_name text," + "PRIMARY KEY ((keyspace_name), view_name))");
    ...
    public static void updateSizeEstimates(String keyspace, String table, Map<Range<Token>, Pair<Long, Long>> estimates) {
        ...
        PartitionUpdate update = ((PartitionUpdate) org.zlab.ocov.tracker.Runtime.update(new PartitionUpdate(SizeEstimates, UTF8Type.instance.decompose(keyspace), SizeEstimates.partitionColumns(), estimates.size()), 61, keyspace, table, estimates));
        Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(update), 129));
        // delete all previous values with a single range tombstone.
        int nowInSec = FBUtilities.nowInSeconds();
        update.add(((RangeTombstone) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone(Slice.make(SizeEstimates.comparator, table), new DeletionTime(timestamp - 1, nowInSec)), 130)));
        // add a CQL row for each primary token range.
        for (Map.Entry<Range<Token>, Pair<Long, Long>> entry : estimates.entrySet()) {
            Range<Token> range = entry.getKey();
            Pair<Long, Long> values = entry.getValue();
            ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(SizeEstimates, timestamp, mutation).clustering(table, range.left.toString(), range.right.toString()).add("partitions_count", values.left).add("mean_partition_size", values.right), 131)).build();
        }
        mutation.apply();
    }
