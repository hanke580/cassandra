...
public final class SystemKeyspace {
    ...
    public static ReplayPosition getTruncatedPosition(UUID cfId) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(record, record.left, "record.left").toJsonString());
    ...
    public static void updateSizeEstimates(String keyspace, String table, Map<Range<Token>, Pair<Long, Long>> estimates) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.db.SystemKeyspace.class, org.apache.cassandra.db.SystemKeyspace.SizeEstimates, "org.apache.cassandra.db.SystemKeyspace.SizeEstimates").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    public static synchronized void updateAvailableRanges(String keyspace, Collection<Range<Token>> completedRanges) {
        ...
        for (Range<Token> range : completedRanges) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(completedRanges, range, "range").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
