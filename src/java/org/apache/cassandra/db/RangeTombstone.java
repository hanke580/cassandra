...
public class RangeTombstone {
    ...
    public static class Bound extends Slice.Bound {
        ...
        public static RangeTombstone.Bound inclusiveOpen(boolean reversed, ByteBuffer[] boundValues) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(reversed ? Kind.INCL_END_BOUND : Kind.INCL_START_BOUND, boundValues), 207));
        }

        public static RangeTombstone.Bound exclusiveOpen(boolean reversed, ByteBuffer[] boundValues) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(reversed ? Kind.EXCL_END_BOUND : Kind.EXCL_START_BOUND, boundValues), 208));
        }

        public static RangeTombstone.Bound inclusiveClose(boolean reversed, ByteBuffer[] boundValues) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(reversed ? Kind.INCL_START_BOUND : Kind.INCL_END_BOUND, boundValues), 209));
        }

        public static RangeTombstone.Bound exclusiveClose(boolean reversed, ByteBuffer[] boundValues) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(reversed ? Kind.EXCL_START_BOUND : Kind.EXCL_END_BOUND, boundValues), 210));
        }

        public static RangeTombstone.Bound inclusiveCloseExclusiveOpen(boolean reversed, ByteBuffer[] boundValues) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(reversed ? Kind.EXCL_END_INCL_START_BOUNDARY : Kind.INCL_END_EXCL_START_BOUNDARY, boundValues), 211));
        }

        public static RangeTombstone.Bound exclusiveCloseInclusiveOpen(boolean reversed, ByteBuffer[] boundValues) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(reversed ? Kind.INCL_END_EXCL_START_BOUNDARY : Kind.EXCL_END_INCL_START_BOUNDARY, boundValues), 212));
        }

        public static RangeTombstone.Bound fromSliceBound(Slice.Bound sliceBound) {
            return ((RangeTombstone.Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone.Bound(sliceBound.kind(), sliceBound.getRawValues()), 213));
        }
        ...
        public RangeTombstone.Bound copy(AbstractAllocator allocator) {
            ...
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(kind(), newValues), 214));
        }

        @Override
        public Bound withNewKind(Kind kind) {
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(kind, values), 215));
        }
        ...
        public static class Serializer {
            ...
            public RangeTombstone.Bound deserializeValues(DataInputPlus in, Kind kind, int version, List<AbstractType<?>> types) throws IOException {
                ...
                return ((RangeTombstone.Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone.Bound(kind, values), 216));
            }
        }
    }
}
