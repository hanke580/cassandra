...
public class CommitLogReplayer {
    ...
    public static CommitLogReplayer construct(CommitLog commitLog) {
        ...
        for (ColumnFamilyStore cfs : ColumnFamilyStore.all()) {
            ...
            if (!filter.isEmpty())
                ...
                // if we have no ranges for this CF, we must replay everything and filter
                globalPosition = ReplayPosition.NONE;
        }
        if (globalPosition == null)
            globalPosition = ReplayPosition.firstNotCovered(cfPersisted.values());
        logger.debug("Global replay position is {} from columnfamilies {}", globalPosition, FBUtilities.toString(cfPersisted));
        return ((CommitLogReplayer) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new CommitLogReplayer(commitLog, globalPosition, cfPersisted, replayFilter), 69));
    }

    public void recover(File[] clogs) throws IOException {
        int i;
        for (i = 0; i < clogs.length; ++i) recover(clogs[i], i + 1 == clogs.length);
    }
    ...
    public int blockForWrites() {
        ...
        futures.clear();
        for (Keyspace keyspace : keyspacesRecovered) futures.addAll(keyspace.flush());
        FBUtilities.waitOnFutures(futures);
    ...
    private int readSyncMarker(CommitLogDescriptor descriptor, int offset, RandomAccessReader reader, boolean tolerateTruncation) throws IOException {
        ...
        if (crc.getValue() != filecrc) {
            if (end != 0 || filecrc != 0) {
                handleReplayError(false, "Encountered bad header at position %d of commit log %s, with invalid CRC. " + "The end of segment marker should be zero.", offset, reader.getPath());
            }
            return -1;
        } else if (end < offset || end > reader.length()) {
            handleReplayError(tolerateTruncation, "Encountered bad header at position %d of commit log %s, with bad position but valid CRC", offset, reader.getPath());
            return -1;
        }
        return end;
    }
            ...
            while ((end = readSyncMarker(desc, end, reader, tolerateTruncation)) >= 0) {
                ...
                if (compressor != null) {
                    ...
                    try {
                        ...
                        if ((org.zlab.ocov.tracker.Runtime.updateBranch(compressedLength, buffer.length, ">", 14)))
                        ...
                        handleReplayError(tolerateErrorsInSection, "Unexpected exception decompressing section at %d: %s", start, e);
                        continue;
                    }
                }
    ...
    private boolean replaySyncSection(FileDataInput reader, int end, CommitLogDescriptor desc, String errorContext, boolean tolerateErrors) throws IOException {
        ...
        while (reader.getFilePointer() < end && !reader.isEOF()) {
            ...
            try {
                ...
                // ok.
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(serializedSize, buffer.length, ">", 15))) {
                    buffer = new byte[(int) (1.2 * serializedSize)];
                }
                reader.readFully(buffer, 0, serializedSize);
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(desc.version, CommitLogDescriptor.VERSION_21, "<", 16)))
    ...
    void replayMutation(byte[] inputBuffer, int size, final int entryLocation, final CommitLogDescriptor desc) throws IOException {
        ...
        try (FastByteArrayInputStream bufIn = new FastByteArrayInputStream(inputBuffer, 0, size)) {
            ...
            for (ColumnFamily cf : mutation.getColumnFamilies()) for (Cell cell : cf) cf.getComparator().validate(cell.name());
