...
public class AtomicBTreeColumns extends ColumnFamily {
    ...
    static final long EMPTY_SIZE = ObjectSizes.measure(new AtomicBTreeColumns(CFMetaData.denseCFMetaData("keyspace", "table", BytesType.instance), null)) + ObjectSizes.measure(new Holder(null, null));
    ...
    public DeletionInfo deletionInfo() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.ref, "this.ref").toJsonString());
    ...
    public Iterator<Cell> iterator(ColumnSlice[] slices) {
        return slices.length == 1 ? slice(ref.tree, asymmetricComparator(), slices[0].start, slices[0].finish, true) : new SliceIterator(ref.tree, asymmetricComparator(), true, slices);
    }

    public Iterator<Cell> reverseIterator(ColumnSlice[] slices) {
        return slices.length == 1 ? slice(ref.tree, asymmetricComparator(), slices[0].finish, slices[0].start, false) : new SliceIterator(ref.tree, asymmetricComparator(), false, slices);
    }
    ...
    static final class ColumnUpdater implements UpdateFunction<Cell> {
        ...
        protected void reset() {
            ...
            this.heapSize = 0;
            if (inserted != null) {
                for (Cell cell : inserted) abort(cell);
                inserted.clear();
            }
            reclaimer.cancel();
