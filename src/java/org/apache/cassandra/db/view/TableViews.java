...
public class TableViews extends AbstractCollection<View> {
    ...
    public void pushViewReplicaUpdates(PartitionUpdate update, boolean writeCommitLog, AtomicLong baseComplete) {
        ...
        org.zlab.ocov.tracker.Runtime.update(command, 108, update, writeCommitLog, baseComplete);
        if (command == null)
            return;
        ...
        try (ReadOrderGroup orderGroup = command.startOrderGroup();
            ...
            org.zlab.ocov.tracker.Runtime.update(updates, 109, update, writeCommitLog, baseComplete);
        }
    ...
    public Iterator<Collection<Mutation>> generateViewUpdates(Collection<View> views, UnfilteredRowIterator updates, UnfilteredRowIterator existings, int nowInSec, boolean separateUpdates) {
        ...
        DeletionTracker existingsDeletion = ((DeletionTracker) org.zlab.ocov.tracker.Runtime.update(new DeletionTracker(existings.partitionLevelDeletion()), 110, views, updates, existings, nowInSec, separateUpdates));
        DeletionTracker updatesDeletion = ((DeletionTracker) org.zlab.ocov.tracker.Runtime.update(new DeletionTracker(updates.partitionLevelDeletion()), 111, views, updates, existings, nowInSec, separateUpdates));
        /*
        ...
        while (existingsIter.hasNext() && updatesIter.hasNext()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(updatesIter, 113, views, updates, existings, nowInSec, separateUpdates);
            Row existingRow;
            Row updateRow;
            ...
            if (cmp < 0) {
                ...
                org.zlab.ocov.tracker.Runtime.update(updatesIter, 114, views, updates, existings, nowInSec, separateUpdates);
                ...
                existingRow = ((Row) existingsIter.next()).withRowDeletion(existingsDeletion.currentDeletion());
                updateRow = ((Row) updatesIter.next()).withRowDeletion(updatesDeletion.currentDeletion());
                org.zlab.ocov.tracker.Runtime.update(updatesIter, 115, views, updates, existings, nowInSec, separateUpdates);
            }
            addToViewUpdateGenerators(existingRow, updateRow, generators, nowInSec);
        }
        org.zlab.ocov.tracker.Runtime.update(updatesIter, 112, views, updates, existings, nowInSec, separateUpdates);
    ...
    private SinglePartitionReadCommand readExistingRowsCommand(PartitionUpdate updates, Collection<View> views, int nowInSec) {
        ...
        if (names != null && names.isEmpty())
            return null;
        ClusteringIndexFilter clusteringFilter = names == null ? ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ClusteringIndexSliceFilter(sliceBuilder.build(), false), 231)) : ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(names, false), 116, updates, views, nowInSec));
    ...
    private static Row emptyRow(Clustering clustering, DeletionTime deletion) {
        ...
        return deletion.isLive() ? null : BTreeRow.emptyDeletedRow(clustering, ((org.apache.cassandra.db.rows.Row.Deletion) org.zlab.ocov.tracker.Runtime.update(Row.Deletion.regular(deletion), 117, clustering, deletion)));
    }
    ...
    private Collection<Mutation> buildMutations(CFMetaData baseTableMetadata, List<ViewUpdateGenerator> generators) {
        ...
        if (generators.size() == 1) {
            ...
            for (PartitionUpdate update : updates) {
                mutations.add(((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(update), 232)));
            }
            generator.clear();
            return mutations;
        }
        ...
        for (ViewUpdateGenerator generator : generators) {
            for (PartitionUpdate update : generator.generateViewUpdates()) {
                ...
                if (mutation == null) {
                    mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(baseTableMetadata.ksName, key), 233));
