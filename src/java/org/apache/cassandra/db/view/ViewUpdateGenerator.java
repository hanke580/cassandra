...
public class ViewUpdateGenerator {
    ...
    private void startNewUpdate(Row baseRow) {
        ...
        currentViewEntryBuilder.newRow(((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(clusteringValues), 234)));
    }
    ...
    private LivenessInfo computeLivenessInfoForEntry(Row baseRow) {
        ...
        if (view.hasSamePrimaryKeyColumnsAsBaseTable()) {
            ...
            if (biggestExpirationCell.localDeletionTime() > baseLiveness.localExpirationTime() || !baseLiveness.isLive(nowInSec))
                return LivenessInfo.create(timestamp, biggestExpirationCell.ttl(), biggestExpirationCell.localDeletionTime());
            return baseLiveness;
        }
    ...
    private void submitUpdate() {
        ...
        if (update == null) {
            ...
            update = ((PartitionUpdate) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new PartitionUpdate(viewMetadata, partitionKey, viewMetadata.partitionColumns(), 4), 235));
