...
public class ViewBuilder extends CompactionInfo.Holder {
    ...
    private void buildKey(DecoratedKey key) {
        ...
        org.zlab.ocov.tracker.Runtime.update(command, 118, key);
        ...
        try (ReadOrderGroup orderGroup = command.startOrderGroup();
            ...
            Iterator<Collection<Mutation>> mutations = baseCfs.keyspace.viewManager.forTable(baseCfs.metadata).generateViewUpdates(Collections.singleton(view), data, empty, nowInSec, true);
    ...
    public void run() {
        ...
        try (Refs<SSTableReader> sstables = baseCfs.selectAndReference(function).refs;
            ...
            ScheduledExecutors.nonPeriodicTasks.schedule(() -> CompactionManager.instance.submitViewBuilder(builder), 5, TimeUnit.MINUTES);
