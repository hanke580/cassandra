...
public class EmptyIterators {
    ...
    public static UnfilteredRowIterator unfilteredRow(CFMetaData metadata, DecoratedKey partitionKey, boolean isReverseOrder, Row staticRow, DeletionTime partitionDeletion) {
        ...
        if (!staticRow.isEmpty()) {
            columns = ((PartitionColumns) org.zlab.ocov.tracker.Runtime.update(new PartitionColumns(Columns.from(staticRow.columns()), Columns.NONE), 131, metadata, partitionKey, isReverseOrder, staticRow, partitionDeletion));
        } else
