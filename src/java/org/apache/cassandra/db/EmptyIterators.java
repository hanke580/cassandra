...
public class EmptyIterators {
    ...
    public static UnfilteredRowIterator unfilteredRow(CFMetaData metadata, DecoratedKey partitionKey, boolean isReverseOrder, Row staticRow, DeletionTime partitionDeletion) {
        ...
        if (!staticRow.isEmpty()) {
            columns = ((PartitionColumns) org.zlab.ocov.tracker.Runtime.update(new PartitionColumns(Columns.from(staticRow.columns()), Columns.NONE), 86, metadata, partitionKey, isReverseOrder, staticRow, partitionDeletion));
        } else
            staticRow = Rows.EMPTY_STATIC_ROW;
        if (partitionDeletion.isLive())
            partitionDeletion = DeletionTime.LIVE;
        return ((EmptyUnfilteredRowIterator) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new EmptyUnfilteredRowIterator(columns, metadata, partitionKey, isReverseOrder, staticRow, partitionDeletion), 173));
    }

    public static UnfilteredRowIterator unfilteredRow(CFMetaData metadata, DecoratedKey partitionKey, boolean isReverseOrder) {
        return ((EmptyUnfilteredRowIterator) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new EmptyUnfilteredRowIterator(PartitionColumns.NONE, metadata, partitionKey, isReverseOrder, Rows.EMPTY_STATIC_ROW, DeletionTime.LIVE), 174));
    }
