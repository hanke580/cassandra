...
public class CounterMutation implements IMutation {
    ...
    public Mutation apply() throws WriteTimeoutException {
        Mutation result = ((Mutation) org.zlab.ocov.tracker.Runtime.update(new Mutation(getKeyspaceName(), key()), 8));
    ...
    private ColumnFamily processModifications(ColumnFamily changesCF) {
        ...
        for (int i = 0; i < counterUpdateCells.size(); i++) {
            ...
            resultCF.addColumn(((BufferCounterCell) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new BufferCounterCell(update.name(), CounterContext.instance().createGlobal(CounterId.getLocalId(), clock, count), update.timestamp()), 41)));
        }
        return resultCF;
    }
