...
public class CounterMutation implements IMutation {
    ...
    public Mutation apply() throws WriteTimeoutException {
        Mutation result = ((Mutation) org.zlab.ocov.tracker.Runtime.update(new Mutation(getKeyspaceName(), key()), 34));
    ...
    private PartitionUpdate processModifications(PartitionUpdate changes) {
        ColumnFamilyStore cfs = ((org.apache.cassandra.db.Keyspace) org.zlab.ocov.tracker.Runtime.update(Keyspace.open(getKeyspaceName()), 35, changes)).getColumnFamilyStore(changes.metadata().cfId);
    ...
    private void updateWithCurrentValuesFromCFS(List<PartitionUpdate.CounterMark> marks, ColumnFamilyStore cfs) {
        ...
        org.zlab.ocov.tracker.Runtime.update(builder, 36, marks, cfs);
        ...
        for (PartitionUpdate.CounterMark mark : marks) {
            ...
            if (mark.path() == null) {
                ...
                org.zlab.ocov.tracker.Runtime.update(builder, 37, marks, cfs);
            } else {
                builder.select(mark.column(), mark.path());
                org.zlab.ocov.tracker.Runtime.update(builder, 38, marks, cfs);
            }
        }
        int nowInSec = FBUtilities.nowInSeconds();
        ClusteringIndexNamesFilter filter = ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(names.build(), false), 39, marks, cfs));
    ...
    private void updateForRow(PeekingIterator<PartitionUpdate.CounterMark> markIter, Row row, ColumnFamilyStore cfs) {
        ...
        org.zlab.ocov.tracker.Runtime.update(markIter, 41, markIter, row, cfs);
        org.zlab.ocov.tracker.Runtime.update(markIter, 40, markIter, row, cfs);
        if (!markIter.hasNext())
            return;
        org.zlab.ocov.tracker.Runtime.update(markIter, 42, markIter, row, cfs);
        while (cmp == 0) {
            ...
            if (!markIter.hasNext())
                return;
            org.zlab.ocov.tracker.Runtime.update(markIter, 43, markIter, row, cfs);
            cmp = compare(markIter.peek().clustering(), row.clustering(), cfs);
            org.zlab.ocov.tracker.Runtime.update(markIter, 44, markIter, row, cfs);
        }
    }
    ...
    public static class CounterMutationSerializer implements IVersionedSerializer<CounterMutation> {
        ...
        public long serializedSize(CounterMutation cm, int version) {
            return Mutation.serializer.serializedSize(cm.mutation, version) + TypeSizes.sizeof(cm.consistency.name());
        }
    }
}
