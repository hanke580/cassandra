...
public class CounterMutation implements IMutation {
    ...
    public Mutation apply() throws WriteTimeoutException {
        Mutation result = ((Mutation) org.zlab.ocov.tracker.Runtime.update(new Mutation(getKeyspaceName(), key()), 17));
    ...
    private PartitionUpdate processModifications(PartitionUpdate changes) {
        ColumnFamilyStore cfs = ((org.apache.cassandra.db.Keyspace) org.zlab.ocov.tracker.Runtime.update(Keyspace.open(getKeyspaceName()), 18, changes)).getColumnFamilyStore(changes.metadata().cfId);
    ...
    private void updateWithCurrentValuesFromCFS(List<PartitionUpdate.CounterMark> marks, ColumnFamilyStore cfs) {
        ...
        org.zlab.ocov.tracker.Runtime.update(builder, 19, marks, cfs);
        ...
        for (PartitionUpdate.CounterMark mark : marks) {
            ...
            if (mark.path() == null) {
                ...
                org.zlab.ocov.tracker.Runtime.update(builder, 20, marks, cfs);
            } else {
                builder.select(mark.column(), mark.path());
                org.zlab.ocov.tracker.Runtime.update(builder, 21, marks, cfs);
            }
        }
        int nowInSec = FBUtilities.nowInSeconds();
        ClusteringIndexNamesFilter filter = ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(names.build(), false), 22, marks, cfs));
        SinglePartitionReadCommand cmd = SinglePartitionReadCommand.create(cfs.metadata, nowInSec, key(), builder.build(), filter);
        org.zlab.ocov.tracker.Runtime.update(cmd, 23, marks, cfs);
