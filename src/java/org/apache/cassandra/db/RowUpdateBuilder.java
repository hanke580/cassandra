...
public class RowUpdateBuilder {
    ...
    public RowUpdateBuilder clustering(Object... clusteringValues) {
        assert clusteringValues.length == update.metadata().comparator.size() : "Invalid clustering values length. Expected: " + update.metadata().comparator.size() + " got: " + clusteringValues.length;
    ...
    private static void deleteRow(PartitionUpdate update, long timestamp, int localDeletionTime, Object... clusteringValues) {
        ...
        builder.addRowDeletion(Row.Deletion.regular(((DeletionTime) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DeletionTime(timestamp, localDeletionTime), 187))));
        update.add(builder.build());
    }
    ...
    private static PartitionUpdate getOrAdd(CFMetaData metadata, Mutation mutation) {
        ...
        if (upd == null) {
            upd = ((PartitionUpdate) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new PartitionUpdate(metadata, mutation.key(), metadata.partitionColumns(), 1), 188));
            mutation.add(upd);
        }
        return upd;
    }
    ...
    private Cell makeCell(ColumnDefinition c, ByteBuffer value, CellPath path) {
        return value == null ? BufferCell.tombstone(c, timestamp, localDeletionTime) : (ttl == LivenessInfo.NO_TTL ? BufferCell.live(update.metadata(), c, timestamp, value, path) : BufferCell.expiring(c, timestamp, ttl, localDeletionTime, value, path));
    }
    ...
    public RowUpdateBuilder map(String columnName, Map<?, ?> map) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) addMapEntry(columnName, entry.getKey(), entry.getValue());
        return this;
    }

    public RowUpdateBuilder set(String columnName, Set<?> set) {
        resetCollection(columnName);
        for (Object element : set) addSetEntry(columnName, element);
        return this;
    }
