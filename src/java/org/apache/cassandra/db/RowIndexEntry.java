...
public class RowIndexEntry<T> implements IMeasurableMemory {
    ...
    public static RowIndexEntry<IndexHelper.IndexInfo> create(long position, DeletionTime deletionTime, ColumnIndex index) {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(index.columnsIndex.size(), 1, ">", 12))) {
            return ((IndexedEntry) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexedEntry(position, deletionTime, index.columnsIndex), 67));
        } else {
    ...
    public static class Serializer implements IndexSerializer<IndexHelper.IndexInfo> {
        ...
        public void serialize(RowIndexEntry<IndexHelper.IndexInfo> rie, DataOutputPlus out) throws IOException {
            ...
            if (rie.isIndexed()) {
                ...
                for (IndexHelper.IndexInfo info : rie.columnsIndex()) idxSerializer.serialize(info, out);
            }
        }
        ...
        public RowIndexEntry<IndexHelper.IndexInfo> deserialize(DataInput in, Version version) throws IOException {
            ...
            int size = in.readInt();
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(size, 0, ">", 13))) {
                ...
                return ((IndexedEntry) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexedEntry(position, deletionTime, columnsIndex), 68));
            } else {
                return new RowIndexEntry<>(position);
            }
        }
