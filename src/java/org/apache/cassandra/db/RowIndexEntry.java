...
public class RowIndexEntry<T> implements IMeasurableMemory {
    ...
    public static RowIndexEntry<IndexHelper.IndexInfo> create(long position, DeletionTime deletionTime, ColumnIndex index) {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(index.columnsIndex.size(), 1, ">", 24))) {
            return ((IndexedEntry) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexedEntry(position, deletionTime, index.partitionHeaderLength, index.columnsIndex), 184));
        } else {
    ...
    public static class Serializer implements IndexSerializer<IndexHelper.IndexInfo> {
        ...
        public RowIndexEntry<IndexHelper.IndexInfo> deserialize(DataInputPlus in) throws IOException {
            if (!version.storeRows()) {
                ...
                int size = in.readInt();
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(size, 0, ">", 25))) {
                    ...
                    return ((IndexedEntry) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexedEntry(position, deletionTime, headerLength, columnsIndex), 185));
                } else {
                    return new RowIndexEntry<>(position);
                }
            }
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(size, 0, ">", 26))) {
                ...
                return ((IndexedEntry) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new IndexedEntry(position, deletionTime, headerLength, columnsIndex), 186));
            } else {
                return new RowIndexEntry<>(position);
            }
        }
    ...
    private static class IndexedEntry extends RowIndexEntry<IndexHelper.IndexInfo> {
        ...
        @Override
        protected int promotedSize(IndexHelper.IndexInfo.Serializer idxSerializer) {
            long size = TypeSizes.sizeofUnsignedVInt(headerLength) + DeletionTime.serializer.serializedSize(deletionTime) + // number of entries
