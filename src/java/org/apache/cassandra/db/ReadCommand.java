...
public abstract class ReadCommand implements ReadQuery {
    ...
    public static final IVersionedSerializer<ReadCommand> readSerializer = new ForwardingVersionedSerializer<ReadCommand>() {
        ...
        protected IVersionedSerializer<ReadCommand> delegate(int version) {
            return version < MessagingService.VERSION_30 ? legacyReadCommandSerializer : serializer;
        }
    };
    ...
    public static final IVersionedSerializer<ReadCommand> rangeSliceSerializer = new ForwardingVersionedSerializer<ReadCommand>() {
        ...
        protected IVersionedSerializer<ReadCommand> delegate(int version) {
            return version < MessagingService.VERSION_30 ? legacyRangeSliceCommandSerializer : serializer;
        }
    };
    ...
    public static final IVersionedSerializer<ReadCommand> pagedRangeSerializer = new ForwardingVersionedSerializer<ReadCommand>() {
        ...
        protected IVersionedSerializer<ReadCommand> delegate(int version) {
            return version < MessagingService.VERSION_30 ? legacyPagedRangeCommandSerializer : serializer;
        }
    };
    ...
    protected ReadCommand(Kind kind, boolean isDigestQuery, int digestVersion, boolean isForThrift, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, RowFilter rowFilter, DataLimits limits, IndexMetadata index) {
        this.kind = kind;
    ...
    UnfilteredPartitionIterator executeLocally(ReadOrderGroup orderGroup) {
        ...
        UnfilteredPartitionIterator resultIterator = searcher == null ? queryStorage(cfs, orderGroup) : searcher.search(orderGroup);
        try {
    ...
    private static class LegacyRangeSliceCommandSerializer implements IVersionedSerializer<ReadCommand> {
        ...
        public void serialize(ReadCommand command, DataOutputPlus out, int version) throws IOException {
            ...
            if (rangeCommand.isNamesQuery()) {
                ...
                if (limits.kind() == DataLimits.Kind.THRIFT_LIMIT)
                    ...
                    // for DISTINCT queries (CASSANDRA-8490)
                    compositesToGroup = -2;
                else
            ...
            // countCQL3Rows
            if (// if for Thrift or DISTINCT
            rangeCommand.isForThrift() || rangeCommand.limits().perPartitionCount() == 1)
        ...
        public ReadCommand deserialize(DataInputPlus in, int version) throws IOException {
            ...
            return PartitionRangeReadCommand.create(true, metadata, nowInSec, selection, rowFilter, limits, ((DataRange) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DataRange(keyRange, filter), 25)));
        }
        ...
        public long serializedSize(ReadCommand command, int version) {
            ...
            // single byte flag: 0 for slices, 1 for names
            size += 1;
            if (rangeCommand.isNamesQuery()) {
        ...
        static PartitionRangeReadCommand maybeConvertNamesToSlice(PartitionRangeReadCommand command) {
            ...
            org.zlab.ocov.tracker.Runtime.update(sliceFilter, 13, command);
            DataRange newRange = ((DataRange) org.zlab.ocov.tracker.Runtime.update(new DataRange(command.dataRange().keyRange(), sliceFilter), 14, command));
    ...
    private static class LegacyPagedRangeCommandSerializer implements IVersionedSerializer<ReadCommand> {
        ...
        public void serialize(ReadCommand command, DataOutputPlus out, int version) throws IOException {
            ...
            if (rangeCommand.dataRange().clusteringIndexFilter.kind() == ClusteringIndexFilter.Kind.NAMES) {
                ...
                org.zlab.ocov.tracker.Runtime.update(filter, 15, command, out, version);
            } else
        ...
        public ReadCommand deserialize(DataInputPlus in, int version) throws IOException {
            ...
            // the composite "stop", which isn't actually needed
            ...
            // is what 3.0 does.
            DataRange dataRange = ((DataRange) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new DataRange(keyRange, filter), 26));
    ...
    static class LegacyReadCommandSerializer implements IVersionedSerializer<ReadCommand> {
        ...
        private static void serializeNamesFilter(ReadCommand command, ClusteringIndexNamesFilter filter, DataOutputPlus out) throws IOException {
            ...
            if (command.isForThrift() || (command.limits().kind() == DataLimits.Kind.CQL_LIMIT && command.limits().perPartitionCount() == 1))
                // it's compact and not a DISTINCT query
                out.writeBoolean(false);
            else
                out.writeBoolean(true);
        }
        ...
        static Pair<ColumnFilter, ClusteringIndexNamesFilter> deserializeNamesSelectionAndFilter(DataInputPlus in, CFMetaData metadata) throws IOException {
            ...
            for (int i = 0; i < numCellNames; i++) {
                ...
                if (metadata.isStaticCompactTable() && cellName.clustering.equals(Clustering.STATIC_CLUSTERING)) {
                    clusterings.add(((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(cellName.column.name.bytes), 27)));
            ...
            ClusteringIndexNamesFilter filter = ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ClusteringIndexNamesFilter(clusterings, false), 28));
        ...
        private void serializeSliceCommand(SinglePartitionReadCommand command, DataOutputPlus out) throws IOException {
            ...
            if (limits.isDistinct())
                // the limit is always 1 for DISTINCT queries
                out.writeInt(1);
            else
        ...
        private SinglePartitionReadCommand deserializeSliceCommand(DataInputPlus in, boolean isDigest, CFMetaData metadata, DecoratedKey key, int nowInSeconds, int version) throws IOException {
            ...
            DataLimits limits;
            if (compositesToGroup == -2)
                // See CASSANDRA-8490 for the explanation of this value
        ...
        private static void serializeStaticSlice(DataOutputPlus out, boolean isReversed, CFMetaData metadata) throws IOException {
            ...
            // two bytes + EOC for each component, plus static prefix
        ...
        static Pair<ClusteringIndexSliceFilter, Boolean> deserializeSlicePartitionFilter(DataInputPlus in, CFMetaData metadata) throws IOException {
            ...
            for (int i = 0; i < numSlices; i++) {
                ...
                // sense for 3.0).
                if (finish.isStatic) {
                    // it would make no sense for a pre-3.0 node to have a slice that stops
            ...
            return Pair.create(((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ClusteringIndexSliceFilter(slicesBuilder.build(), reversed), 29)), selectsStatics);
        }
        ...
        private static SinglePartitionReadCommand maybeConvertNamesToSlice(SinglePartitionReadCommand command) {
            ...
            org.zlab.ocov.tracker.Runtime.update(sliceFilter, 16, command);
        ...
        private static ClusteringIndexSliceFilter convertNamesFilterToSliceFilter(ClusteringIndexNamesFilter filter, CFMetaData metadata) {
            ...
            if (requestedRows.isEmpty()) {
                ...
                Slices.Builder slicesBuilder = new Slices.Builder(metadata.comparator);
                for (Clustering clustering : requestedRows) slicesBuilder.add(Slice.Bound.inclusiveStartOf(clustering), Slice.Bound.inclusiveEndOf(clustering));
                slices = slicesBuilder.build();
            }
            return ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ClusteringIndexSliceFilter(slices, filter.isReversed()), 30));
        }
