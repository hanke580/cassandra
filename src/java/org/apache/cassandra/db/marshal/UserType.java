...
public class UserType extends TupleType {
    ...
    public Term fromJSONObject(Object parsed) throws MarshalException {
        ...
        for (int i = 0; i < types.size(); i++) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.stringFieldNames, "this.stringFieldNames").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        ...
        if (foundValues != map.size()) {
            for (Object fieldName : keys) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(keys, fieldName, "fieldName").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
    ...
    public boolean equals(Object o) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(that, that.types, "that.types").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(that, that.fieldNames, "that.fieldNames").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
