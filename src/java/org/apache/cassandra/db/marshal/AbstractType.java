...
public abstract class AbstractType<T> implements Comparator<ByteBuffer>, AssignmentTestable {
    ...
    public boolean isSerializationCompatibleWith(AbstractType<?> previous) {
        return isValueCompatibleWith(previous) && valueLengthIfFixed() == previous.valueLengthIfFixed() && isMultiCell() == previous.isMultiCell();
    }
    ...
    public <V> V read(ValueAccessor<V> accessor, DataInputPlus in, int maxValueSize) throws IOException {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(length, 0, ">=", 5)))
            ...
            if (l > maxValueSize)
                throw new IOException(String.format("Corrupt value length %d encountered, as it exceeds the maximum of %d, " + "which is set via max_value_size in cassandra.yaml", l, maxValueSize));
