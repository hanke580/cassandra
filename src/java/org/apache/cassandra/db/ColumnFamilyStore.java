...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean {
        ...
        for (ColumnDefinition def : metadata.allColumns()) {
            if (def.isIndexed()) {
                ...
                if (indexComparator != null) {
                    CFMetaData indexMetadata = ((org.apache.cassandra.config.CFMetaData) org.zlab.ocov.tracker.Runtime.update(CFMetaData.newIndexMetadata(metadata, def, indexComparator), 9, metadata));
    ...
    public static void removeUnfinishedCompactionLeftovers(CFMetaData metadata, Map<Integer, UUID> unfinishedCompactions) {
        ...
        if (!allGenerations.containsAll(unfinishedGenerations)) {
            ...
            logger.trace("Unfinished compactions of {}.{} reference missing sstables of generations {}", metadata.ksName, metadata.cfName, missingGenerations);
        }
    ...
    public synchronized void loadNewSSTables() {
        ...
        for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet()) {
            ...
            if (currentDescriptors.contains(descriptor))
                ...
                continue;
            if (// in the process of being written
            descriptor.type.isTemporary)
                continue;
    ...
    public ListenableFuture<ReplayPosition> switchMemtable() {
        synchronized (data) {
            if (previousFlushFailure != null)
                throw new IllegalStateException("A flush previously failed with the error below. To prevent data loss, " + "no flushes can be carried out until the node is restarted.", previousFlushFailure);
            logFlush();
            Flush flush = ((Flush) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Flush(false), 42));
            ...
            ListenableFuture<ReplayPosition> future = // If either of the two tasks errors out, resulting future must also error out.
            ...
            Futures.transform(Futures.allAsList(flushTask, task), new Function<List<Object>, ReplayPosition>() {
    ...
    private final class PostFlush implements Callable<ReplayPosition> {
        ...
        private PostFlush(boolean flushSecondaryIndexes, OpOrder.Barrier writeBarrier, ReplayPosition commitLogUpperBound, List<Memtable> memtables, List<SSTableReader> readers) {
    ...
    private final class Flush implements Runnable {
        ...
        public void run() {
            org.zlab.ocov.tracker.Runtime.update(this, 10);
    ...
    @VisibleForTesting
    public // the commit log as done, however they *will* terminate (unlike under typical failures) to ensure progress is made
    void simulateFailedFlush() {
    ...
    private ColumnFamily getThroughCache(UUID cfId, QueryFilter filter) {
        ...
        try {
            ...
            if (filter.filter.isHeadFilter() && filter.filter.countCQL3Rows(metadata.comparator)) {
                ...
                QueryFilter cacheFilter = ((QueryFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new QueryFilter(filter.key, name, cacheSlice, filter.timestamp), 43));
    ...
    public SliceQueryFilter readFilterForCache() {
        ...
        return ((SliceQueryFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SliceQueryFilter(ColumnSlice.ALL_COLUMNS_ARRAY, false, metadata.getCaching().rowCache.rowsToCache, metadata.clusteringColumns().size()), 44));
    }
    ...
    public ColumnFamily getTopLevelColumns(QueryFilter filter, int gcBefore) {
        ...
        CollationController controller = ((CollationController) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new CollationController(this, filter, gcBefore), 45));
    ...
    public ExtendedFilter makeExtendedFilter(AbstractBounds<RowPosition> range, IDiskAtomFilter columnFilter, List<IndexExpression> rowFilter, int maxResults, boolean countCQL3Rows, boolean isPaging, long timestamp) {
        ...
        if (isPaging) {
            ...
            SliceQueryFilter newFilter = ((SliceQueryFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SliceQueryFilter(ColumnSlice.ALL_COLUMNS_ARRAY, sfilter.isReversed(), sfilter.count), 46));
        ...
        try {
            while (rowIterator.hasNext() && matched < filter.maxRows() && columnsCount < filter.maxColumns()) {
                ...
                if (rowIterator.needsFiltering()) {
                    ...
                    if (extraFilter != null) {
                        ColumnFamily cf = filter.cfs.getColumnFamily(((QueryFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new QueryFilter(rawRow.key, name, extraFilter, filter.timestamp), 47)));
                        if (cf != null)
                            data.addAll(cf);
                    }
                ...
                rows.add(((Row) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Row(rawRow.key, data), 48)));
    ...
    public void truncateBlocking(final boolean takeSnapshot) {
        ...
        if (keyspace.getMetadata().durableWrites || takeSnapshot) {
            ...
            synchronized (data) {
                final Flush flush = ((Flush) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Flush(true), 49));
    ...
    public long estimateKeys() {
        long n = 0;
        for (SSTableReader sstable : getSSTables()) n += sstable.estimatedKeys();
        return n;
    }
    ...
    public static final Function<View, List<SSTableReader>> CANONICAL_SSTABLES = new Function<View, List<SSTableReader>>() {
        ...
        public List<SSTableReader> apply(View view) {
            ...
            for (SSTableReader sstable : view.compacting) if (sstable.openReason != SSTableReader.OpenReason.EARLY)
                sstables.add(sstable);
            for (SSTableReader sstable : view.sstables) if (!view.compacting.contains(sstable) && sstable.openReason != SSTableReader.OpenReason.EARLY)
                sstables.add(sstable);
            return sstables;
        }
    };
