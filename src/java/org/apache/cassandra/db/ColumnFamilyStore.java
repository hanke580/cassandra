        ...
        for (Descriptor desc : directories.sstableLister().list().keySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(directories.sstableLister().list().keySet(), desc, "desc").toJsonString());
    ...
    public void apply(DecoratedKey key, ColumnFamily columnFamily, SecondaryIndexManager.Updater indexer, OpOrder.Group opGroup, ReplayPosition replayPosition) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.metrics.ColumnFamilyMetrics.Sampler.class, org.apache.cassandra.metrics.ColumnFamilyMetrics.Sampler.WRITES, "org.apache.cassandra.metrics.ColumnFamilyMetrics.Sampler.WRITES").toJsonString());
