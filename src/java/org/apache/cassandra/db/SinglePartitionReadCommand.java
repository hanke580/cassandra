...
public class SinglePartitionReadCommand extends ReadCommand {
    ...
    public static SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, Slices slices) {
        ClusteringIndexSliceFilter filter = ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexSliceFilter(slices, false), 132, metadata, nowInSec, key, slices));
    ...
    public SinglePartitionReadCommand copyAsDigestQuery() {
        return new SinglePartitionReadCommand(true, digestVersion(), isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits(), partitionKey(), clusteringIndexFilter(), indexMetadata());
    }

    public SinglePartitionReadCommand withUpdatedClusteringIndexFilter(ClusteringIndexFilter filter) {
        return new SinglePartitionReadCommand(isDigestQuery(), digestVersion(), isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits(), partitionKey(), filter, indexMetadata());
    }
    ...
    public SinglePartitionReadCommand forPaging(Clustering lastReturned, int pageSize) {
        ...
        return ((org.apache.cassandra.db.SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.update(create(isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits().forPaging(pageSize), partitionKey(), lastReturned == null ? clusteringIndexFilter() : clusteringIndexFilter.forPaging(metadata().comparator, lastReturned, false)), 133, lastReturned, pageSize));
    }

    public PartitionIterator execute(ConsistencyLevel consistency, ClientState clientState) throws RequestExecutionException {
        return StorageProxy.read(((org.apache.cassandra.db.SinglePartitionReadCommand.Group) org.zlab.ocov.tracker.Runtime.update(Group.one(this), 134, consistency, clientState)), consistency, clientState);
    }
    ...
    private UnfilteredRowIterator queryMemtableAndDiskInternal(ColumnFamilyStore cfs, boolean copyOnHeap) {
        ...
        if (clusteringIndexFilter() instanceof ClusteringIndexNamesFilter && !queriesMulticellType()) {
            return ((org.apache.cassandra.db.rows.UnfilteredRowIterator) org.zlab.ocov.tracker.Runtime.update(queryMemtableAndSSTablesInTimestampOrder(cfs, copyOnHeap, (ClusteringIndexNamesFilter) clusteringIndexFilter()), 135, cfs, copyOnHeap));
        }
                ...
                if (shouldInclude(sstable)) {
                    ...
                    if (sstable.hasTombstones()) {
                        ...
                        if (!iter.partitionLevelDeletion().isLive()) {
                            ...
                            iterators.add(((org.apache.cassandra.db.rows.UnfilteredRowIterator) org.zlab.ocov.tracker.Runtime.update(RTBoundValidator.validate(iter, RTBoundValidator.Stage.SSTABLE, false), 136, cfs, copyOnHeap)));
                            if (!sstable.isRepaired())
                                oldestUnrepairedTombstone = Math.min(oldestUnrepairedTombstone, sstable.getMinLocalDeletionTime());
                            mostRecentPartitionTombstone = Math.max(mostRecentPartitionTombstone, iter.partitionLevelDeletion().markedForDeleteAt());
                        } else {
    ...
    private UnfilteredRowIterator queryMemtableAndSSTablesInTimestampOrder(ColumnFamilyStore cfs, boolean copyOnHeap, ClusteringIndexNamesFilter filter) {
        Tracing.trace("Acquiring sstable references");
        ColumnFamilyStore.ViewFragment view = cfs.select(View.select(SSTableSet.LIVE, partitionKey()));
        org.zlab.ocov.tracker.Runtime.update(view, 137, cfs, copyOnHeap, filter);
        ...
        for (Memtable memtable : view.memtables) {
            ...
            try (UnfilteredRowIterator iter = filter.getUnfilteredRowIterator(columnFilter(), partition)) {
                ...
                result = add(RTBoundValidator.validate(isForThrift() ? ThriftResultsMerger.maybeWrap(clonedIter, nowInSec()) : clonedIter, RTBoundValidator.Stage.MEMTABLE, false), result, filter, false);
            }
        }
        ...
        for (SSTableReader sstable : view.sstables) {
            ...
            filter = ((org.apache.cassandra.db.filter.ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(reduceFilter(filter, result, currentMaxTs), 138, cfs, copyOnHeap, filter));
            if (filter == null)
                break;
            if (!shouldInclude(sstable)) {
                ...
                try (UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), columnFilter(), filter.isReversed(), isForThrift(), metricsCollector))) {
                    if (!iter.partitionLevelDeletion().isLive()) {
                        ...
                        result = add(((org.apache.cassandra.db.rows.UnfilteredRowIterator) org.zlab.ocov.tracker.Runtime.update(RTBoundValidator.validate(iter, RTBoundValidator.Stage.SSTABLE, false), 139, cfs, copyOnHeap, filter)), result, filter, sstable.isRepaired());
                    }
                }
                continue;
            }
            ...
            try (UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), columnFilter(), filter.isReversed(), isForThrift(), metricsCollector))) {
                if (iter.isEmpty())
                    continue;
                result = add(RTBoundValidator.validate(isForThrift() ? ThriftResultsMerger.maybeWrap(iter, nowInSec()) : iter, RTBoundValidator.Stage.SSTABLE, false), result, filter, sstable.isRepaired());
            }
        }
        cfs.metric.updateSSTableIterated(metricsCollector.getMergedSSTables());
    ...
    private ClusteringIndexNamesFilter reduceFilter(ClusteringIndexNamesFilter filter, Partition result, long sstableTimestamp) {
        ...
        if (!columns.statics.isEmpty()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(searchIter, 140, filter, result, sstableTimestamp);
            removeStatic = staticRow != null && isRowComplete(staticRow, columns.statics, sstableTimestamp);
        }
        NavigableSet<Clustering> toRemove = null;
        for (Clustering clustering : clusterings) {
            Row row = searchIter.next(clustering);
            org.zlab.ocov.tracker.Runtime.update(searchIter, 141, filter, result, sstableTimestamp);
        ...
        return ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(clusterings, filter.isReversed()), 142, filter, result, sstableTimestamp));
    }
