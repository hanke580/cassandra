...
public class SinglePartitionReadCommand extends ReadCommand {
    ...
    public static SinglePartitionReadCommand create(boolean isForThrift, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, RowFilter rowFilter, DataLimits limits, DecoratedKey partitionKey, ClusteringIndexFilter clusteringIndexFilter, IndexMetadata indexMetadata) {
        return ((SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionReadCommand(false, 0, isForThrift, metadata, nowInSec, columnFilter, rowFilter, limits, partitionKey, clusteringIndexFilter, indexMetadata), 195));
    }
    ...
    public static SinglePartitionReadCommand create(boolean isForThrift, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, RowFilter rowFilter, DataLimits limits, DecoratedKey partitionKey, ClusteringIndexFilter clusteringIndexFilter) {
        return create(isForThrift, metadata, nowInSec, columnFilter, rowFilter, limits, partitionKey, clusteringIndexFilter, findIndex(metadata, rowFilter));
    }
    ...
    public static SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, Slices slices) {
        ClusteringIndexSliceFilter filter = ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexSliceFilter(slices, false), 87, metadata, nowInSec, key, slices));
    ...
    public SinglePartitionReadCommand copy() {
        return new SinglePartitionReadCommand(isDigestQuery(), digestVersion(), isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits(), partitionKey(), clusteringIndexFilter(), indexMetadata());
    }

    public SinglePartitionReadCommand copyAsDigestQuery() {
        return ((SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionReadCommand(true, digestVersion(), isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits(), partitionKey(), clusteringIndexFilter(), indexMetadata()), 196));
    }

    public SinglePartitionReadCommand withUpdatedClusteringIndexFilter(ClusteringIndexFilter filter) {
        return ((SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionReadCommand(isDigestQuery(), digestVersion(), isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits(), partitionKey(), filter, indexMetadata()), 197));
    }
    ...
    static SinglePartitionReadCommand legacySliceCommand(boolean isDigest, int digestVersion, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, DataLimits limits, DecoratedKey partitionKey, ClusteringIndexSliceFilter filter) {
        ...
        return ((SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionReadCommand(isDigest, digestVersion, true, metadata, nowInSec, columnFilter, RowFilter.NONE, limits, partitionKey, filter, null), 198));
    }
    ...
    static SinglePartitionReadCommand legacyNamesCommand(boolean isDigest, int digestVersion, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, DecoratedKey partitionKey, ClusteringIndexNamesFilter filter) {
        // messages from old nodes will expect the thrift format, so always use 'true' for isForThrift
        return ((SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionReadCommand(isDigest, digestVersion, true, metadata, nowInSec, columnFilter, RowFilter.NONE, DataLimits.NONE, partitionKey, filter, null), 199));
    }
    ...
    public SinglePartitionReadCommand forPaging(Clustering lastReturned, int pageSize) {
        ...
        return ((org.apache.cassandra.db.SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.update(create(isForThrift(), metadata(), nowInSec(), columnFilter(), rowFilter(), limits().forPaging(pageSize), partitionKey(), lastReturned == null ? clusteringIndexFilter() : clusteringIndexFilter.forPaging(metadata().comparator, lastReturned, false)), 88, lastReturned, pageSize));
    }

    public PartitionIterator execute(ConsistencyLevel consistency, ClientState clientState) throws RequestExecutionException {
        return StorageProxy.read(((org.apache.cassandra.db.SinglePartitionReadCommand.Group) org.zlab.ocov.tracker.Runtime.update(Group.one(this), 89, consistency, clientState)), consistency, clientState);
    }
    ...
    private static SinglePartitionPager getPager(SinglePartitionReadCommand command, PagingState pagingState, int protocolVersion) {
        return ((SinglePartitionPager) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionPager(command, pagingState, protocolVersion), 200));
    }
    ...
    protected UnfilteredPartitionIterator queryStorage(final ColumnFamilyStore cfs, ReadOrderGroup orderGroup) {
        UnfilteredRowIterator partition = cfs.isRowCacheEnabled() ? getThroughCache(cfs, orderGroup.baseReadOpOrderGroup()) : queryMemtableAndDisk(cfs, orderGroup.baseReadOpOrderGroup());
    ...
    private UnfilteredRowIterator queryMemtableAndDiskInternal(ColumnFamilyStore cfs, boolean copyOnHeap) {
        ...
        if (clusteringIndexFilter() instanceof ClusteringIndexNamesFilter && !queriesMulticellType()) {
            return ((org.apache.cassandra.db.rows.UnfilteredRowIterator) org.zlab.ocov.tracker.Runtime.update(queryMemtableAndSSTablesInTimestampOrder(cfs, copyOnHeap, (ClusteringIndexNamesFilter) clusteringIndexFilter()), 90, cfs, copyOnHeap));
        }
        ...
        try {
            ...
            for (SSTableReader sstable : view.sstables) {
                ...
                // 'iter' is added to iterators which is closed on exception, or through the closing of the final merged iterator
                @SuppressWarnings("resource")
                UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), columnFilter(), filter.isReversed(), isForThrift(), metricsCollector));
            ...
            if (skippedSSTables != null) {
                for (SSTableReader sstable : skippedSSTables) {
                    if (sstable.getMaxTimestamp() <= minTimestamp)
                        continue;
                    // 'iter' is either closed right away, or added to iterators which is close on exception, or through the closing of the final merged iterator
                    @SuppressWarnings("resource")
                    UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), columnFilter(), filter.isReversed(), isForThrift(), metricsCollector));
            ...
            //  Closed through the closing of the result of that method.
            @SuppressWarnings("resource")
    ...
    private UnfilteredRowIterator queryMemtableAndSSTablesInTimestampOrder(ColumnFamilyStore cfs, boolean copyOnHeap, ClusteringIndexNamesFilter filter) {
        ...
        for (SSTableReader sstable : view.sstables) {
            ...
            if (!shouldInclude(sstable)) {
                ...
                if (!sstable.hasTombstones())
                    // Means no tombstone at all, we can skip that sstable
                    continue;
                // We need to get the partition deletion and include it if it's live. In any case though, we're done with that sstable.
                try (UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), columnFilter(), filter.isReversed(), isForThrift(), metricsCollector))) {
        ...
        if (metricsCollector.getMergedSSTables() > cfs.getMinimumCompactionThreshold() && onlyUnrepaired && !cfs.isAutoCompactionDisabled() && cfs.getCompactionStrategyManager().shouldDefragment()) {
            ...
            try (UnfilteredRowIterator iter = result.unfilteredIterator(columnFilter(), Slices.ALL, false)) {
                final Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(PartitionUpdate.fromIterator(iter)), 201));
    ...
    private ClusteringIndexNamesFilter reduceFilter(ClusteringIndexNamesFilter filter, Partition result, long sstableTimestamp) {
        ...
        if (!columns.statics.isEmpty()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(searchIter, 92, filter, result, sstableTimestamp);
            removeStatic = staticRow != null && canRemoveRow(staticRow, columns.statics, sstableTimestamp);
        }
        NavigableSet<Clustering> toRemove = null;
        for (Clustering clustering : clusterings) {
            Row row = searchIter.next(clustering);
            org.zlab.ocov.tracker.Runtime.update(searchIter, 93, filter, result, sstableTimestamp);
        ...
        return ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(clusterings, filter.isReversed()), 94, filter, result, sstableTimestamp));
    }
    ...
    public static class Group implements ReadQuery {
        ...
        public static Group one(SinglePartitionReadCommand command) {
            return ((Group) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Group(Collections.<SinglePartitionReadCommand>singletonList(command), command.limits()), 202));
        }
        ...
        public PartitionIterator executeInternal(ReadOrderGroup orderGroup) {
            ...
            for (SinglePartitionReadCommand cmd : commands) partitions.add(cmd.executeInternal(orderGroup));
            // Note that the only difference between the command in a group must be the partition key on which
            // they applied.
            ...
            return limits.filter(PartitionIterators.concat(partitions), nowInSec, selectsFullPartitions, enforceStrictLiveness);
        }
    ...
    private static class Deserializer extends SelectionDeserializer {
        ...
        public ReadCommand deserialize(DataInputPlus in, int version, boolean isDigest, int digestVersion, boolean isForThrift, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, RowFilter rowFilter, DataLimits limits, IndexMetadata index) throws IOException {
            ...
            return ((SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SinglePartitionReadCommand(isDigest, digestVersion, isForThrift, metadata, nowInSec, columnFilter, rowFilter, limits, key, filter, index), 203));
        }
    }
