...
public class RangeTombstoneList implements Iterable<RangeTombstone>, IMeasurableMemory {
    ...
    public RangeTombstoneList copy(AbstractAllocator allocator) {
        RangeTombstoneList copy = new RangeTombstoneList(comparator, new Slice.Bound[size], new Slice.Bound[size], Arrays.copyOf(markedAts, size), Arrays.copyOf(delTimes, size), boundaryHeapSize, size);
    ...
    private static Slice.Bound clone(Slice.Bound bound, AbstractAllocator allocator) {
        ...
        return ((Slice.Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice.Bound(bound.kind(), values), 132));
    }
    ...
    private RangeTombstone rangeTombstone(int idx) {
        return ((RangeTombstone) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone(Slice.make(starts[idx], ends[idx]), new DeletionTime(markedAts[idx], delTimes[idx])), 133));
    }

    private RangeTombstone rangeTombstoneWithNewStart(int idx, Slice.Bound newStart) {
        return ((RangeTombstone) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone(Slice.make(newStart, ends[idx]), new DeletionTime(markedAts[idx], delTimes[idx])), 134));
    }

    private RangeTombstone rangeTombstoneWithNewEnd(int idx, Slice.Bound newEnd) {
        return ((RangeTombstone) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone(Slice.make(starts[idx], newEnd), new DeletionTime(markedAts[idx], delTimes[idx])), 135));
    }

    private RangeTombstone rangeTombstoneWithNewBounds(int idx, Slice.Bound newStart, Slice.Bound newEnd) {
        return ((RangeTombstone) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone(Slice.make(newStart, newEnd), new DeletionTime(markedAts[idx], delTimes[idx])), 136));
    }
