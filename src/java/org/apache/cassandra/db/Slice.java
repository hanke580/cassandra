...
public class Slice {
    ...
    public static Slice make(Bound start, Bound end) {
        if (start == Bound.BOTTOM && end == Bound.TOP)
            return ALL;
        return ((Slice) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice(start, end), 93));
    }
    ...
    public static Slice make(ClusteringComparator comparator, Object... values) {
        ...
        for (Object val : values) {
            if (val instanceof ByteBuffer) {
                ...
                org.zlab.ocov.tracker.Runtime.update(builder, 42, comparator, values);
            } else {
                builder.add(val);
                org.zlab.ocov.tracker.Runtime.update(builder, 43, comparator, values);
            }
        }
        return ((Slice) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice(builder.buildBound(true, true), builder.buildBound(false, true)), 94));
    }
    ...
    public static Slice make(Clustering clustering) {
        ...
        return ((Slice) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice(Bound.inclusiveStartOf(values), Bound.inclusiveEndOf(values)), 95));
    }
    ...
    public Slice forPaging(ClusteringComparator comparator, Clustering lastReturned, boolean inclusive, boolean reversed) {
        ...
        if (reversed) {
            ...
            return ((Slice) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice(start, inclusive ? Bound.inclusiveEndOf(values) : Bound.exclusiveEndOf(values)), 96));
        } else {
            ...
            if (cmp < 0 || (inclusive && cmp == 0))
                return this;
            ByteBuffer[] values = extractValues(lastReturned);
            return ((Slice) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice(inclusive ? Bound.inclusiveStartOf(values) : Bound.exclusiveStartOf(values), end), 97));
        }
    }
    ...
    public static class Serializer {
        ...
        public Slice deserialize(DataInputPlus in, int version, List<AbstractType<?>> types) throws IOException {
            ...
            return ((Slice) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Slice(start, end), 98));
        }
    }
    ...
    public static class Bound extends AbstractClusteringPrefix {
        ...
        public static Bound create(Kind kind, ByteBuffer[] values) {
            ...
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(kind, values), 99));
        }

        public static Kind boundKind(boolean isStart, boolean isInclusive) {
            return isStart ? (isInclusive ? Kind.INCL_START_BOUND : Kind.EXCL_START_BOUND) : (isInclusive ? Kind.INCL_END_BOUND : Kind.EXCL_END_BOUND);
        }
        ...
        public static Bound inclusiveStartOf(ClusteringPrefix prefix) {
            ...
            for (int i = 0; i < prefix.size(); i++) values[i] = prefix.get(i);
        ...
        public Bound withNewKind(Kind kind) {
            assert !kind.isBoundary();
            return ((Bound) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Bound(kind, values), 100));
        }
