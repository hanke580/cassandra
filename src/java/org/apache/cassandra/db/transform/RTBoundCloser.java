...
public final class RTBoundCloser extends Transformation<UnfilteredRowIterator> {
    ...
    private final static class RowsTransformation extends Transformation implements MoreRows<UnfilteredRowIterator> {
        ...
        public UnfilteredRowIterator moreContents() {
            ...
            RangeTombstoneBoundMarker closingBound = RangeTombstoneBoundMarker.inclusiveClose(partition.isReverseOrder(), lastRowClustering.getRawValues(), openMarkerDeletionTime);
            return UnfilteredRowIterators.singleton(closingBound, partition.metadata(), partition.partitionKey(), partition.partitionLevelDeletion(), partition.columns(), partition.staticRow(), partition.isReverseOrder(), ((org.apache.cassandra.db.rows.EncodingStats) org.zlab.ocov.tracker.Runtime.update(partition.stats(), 157)));
        }
    }
}
