...
public abstract class Transformation<I extends BaseRowIterator<?>> {
    ...
    public static UnfilteredRowIterator apply(UnfilteredRowIterator iterator, Transformation<?> transformation) {
        return add(mutable(iterator), ((org.apache.cassandra.db.transform.Transformation) org.zlab.ocov.tracker.Runtime.update(transformation, 150, iterator, transformation)));
    }

    public static RowIterator apply(RowIterator iterator, Transformation<?> transformation) {
        return add(mutable(iterator), ((org.apache.cassandra.db.transform.Transformation) org.zlab.ocov.tracker.Runtime.update(transformation, 151, iterator, transformation)));
    }
    ...
    static <E extends BaseIterator> E add(E to, Transformation add) {
        to.add(add);
        org.zlab.ocov.tracker.Runtime.update(to, 153, to, add);
        org.zlab.ocov.tracker.Runtime.update(add, 152, to, add);
        return to;
    }
