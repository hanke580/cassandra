...
public class Gossiper implements IFailureDetectionEventListener, GossiperMBean {
    ...
    public Set<InetAddress> getLiveMembers() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.liveEndpoints, "this.liveEndpoints").toJsonString());
    ...
    private void doStatusCheck() {
        ...
        for (InetAddress endpoint : eps) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(eps, endpoint, "endpoint").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public UUID getHostId(InetAddress endpoint) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.gms.ApplicationState.class, org.apache.cassandra.gms.ApplicationState.HOST_ID, "org.apache.cassandra.gms.ApplicationState.HOST_ID").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    private void applyNewStates(InetAddress addr, EndpointState localState, EndpointState remoteState) {
        ...
        for (Entry<ApplicationState, VersionedValue> remoteEntry : remoteStates) doOnChangeNotifications(addr, remoteEntry.getKey(), remoteEntry.getValue());
    }
    ...
    private void buildSeedsList() {
        for (InetAddress seed : DatabaseDescriptor.getSeeds()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(DatabaseDescriptor.getSeeds(), seed, "seed").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
