...
public class Gossiper implements IFailureDetectionEventListener, GossiperMBean {
    ...
    private boolean sendGossip(MessageOut<GossipDigestSyn> message, Set<InetAddress> epSet) {
        ...
        org.zlab.net.tracker.Runtime.record("sendOneWay", 7, message, to);
    ...
    private void maybeGossipToUnreachableMember(MessageOut<GossipDigestSyn> message) {
        ...
        if (unreachableEndpointCount > 0) {
            ...
            if (randDbl < prob)
                sendGossip(message, Sets.filter(unreachableEndpoints.keySet(), ep -> !isDeadState(endpointStateMap.get(ep))));
        }
    }
    ...
    private void markAlive(final InetAddress addr, final EndpointState localState) {
        ...
        org.zlab.net.tracker.Runtime.record("sendRR", 8, echoMessage, addr, echoHandler);
    }
    ...
    void applyStateLocally(Map<InetAddress, EndpointState> epStateMap) {
        ...
        if (any30 != anyNodeOn30) {
            logger.info(any30 ? "There is at least one 3.0 node in the cluster - will store and announce compatible schema version" : "There are no 3.0 nodes in the cluster - will store and announce real schema version");
            anyNodeOn30 = any30;
            ...
            while (true) {
                if (slept % 5000 == 0) {
                    ...
                    if (includePeers) {
                        ...
                        for (InetAddress peer : peers) MessagingService.instance().sendOneWay(message, peer);
                    }
                    includePeers = true;
                }
                Thread.sleep(1000);
