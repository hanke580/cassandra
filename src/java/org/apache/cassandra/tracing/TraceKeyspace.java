...
public final class TraceKeyspace {
    ...
    static Mutation makeStartSessionMutation(ByteBuffer sessionId, InetAddress client, Map<String, String> parameters, String request, long startedAt, String command, int ttl) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Sessions, FBUtilities.timestampMicros(), ttl, sessionId).clustering().add("client", client).add("coordinator", FBUtilities.getBroadcastAddress()).add("request", request).add("started_at", new Date(startedAt)).add("command", command), 5));
    ...
    static Mutation makeStopSessionMutation(ByteBuffer sessionId, int elapsed, int ttl) {
        return ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Sessions, FBUtilities.timestampMicros(), ttl, sessionId).clustering().add("duration", elapsed), 6)).build();
    }

    static Mutation makeEventMutation(ByteBuffer sessionId, String message, int elapsed, String threadName, int ttl) {
        RowUpdateBuilder adder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(Events, FBUtilities.timestampMicros(), ttl, sessionId).clustering(UUIDGen.getTimeUUID()), 7));
