...
public final class TraceKeyspace {
    ...
    static Mutation makeStartSessionMutation(ByteBuffer sessionId, InetAddress client, Map<String, String> parameters, String request, long startedAt, String command, int ttl) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.tracing.TraceKeyspace.class, org.apache.cassandra.tracing.TraceKeyspace.Sessions, "org.apache.cassandra.tracing.TraceKeyspace.Sessions").toJsonString());
        ...
        adder.add("client", client).add("coordinator", FBUtilities.getBroadcastAddress()).add("request", request).add("started_at", new Date(startedAt)).add("command", command);
    ...
    static Mutation makeEventMutation(ByteBuffer sessionId, String message, int elapsed, String threadName, int ttl) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.tracing.TraceKeyspace.class, org.apache.cassandra.tracing.TraceKeyspace.Events, "org.apache.cassandra.tracing.TraceKeyspace.Events").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
        ...
        adder.add("activity", message).add("source", FBUtilities.getBroadcastAddress()).add("thread", threadName);
        if (elapsed >= 0)
