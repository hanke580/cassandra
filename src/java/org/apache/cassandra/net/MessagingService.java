    ...
    List<String> getDroppedMessagesLogs() {
        ...
        for (Map.Entry<Verb, DroppedMessages> entry : droppedMessagesMap.entrySet()) {
            ...
            if (droppedInternalTimeout > 0 || droppedCrossNodeTimeout > 0) {
                ret.add(String.format("%s messages were dropped in last %d ms: %d for internal timeout and %d for cross node timeout", verb, LOG_DROPPED_INTERVAL_IN_MS, droppedInternalTimeout, droppedCrossNodeTimeout));
            }
        }
        return ret;
    }
