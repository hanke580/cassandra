...
public final class LegacyBatchlogMigrator {
    ...
    private static boolean apply(UntypedResultSet.Row row, long counter) {
        ...
        try (DataInputBuffer in = new DataInputBuffer(row.getBytes("data"), false)) {
            ...
            BatchlogManager.store(((org.apache.cassandra.batchlog.Batch) org.zlab.ocov.tracker.Runtime.update(Batch.createLocal(id, TimeUnit.MILLISECONDS.toMicros(timestamp), mutations), 216, row, counter)));
            return true;
        } catch (Throwable t) {
