...
public final class LegacyBatchlogMigrator {
    ...
    public static void syncWriteToBatchlog(WriteResponseHandler<?> handler, Batch batch, Collection<InetAddress> endpoints) throws WriteTimeoutException, WriteFailureException {
        for (InetAddress target : endpoints) {
            ...
            org.zlab.net.tracker.Runtime.record("sendRR", 5, getStoreMutation(batch, targetVersion).createMessage(MessagingService.Verb.MUTATION), target, handler, false);
        }
    }
    ...
    public static void asyncRemoveFromBatchlog(Collection<InetAddress> endpoints, UUID uuid) {
        ...
        for (InetAddress target : endpoints) {
            ...
            org.zlab.net.tracker.Runtime.record("sendRR", 6, mutation.createMessage(MessagingService.Verb.MUTATION), target, handler, false);
        }
    }
    ...
    static Mutation getStoreMutation(Batch batch, int version) {
        return new RowUpdateBuilder(SystemKeyspace.LegacyBatchlog, batch.creationTime, batch.id).clustering().add("written_at", new Date(batch.creationTime / 1000)).add("data", getSerializedMutations(version, batch.decodedMutations)).add("version", version).build();
    }
