...
public final class LegacyBatchlogMigrator {
    ...
    public static boolean isLegacyBatchlogMutation(Mutation mutation) {
        return mutation.getKeyspaceName().equals(SystemKeyspace.NAME) && mutation.getPartitionUpdate(SystemKeyspace.LegacyBatchlog.cfId) != null;
    }
    ...
    private static boolean apply(UntypedResultSet.Row row, long counter) {
        ...
        try (DataInputBuffer in = new DataInputBuffer(row.getBytes("data"), false)) {
            ...
            BatchlogManager.store(((org.apache.cassandra.batchlog.Batch) org.zlab.ocov.tracker.Runtime.update(Batch.createLocal(id, TimeUnit.MILLISECONDS.toMicros(timestamp), mutations), 148, row, counter)));
            return true;
        } catch (Throwable t) {
    ...
    static Mutation getStoreMutation(Batch batch, int version) {
        return ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(SystemKeyspace.LegacyBatchlog, batch.creationTime, batch.id).clustering().add("written_at", new Date(batch.creationTime / 1000)).add("data", getSerializedMutations(version, batch.decodedMutations)).add("version", version), 297)).build();
    }

    @SuppressWarnings("deprecation")
    private static Mutation getRemoveMutation(UUID uuid) {
        return ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(PartitionUpdate.fullPartitionDelete(SystemKeyspace.LegacyBatchlog, UUIDType.instance.decompose(uuid), FBUtilities.timestampMicros(), FBUtilities.nowInSeconds())), 298));
    }
