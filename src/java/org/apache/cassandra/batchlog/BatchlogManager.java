...
public class BatchlogManager implements BatchlogManagerMBean {
    ...
    public static void remove(UUID id) {
        ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(PartitionUpdate.fullPartitionDelete(SystemKeyspace.Batches, UUIDType.instance.decompose(id), FBUtilities.timestampMicros(), FBUtilities.nowInSeconds())), 295)).apply();
    }
    ...
    public static void store(Batch batch, boolean durableWrites) {
        RowUpdateBuilder builder = ((org.apache.cassandra.db.RowUpdateBuilder) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RowUpdateBuilder(SystemKeyspace.Batches, batch.creationTime, batch.id).clustering().add("version", MessagingService.current_version), 296));
        for (ByteBuffer mutation : batch.encodedMutations) {
            builder.addListEntry("mutations", mutation);
            org.zlab.ocov.tracker.Runtime.update(builder, 143, batch, durableWrites);
        }
        for (Mutation mutation : batch.decodedMutations) {
            try (DataOutputBuffer buffer = new DataOutputBuffer()) {
                ...
                org.zlab.ocov.tracker.Runtime.update(builder, 144, batch, durableWrites);
            } catch (IOException e) {
                // shouldn't happen
    ...
    private void replayFailedBatches() {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(endpointsCount, 0, "<=", 35))) {
    ...
    private void processBatchlogEntries(UntypedResultSet batches, int pageSize, RateLimiter rateLimiter) {
        ...
        for (UntypedResultSet.Row row : batches) {
            ...
            try {
                ReplayingBatch batch = ((ReplayingBatch) org.zlab.ocov.tracker.Runtime.update(new ReplayingBatch(id, version, row.getList("mutations", BytesType.instance)), 145, batches, pageSize, rateLimiter));
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(batch.replay(rateLimiter, hintedNodes), 0, ">", 36))) {
        ...
        private void writeHintsForUndeliveredEndpoints(int startFrom, Set<InetAddress> hintedNodes) {
            ...
            for (int i = startFrom; i < replayHandlers.size(); i++) {
                ...
                if (handler != null) {
                    ...
                    HintsService.instance.write(transform(handler.undelivered, StorageService.instance::getHostIdForEndpoint), ((org.apache.cassandra.hints.Hint) org.zlab.ocov.tracker.Runtime.update(Hint.create(undeliveredMutation, writtenAt), 146, startFrom, hintedNodes)));
                }
            }
        }
        ...
        private static ReplayWriteResponseHandler<Mutation> sendSingleReplayMutation(final Mutation mutation, long writtenAt, Set<InetAddress> hintedNodes) {
            ...
            for (InetAddress endpoint : StorageService.instance.getNaturalAndPendingEndpoints(ks, tk)) {
                if (endpoint.equals(FBUtilities.getBroadcastAddress())) {
                    ...
                    HintsService.instance.write(StorageService.instance.getHostIdForEndpoint(endpoint), ((org.apache.cassandra.hints.Hint) org.zlab.ocov.tracker.Runtime.update(Hint.create(mutation, writtenAt), 147, mutation, writtenAt, hintedNodes)));
                }
            }
