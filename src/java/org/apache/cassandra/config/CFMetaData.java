...
public final class CFMetaData {
    ...
    // null means "we don't know and need to infer from other data"
    ...
    // We use a sorted set so iteration is of predictable order (for SELECT for instance)
    ...
    public CFMetaData comment(String prop) {
        comment = Strings.nullToEmpty(prop);
        return this;
    }
    ...
    private static CFMetaData newSystemMetadata(String keyspace, String cfName, String comment, CellNameType comparator) {
        return new CFMetaData(keyspace, cfName, ColumnFamilyType.Standard, comparator, generateLegacyCfId(keyspace, cfName)).comment(comment).readRepairChance(0).dcLocalReadRepairChance(0).gcGraceSeconds(0).memtableFlushPeriod(3600 * 1000);
    }
    ...
    public static CFMetaData newIndexMetadata(CFMetaData parent, ColumnDefinition info, CellNameType indexComparator) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(parent, parent.ksName, "parent.ksName").toJsonString());
    ...
    public static CFMetaData copyOpts(CFMetaData newCFMD, CFMetaData oldCFMD) {
        ...
        for (ColumnDefinition cd : oldCFMD.allColumns()) clonedColumns.add(cd.copy());
    ...
    public AbstractType<?> getDefaultValidator() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.defaultValidator, "this.defaultValidator").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public AbstractType<?> getKeyValidator() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.keyValidator, "this.keyValidator").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        return keyValidator;
    ...
    public List<ColumnDefinition> partitionKeyColumns() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.partitionKeyColumns, "this.partitionKeyColumns").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public List<ColumnDefinition> clusteringColumns() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.clusteringColumns, "this.clusteringColumns").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public Set<ColumnDefinition> regularColumns() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.regularColumns, "this.regularColumns").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public CType getKeyValidatorAsCType() {
        return keyValidator instanceof CompositeType ? new CompoundCType(((CompositeType) keyValidator).types) : new SimpleCType(keyValidator);
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equal(cfId, other.cfId) && Objects.equal(ksName, other.ksName) && Objects.equal(cfName, other.cfName) && Objects.equal(cfType, other.cfType) && Objects.equal(comparator, other.comparator) && Objects.equal(comment, other.comment) && Objects.equal(readRepairChance, other.readRepairChance) && Objects.equal(dcLocalReadRepairChance, other.dcLocalReadRepairChance) && Objects.equal(gcGraceSeconds, other.gcGraceSeconds) && Objects.equal(defaultValidator, other.defaultValidator) && Objects.equal(keyValidator, other.keyValidator) && Objects.equal(minCompactionThreshold, other.minCompactionThreshold) && Objects.equal(maxCompactionThreshold, other.maxCompactionThreshold) && Objects.equal(columnMetadata, other.columnMetadata) && Objects.equal(compactionStrategyClass, other.compactionStrategyClass) && Objects.equal(compactionStrategyOptions, other.compactionStrategyOptions) && Objects.equal(compressionParameters, other.compressionParameters) && Objects.equal(getBloomFilterFpChance(), other.getBloomFilterFpChance()) && Objects.equal(memtableFlushPeriod, other.memtableFlushPeriod) && Objects.equal(caching, other.caching) && Objects.equal(defaultTimeToLive, other.defaultTimeToLive) && Objects.equal(minIndexInterval, other.minIndexInterval) && Objects.equal(maxIndexInterval, other.maxIndexInterval) && Objects.equal(speculativeRetry, other.speculativeRetry) && Objects.equal(droppedColumns, other.droppedColumns) && Objects.equal(triggers, other.triggers) && Objects.equal(isDense, other.isDense);
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder(29, 1597).append(cfId).append(ksName).append(cfName).append(cfType).append(comparator).append(comment).append(readRepairChance).append(dcLocalReadRepairChance).append(gcGraceSeconds).append(defaultValidator).append(keyValidator).append(minCompactionThreshold).append(maxCompactionThreshold).append(columnMetadata).append(compactionStrategyClass).append(compactionStrategyOptions).append(compressionParameters).append(getBloomFilterFpChance()).append(memtableFlushPeriod).append(caching).append(defaultTimeToLive).append(minIndexInterval).append(maxIndexInterval).append(speculativeRetry).append(droppedColumns).append(triggers).append(isDense).toHashCode();
    }
    ...
    public boolean apply(CFMetaData cfm) throws ConfigurationException {
        ...
        for (ColumnDefinition cd : columnDiff.entriesOnlyOnLeft().values()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(columnDiff.entriesOnlyOnLeft().values(), cd, "cd").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        ...
        for (ColumnDefinition cd : columnDiff.entriesOnlyOnRight().values()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(columnDiff.entriesOnlyOnRight().values(), cd, "cd").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public ColumnDefinition getColumnDefinition(ColumnIdentifier name) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(name, name.bytes, "name.bytes").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public ColumnDefinition getColumnDefinition(CellName cellName) {
        ...
        ColumnDefinition def = id == null ? // Means a dense layout, try the full column name
        getColumnDefinition(cellName.toByteBuffer()) : getColumnDefinition(id);
        ...
        if (cfId != null) {
            ...
            for (ColumnDefinition newDef : allColumns()) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(allColumns(), newDef, "newDef").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
    ...
    public CFMetaData validate() throws ConfigurationException {
        ...
        for (ColumnDefinition c : allColumns()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(allColumns(), c, "c").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    private static Set<String> existingIndexNames(String cfToExclude) {
        ...
        for (ColumnFamilyStore cfs : ColumnFamilyStore.all()) if (cfToExclude == null || !cfs.name.equals(cfToExclude))
    ...
    private void validateIndexIntervalThresholds() throws ConfigurationException {
        ...
        if (maxIndexInterval < minIndexInterval)
            throw new ConfigurationException(String.format("Max index interval (%d) must be greater than the min index " + "interval (%d).", maxIndexInterval, minIndexInterval));
    }
    ...
    public CFMetaData addAllColumnDefinitions(Collection<ColumnDefinition> defs) {
        for (ColumnDefinition def : defs) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(defs, def, "def").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public CFMetaData rebuild() {
        ...
        for (ColumnDefinition def : allColumns()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(allColumns(), def, "def").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            switch(def.kind) {
