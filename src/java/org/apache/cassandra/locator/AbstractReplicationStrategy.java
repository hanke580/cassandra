...
public abstract class AbstractReplicationStrategy {
    ...
    public <T> AbstractWriteResponseHandler<T> getWriteResponseHandler(Collection<InetAddress> naturalEndpoints, Collection<InetAddress> pendingEndpoints, ConsistencyLevel consistency_level, Runnable callback, WriteType writeType) {
    ...
    public Multimap<InetAddress, Range<Token>> getAddressRanges(TokenMetadata metadata) {
        ...
        for (Token token : metadata.sortedTokens()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(metadata.sortedTokens(), token, "token").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    private void validateExpectedOptions() throws ConfigurationException {
        ...
        for (String key : configOptions.keySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(configOptions.keySet(), key, "key").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
