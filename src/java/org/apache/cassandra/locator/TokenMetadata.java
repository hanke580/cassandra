...
public class TokenMetadata {
    ...
    public TokenMetadata() {
        this(SortedBiMultiValMap.<Token, InetAddress>create(null, inetaddressCmp), HashBiMap.<InetAddress, UUID>create(), new Topology(), DatabaseDescriptor.getPartitioner());
    }
    ...
    public void calculatePendingRanges(AbstractReplicationStrategy strategy, String keyspaceName) {
        ...
        synchronized (pendingRanges) {
            if (bootstrapTokens.isEmpty() && leavingEndpoints.isEmpty() && movingEndpoints.isEmpty()) {
                ...
                pendingRanges.put(keyspaceName, calculatePendingRanges(strategy, metadata, bootstrapTokens, leavingEndpoints, movingEndpoints));
    ...
    public static Iterator<Token> ringIterator(final ArrayList<Token> ring, Token start, boolean includeMin) {
        if (ring.isEmpty())
            return includeMin ? Iterators.singletonIterator(start.getPartitioner().getMinimumToken()) : Iterators.<Token>emptyIterator();
