...
public class NetworkTopologyStrategy extends AbstractReplicationStrategy {
        ...
        for (Map.Entry<String, Integer> dc : datacenters.entrySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(datacenters.entrySet(), dc, "dc").toJsonString());
    ...
    private boolean hasSufficientReplicas(Map<String, Set<InetAddress>> dcReplicas, Multimap<String, InetAddress> allEndpoints) {
        for (String dc : datacenters.keySet()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(datacenters.keySet(), dc, "dc").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public int getReplicationFactor() {
        int total = 0;
        for (int repFactor : datacenters.values()) total += repFactor;
        return total;
    }
