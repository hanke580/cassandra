...
public class ColumnFamilyMetrics {
    ...
    public ColumnFamilyMetrics(final ColumnFamilyStore cfs) {
        ...
        bloomFilterOffHeapMemoryUsed = createColumnFamilyGauge("BloomFilterOffHeapMemoryUsed", new Gauge<Long>() {
            ...
            public Long getValue() {
                long total = 0;
                for (SSTableReader sst : cfs.getSSTables()) total += sst.getBloomFilterOffHeapSize();
                return total;
            }
        });
        indexSummaryOffHeapMemoryUsed = createColumnFamilyGauge("IndexSummaryOffHeapMemoryUsed", new Gauge<Long>() {
            ...
            public Long getValue() {
                long total = 0;
                for (SSTableReader sst : cfs.getSSTables()) total += sst.getIndexSummaryOffHeapSize();
                return total;
            }
        });
        compressionMetadataOffHeapMemoryUsed = createColumnFamilyGauge("CompressionMetadataOffHeapMemoryUsed", new Gauge<Long>() {
            ...
            public Long getValue() {
                long total = 0;
                for (SSTableReader sst : cfs.getSSTables()) total += sst.getCompressionMetadataOffHeapSize();
                return total;
            }
        });
    ...
    public void release() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.metrics.ColumnFamilyMetrics.class, org.apache.cassandra.metrics.ColumnFamilyMetrics.all, "org.apache.cassandra.metrics.ColumnFamilyMetrics.all").toJsonString());
        ...
        for (String name : all) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(all, name, "name").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
