...
public class AutoSavingCache<K extends CacheKey, V> extends InstrumentingCache<K, V> {
    ...
    public class Writer extends CompactionInfo.Holder {
        ...
        protected Writer(int keysToSave) {
            ...
            info = new CompactionInfo(CFMetaData.denseCFMetaData(SystemKeyspace.NAME, cacheType.toString(), BytesType.instance), type, 0, keysEstimate, "keys", UUIDGen.getTimeUUID());
        }
                    ...
                    while (keyIterator.hasNext()) {
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                ...
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.keyIterator, "this.keyIterator").toJsonString());
                        ...
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(key, key.ksAndCFName, "key.ksAndCFName").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
