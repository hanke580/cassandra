...
public class StreamReader {
    ...
    public SSTableWriter read(ReadableByteChannel channel) throws IOException {
        ...
        if (kscf != null) {
            cfs = ((org.apache.cassandra.db.Keyspace) org.zlab.ocov.tracker.Runtime.update(Keyspace.open(kscf.left), 7, channel)).getColumnFamilyStore(kscf.right);
        }
    ...
    protected long totalSize() {
        long size = 0;
        for (Pair<Long, Long> section : sections) size += section.right - section.left;
        return size;
    }
