    ...
    public static CompositeData toCompositeData(final UUID planId, SessionInfo sessionInfo) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(sessionInfo, sessionInfo.sessionIndex, "sessionInfo.sessionIndex").toJsonString());
    ...
    public static SessionInfo fromCompositeData(CompositeData cd) {
        ...
        for (ProgressInfo progress : fromArrayOfCompositeData((CompositeData[]) values[6], toProgressInfo)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(fromArrayOfCompositeData((CompositeData[]) values[6], toProgressInfo), progress, "progress").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
        ...
        for (ProgressInfo progress : fromArrayOfCompositeData((CompositeData[]) values[7], toProgressInfo)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(fromArrayOfCompositeData((CompositeData[]) values[7], toProgressInfo), progress, "progress").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
            info.updateProgress(progress);
