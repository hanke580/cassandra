...
public final class SessionInfo implements Serializable {
    ...
    public void updateProgress(ProgressInfo newProgress) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(newProgress, newProgress.fileName, "newProgress.fileName").toJsonString());
    ...
    private long getTotalSizeInProgress(Collection<ProgressInfo> files) {
        long total = 0;
        for (ProgressInfo file : files) total += file.currentBytes;
        return total;
    }
