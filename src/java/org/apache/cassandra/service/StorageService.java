...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean {
        ...
        try {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.gms.ApplicationState.class, org.apache.cassandra.gms.ApplicationState.TOKENS, "org.apache.cassandra.gms.ApplicationState.TOKENS").toJsonString());
            ...
            // clean up since we have what we need
        ...
        if (shouldBootstrap()) {
            ...
            if (!replacing) {
                ...
                if (!isReplacingSameAddress()) {
                    ...
                    if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                        if (!isSerializeLoggingActive.get()) {
                            isSerializeLoggingActive.set(true);
                            serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.bootstrapTokens, "this.bootstrapTokens").toJsonString());
                            isSerializeLoggingActive.set(false);
                        }
                    }
                    ...
                    for (Token token : bootstrapTokens) {
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(bootstrapTokens, token, "token").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
            ...
            if (bootstrapTokens.isEmpty()) {
                ...
                if (initialTokens.size() < 1) {
                    ...
                    for (String token : initialTokens) bootstrapTokens.add(getPartitioner().getTokenFactory().fromString(token));
                ...
                if (!current.isEmpty()) {
                    for (InetAddress existing : current) {
                        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                            if (!isSerializeLoggingActive.get()) {
                                isSerializeLoggingActive.set(true);
                                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(current, existing, "existing").toJsonString());
                                isSerializeLoggingActive.set(false);
                            }
                        }
    ...
    private void maybeAddOrUpdateKeyspace(KSMetaData expected) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(expected, expected.name, "expected.name").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
        ...
        for (CFMetaData expectedTable : expected.cfMetaData().values()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(expected.cfMetaData().values(), expectedTable, "expectedTable").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public void rebuild(String sourceDc) {
        ...
        try {
            RangeStreamer streamer = new RangeStreamer(tokenMetadata, null, FBUtilities.getBroadcastAddress(), "Rebuild", !replacing && useStrictConsistency, DatabaseDescriptor.getEndpointSnitch(), streamStateStore);
    ...
    private boolean bootstrap(final Collection<Token> tokens) {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                isSerializeLoggingActive.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.progressSupport, "this.progressSupport").toJsonString());
                isSerializeLoggingActive.set(false);
            }
        }
    ...
    public String getRpcaddress(InetAddress endpoint) {
        if (endpoint.equals(FBUtilities.getBroadcastAddress()))
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.gms.ApplicationState.class, org.apache.cassandra.gms.ApplicationState.RPC_ADDRESS, "org.apache.cassandra.gms.ApplicationState.RPC_ADDRESS").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    private Map<Range<Token>, List<InetAddress>> getRangeToAddressMap(String keyspace, List<Token> sortedTokens) {
        ...
        if (keyspace == null) {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(Schema.instance.getNonSystemKeyspaces(), Schema.instance.getNonSystemKeyspaces().get(0), "Schema.instance.getNonSystemKeyspaces().get(0)").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        }
    ...
    public boolean isRpcReady(InetAddress endpoint) {
        return MessagingService.instance().getVersion(endpoint) < MessagingService.VERSION_22 || Gossiper.instance.getEndpointStateForEndpoint(endpoint).isRpcReady();
    }
    ...
    private void handleStateNormal(final InetAddress endpoint, final String status) {
        ...
        if (replacing && isReplacingSameAddress() && Gossiper.instance.getEndpointStateForEndpoint(DatabaseDescriptor.getReplaceAddress()) != null && (hostId.equals(Gossiper.instance.getHostId(DatabaseDescriptor.getReplaceAddress()))))
        ...
        for (final Token token : tokens) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(tokens, token, "token").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
        ...
        for (InetAddress ep : endpointsToRemove) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(endpointsToRemove, ep, "ep").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            removeEndpoint(ep);
        ...
        if (tokenMetadata.isMember(endpoint)) {
            ...
            if (VersionedValue.REMOVED_TOKEN.equals(state)) {
                ...
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(org.apache.cassandra.gms.ApplicationState.class, org.apache.cassandra.gms.ApplicationState.REMOVAL_COORDINATOR, "org.apache.cassandra.gms.ApplicationState.REMOVAL_COORDINATOR").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
    ...
    private Multimap<Range<Token>, InetAddress> getChangedRangesForLeaving(String keyspaceName, InetAddress endpoint) {
        ...
        for (Range<Token> range : ranges) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(ranges, range, "range").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public Set<InetAddress> getLiveRingMembers(boolean excludeDeadStates) {
        ...
        for (InetAddress ep : Gossiper.instance.getLiveMembers()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(Gossiper.instance.getLiveMembers(), ep, "ep").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
    ...
    public String[] getAllDataFileLocations() {
        ...
        for (int i = 0; i < locations.length; i++) locations[i] = FileUtils.getCanonicalPath(locations[i]);
        return locations;
    }
    ...
    public void takeSnapshot(String tag, String... keyspaceNames) throws IOException {
        ...
        for (Keyspace keyspace : keyspaces) keyspace.snapshot(tag, null);
    }
    ...
    public void takeMultipleColumnFamilySnapshot(String tag, String... columnFamilyList) throws IOException {
        ...
        for (Entry<Keyspace, List<String>> entry : keyspaceColumnfamily.entrySet()) {
            for (String columnFamily : entry.getValue()) entry.getKey().snapshot(tag, columnFamily);
        }
    }
    ...
    @Deprecated
    public int forceRepairRangeAsync(String beginToken, String endToken, String keyspaceName, boolean isSequential, Collection<String> dataCenters, Collection<String> hosts, boolean fullRepair, String... columnFamilies) {
    ...
    private Future<StreamState> streamHints() {
        ...
        if (candidates.isEmpty()) {
            ...
            return new StreamPlan("Hints").transferRanges(hintsDestinationHost, preferred, SystemKeyspace.NAME, ranges, SystemKeyspace.HINTS).execute();
        }
    }
    ...
    List<String> // do not rename to getter - see CASSANDRA-4452 for details
    sampleKeyRange() {
        List<DecoratedKey> keys = new ArrayList<>();
        for (Keyspace keyspace : Keyspace.nonSystem()) {
