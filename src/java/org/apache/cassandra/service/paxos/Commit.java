...
public class Commit {
    ...
    public boolean equals(Object o) {
        ...
        if (!ballot.equals(commit.ballot))
            return false;
        if (!key.equals(commit.key))
            return false;
        if (!update.equals(commit.update))
            return false;
        return true;
    }
        ...
        public void serialize(Commit commit, DataOutputPlus out, int version) throws IOException {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    ...
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(commit, commit.key, "commit.key").toJsonString());
