...
public class StorageProxy implements StorageProxyMBean {
    ...
    private static Pair<UUID, Integer> beginAndRepairPaxos(long start, ByteBuffer key, CFMetaData metadata, List<InetAddress> liveEndpoints, int requiredParticipants, ConsistencyLevel consistencyForPaxos, ConsistencyLevel consistencyForCommit, final boolean isWrite, ClientState state) throws WriteTimeoutException, WriteFailureException {
        ...
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(metadata, metadata.ksName, "metadata.ksName").toJsonString());
    ...
    private static void commitPaxos(Commit proposal, ConsistencyLevel consistencyLevel, boolean shouldHint) throws WriteTimeoutException {
        ...
        for (InetAddress destination : Iterables.concat(naturalEndpoints, pendingEndpoints)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(Iterables.concat(naturalEndpoints, pendingEndpoints), destination, "destination").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    private static void syncWriteToBatchlog(Collection<Mutation> mutations, Collection<InetAddress> endpoints, UUID uuid) throws WriteTimeoutException, WriteFailureException {
        ...
        for (InetAddress target : endpoints) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(endpoints, target, "target").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    public static void sendToHintedEndpoints(final Mutation mutation, Iterable<InetAddress> targets, AbstractWriteResponseHandler<IMutation> responseHandler, String localDataCenter) throws OverloadedException {
        ...
        for (InetAddress destination : targets) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(targets, destination, "destination").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
    ...
    public static Future<Void> submitHint(final Mutation mutation, final InetAddress target, final AbstractWriteResponseHandler<IMutation> responseHandler) {
    ...
    private static boolean systemKeyspaceQuery(List<ReadCommand> cmds) {
        for (ReadCommand cmd : cmds) if (!cmd.ksName.equals(SystemKeyspace.NAME))
            return false;
        return true;
    }

    public static List<Row> read(List<ReadCommand> commands, ConsistencyLevel consistencyLevel) throws UnavailableException, IsBootstrappingException, ReadFailureException, ReadTimeoutException, InvalidRequestException {
     ...
     */
    public static List<Row> read(List<ReadCommand> commands, ConsistencyLevel consistencyLevel, ClientState state) throws UnavailableException, IsBootstrappingException, ReadFailureException, ReadTimeoutException, InvalidRequestException {
        if (StorageService.instance.isBootstrapMode() && !systemKeyspaceQuery(commands)) {
            readMetrics.unavailables.mark();
            throw new IsBootstrappingException();
        }
        return consistencyLevel.isSerialConsistency() ? readWithPaxos(commands, consistencyLevel, state) : readRegular(commands, consistencyLevel);
    }

    private static List<Row> readWithPaxos(List<ReadCommand> commands, ConsistencyLevel consistencyLevel, ClientState state) throws InvalidRequestException, UnavailableException, ReadFailureException, ReadTimeoutException {
        assert state != null;
        ...
        try {
            ...
            for (ReadCommand command : commands) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActiveStatic.get()) {
                        isSerializeLoggingActiveStatic.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(commands, command, "command").toJsonString());
                        isSerializeLoggingActiveStatic.set(false);
                    }
                }
    ...
    private static List<Row> readRegular(List<ReadCommand> commands, ConsistencyLevel consistencyLevel) throws UnavailableException, ReadFailureException, ReadTimeoutException {
        long start = System.nanoTime();
        List<Row> rows = null;
        try {
     ...
     */
    private static List<Row> fetchRows(List<ReadCommand> initialCommands, ConsistencyLevel consistencyLevel) throws UnavailableException, ReadFailureException, ReadTimeoutException {
        ...
        do {
            ...
            for (AbstractReadExecutor exec : readExecutors) exec.maybeTryAdditionalReplicas();
            ...
            for (AbstractReadExecutor exec : readExecutors) {
                try {
                    ...
                    String gotData = responseCount > 0 ? exec.resolver.isDataPresent() ? " (including data)" : " (only digests)" : "";
                    boolean isTimeout = ex instanceof ReadTimeoutException;
                    if (Tracing.isTracing()) {
                        Tracing.trace("{}; received {} of {} responses{}", isTimeout ? "Timed out" : "Failed", responseCount, blockFor, gotData);
                    ...
                    ReadCallback<ReadResponse, Row> repairHandler = new ReadCallback<>(resolver, ConsistencyLevel.ALL, exec.getContactedReplicas().size(), exec.command, Keyspace.open(exec.command.getKeyspace()), exec.handler.endpoints);
    ...
    private static float estimateResultRowsPerRange(AbstractRangeCommand command, Keyspace keyspace) {
        ...
        if (command.rowFilter != null && !command.rowFilter.isEmpty()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(command, command.rowFilter, "command.rowFilter").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
