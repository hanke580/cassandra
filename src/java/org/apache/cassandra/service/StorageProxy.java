...
public class StorageProxy implements StorageProxyMBean {
        ...
        for (InetAddress destination : Iterables.concat(naturalEndpoints, pendingEndpoints)) {
            if (FailureDetector.instance.isAlive(destination)) {
                if (shouldBlock) {
                    ...
                    org.zlab.net.tracker.Runtime.record("sendOneWay", 12, message, destination);
                }
            } else if (shouldHint) {
    ...
    private static void syncWriteToBatchlog(Collection<Mutation> mutations, Collection<InetAddress> endpoints, UUID uuid) throws WriteTimeoutException, WriteFailureException {
        ...
        for (InetAddress target : endpoints) {
            ...
            if (target.equals(FBUtilities.getBroadcastAddress())) {
                ...
                org.zlab.net.tracker.Runtime.record("sendRR", 13, message, target, handler, false);
            } else {
                ...
                org.zlab.net.tracker.Runtime.record("sendRR", 14, BatchlogManager.getBatchlogMutationFor(mutations, uuid, targetVersion).createMessage(), target, handler, false);
            }
        }
        handler.get();
    }
            ...
            if (FailureDetector.instance.isAlive(destination)) {
                if (destination.equals(FBUtilities.getBroadcastAddress())) {
                    ...
                    if (localDataCenter.equals(dc)) {
                        ...
                        org.zlab.net.tracker.Runtime.record("sendRR", 15, message, destination, responseHandler, true);
                    } else {
    ...
    public static AbstractWriteResponseHandler<IMutation> mutateCounter(CounterMutation cm, String localDataCenter) throws UnavailableException, OverloadedException {
        ...
        if (endpoint.equals(FBUtilities.getBroadcastAddress())) {
            ...
            org.zlab.net.tracker.Runtime.record("sendRR", 16, cm.makeMutationMessage(), endpoint, responseHandler, false);
            return responseHandler;
        }
    }
