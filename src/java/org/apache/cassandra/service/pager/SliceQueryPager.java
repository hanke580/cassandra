...
public class SliceQueryPager extends AbstractQueryPager implements SinglePartitionPager {
    ...
    public PagingState state() {
        return lastReturned == null ? null : new PagingState(null, lastReturned.toByteBuffer(), maxRemaining());
    }
    ...
    protected List<Row> queryNextPage(int pageSize, ConsistencyLevel consistencyLevel, boolean localQuery) throws RequestValidationException, RequestExecutionException {
        ...
        org.zlab.ocov.tracker.Runtime.update(filter, 34, pageSize, consistencyLevel, localQuery);
        ...
        org.zlab.ocov.tracker.Runtime.update(pageCmd, 35, pageSize, consistencyLevel, localQuery);
        return localQuery ? Collections.singletonList(pageCmd.getRow(Keyspace.open(command.ksName))) : StorageProxy.read(Collections.singletonList(pageCmd), consistencyLevel, cstate);
    }
    ...
    protected boolean containsPreviousLast(Row first) {
        ...
        return !first.cf.deletionInfo().isDeleted(firstCell) && firstCell.isLive(timestamp()) && firstCell.name().isSameCQL3RowAs(metadata.comparator, lastReturned);
    }
