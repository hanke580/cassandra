...
public class QueryPagers {
    ...
    public static boolean mayNeedPaging(Pageable command, int pageSize) {
        if (command instanceof Pageable.ReadCommands) {
            ...
            long maxQueried = 0;
            for (ReadCommand readCmd : commands) maxQueried += maxQueried(readCmd);
    ...
    public static Iterator<ColumnFamily> pageRowLocally(final ColumnFamilyStore cfs, ByteBuffer key, final int pageSize) {
        SliceFromReadCommand command = ((SliceFromReadCommand) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new SliceFromReadCommand(cfs.metadata.ksName, key, cfs.name, System.currentTimeMillis(), new IdentityQueryFilter()), 100));
     ...
     */
    public static int countPaged(String keyspace, String columnFamily, ByteBuffer key, SliceQueryFilter filter, ConsistencyLevel consistencyLevel, ClientState cState, final int pageSize, long now) throws RequestValidationException, RequestExecutionException {
