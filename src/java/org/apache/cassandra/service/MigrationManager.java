...
public class MigrationManager {
    ...
    public static void announceNewType(UserType newType, boolean announceLocally) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(newType, newType.keyspace, "newType.keyspace").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    public static void announceTypeDrop(UserType droppedType, boolean announceLocally) {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActiveStatic.get()) {
                isSerializeLoggingActiveStatic.set(true);
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(droppedType, droppedType.keyspace, "droppedType.keyspace").toJsonString());
                isSerializeLoggingActiveStatic.set(false);
            }
        }
    ...
    private static Future<?> announce(final Collection<Mutation> schema) {
        ...
        for (InetAddress endpoint : Gossiper.instance.getLiveMembers()) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActiveStatic.get()) {
                    isSerializeLoggingActiveStatic.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(Gossiper.instance.getLiveMembers(), endpoint, "endpoint").toJsonString());
                    isSerializeLoggingActiveStatic.set(false);
                }
            }
