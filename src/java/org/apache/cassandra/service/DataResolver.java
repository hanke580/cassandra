...
public class DataResolver extends ResponseResolver {
    ...
    public PartitionIterator resolve() {
        if (!needsReplicaFilteringProtection()) {
            ...
            return resolveWithReadRepair(context, i -> shortReadProtectedResponse(i, context), UnaryOperator.identity());
        }
    ...
    private class ResolveContext {
        ...
        private ResolveContext(int count) {
            ...
            for (int i = 0; i < count; i++) sources[i] = responses.get(i).from;
    ...
    private UnfilteredPartitionIterator shortReadProtectedResponse(int i, ResolveContext context) {
        ...
        return context.needShortReadProtection() ? extendWithShortReadProtection(originalResponse, context, i) : originalResponse;
    }
    ...
    private PartitionIterator resolveWithReplicaFilteringProtection() {
        ...
        ReplicaFilteringProtection rfp = new ReplicaFilteringProtection(keyspace, command, consistency, firstPhaseContext.sources, DatabaseDescriptor.getCachedReplicaRowsWarnThreshold(), DatabaseDescriptor.getCachedReplicaRowsFailThreshold());
        ...
        PartitionIterator completedPartitions = resolveWithReadRepair(secondPhaseContext, i -> rfp.queryProtectedPartitions(firstPhasePartitions, i), results -> command.rowFilter().filter(results, command.metadata(), command.nowInSec()));
    ...
    private class RepairMergeListener implements UnfilteredPartitionIterators.MergeListener {
        ...
        private class MergeListener implements UnfilteredRowIterators.MergeListener {
            ...
            public void onMergedPartitionLevelDeletion(DeletionTime mergedDeletion, DeletionTime[] versions) {
                ...
                org.zlab.ocov.tracker.Runtime.update(this, 16, mergedDeletion, versions);
            ...
            public void onMergedRangeTombstoneMarkers(RangeTombstoneMarker merged, RangeTombstoneMarker[] versions) {
                try {
                    ...
                    String details = String.format("Error merging RTs on %s.%s: command=%s, reversed=%b, merged=%s, versions=%s, sources={%s}, responses:%n %s", table.ksName, table.cfName, command.toCQLString(), isReversed, merged == null ? "null" : merged.toString(table), '[' + Joiner.on(", ").join(Iterables.transform(Arrays.asList(versions), rt -> rt == null ? "null" : rt.toString(table))) + ']', Arrays.toString(sources), makeResponsesDebugString());
            ...
            public void close() {
                for (int i = 0; i < repairs.length; i++) if (null != repairs[i])
                    sendRepairMutation(repairs[i], sources[i]);
            }

            private void sendRepairMutation(PartitionUpdate partition, InetAddress destination) {
                Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.update(new Mutation(partition), 17, partition, destination));
                ...
                if (mutationSize <= maxMutationSize) {
                    ...
                    logger.debug("Encountered an oversized ({}/{}) read repair mutation for table {}.{}, key {}, node {}", mutationSize, maxMutationSize, command.metadata().ksName, command.metadata().cfName, command.metadata().getKeyValidator().getString(partitionKey.getKey()), destination);
                } else {
                    logger.warn("Encountered an oversized ({}/{}) read repair mutation for table {}.{}, key {}, node {}", mutationSize, maxMutationSize, command.metadata().ksName, command.metadata().cfName, command.metadata().getKeyValidator().getString(partitionKey.getKey()), destination);
                    int blockFor = consistency.blockFor(keyspace);
                    Tracing.trace("Timed out while read-repairing after receiving all {} data and digest responses", blockFor);
                    throw new ReadTimeoutException(consistency, blockFor - 1, blockFor, true);
                }
    ...
    private UnfilteredPartitionIterator extendWithShortReadProtection(UnfilteredPartitionIterator partitions, ResolveContext context, int i) {
        ...
        ShortReadPartitionsProtection protection = new ShortReadPartitionsProtection(context.sources[i], () -> responses.clearUnsafe(i), singleResultCounter, context.mergedResultCounter);
        /*
    ...
    private class ShortReadPartitionsProtection extends Transformation<UnfilteredRowIterator> implements MorePartitions<UnfilteredPartitionIterator> {
        ...
        public UnfilteredPartitionIterator moreContents() {
             ...
             */
            int toQuery = command.limits().count() != DataLimits.NO_LIMIT ? command.limits().count() - mergedResultCounter.counted() : command.limits().perPartitionCount();
