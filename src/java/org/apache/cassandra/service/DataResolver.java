...
public class DataResolver extends ResponseResolver {
    ...
    private UnfilteredPartitionIterator mergeWithShortReadProtection(List<UnfilteredPartitionIterator> results, InetAddress[] sources, DataLimits.Counter mergedResultCounter) {
        ...
        if (!command.limits().isUnlimited())
            for (int i = 0; i < results.size(); i++) results.set(i, extendWithShortReadProtection(results.get(i), sources[i], mergedResultCounter));
    ...
    private class RepairMergeListener implements UnfilteredPartitionIterators.MergeListener {
        ...
        public UnfilteredRowIterators.MergeListener getRowMergeListener(DecoratedKey partitionKey, List<UnfilteredRowIterator> versions) {
            return ((MergeListener) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new MergeListener(partitionKey, columns(versions), isReversed(versions)), 9));
        }
        ...
        private PartitionColumns columns(List<UnfilteredRowIterator> versions) {
            ...
            return ((PartitionColumns) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new PartitionColumns(statics, regulars), 10));
        }
        ...
        private class MergeListener implements UnfilteredRowIterators.MergeListener {
            ...
            private PartitionUpdate update(int i) {
                if (repairs[i] == null) {
                    repairs[i] = ((PartitionUpdate) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new PartitionUpdate(command.metadata(), partitionKey, columns, 1), 11));
                }
                return repairs[i];
            }
            ...
            public void onMergedPartitionLevelDeletion(DeletionTime mergedDeletion, DeletionTime[] versions) {
                ...
                org.zlab.ocov.tracker.Runtime.update(this, 2, mergedDeletion, versions);
            ...
            private void closeOpenMarker(int i, Slice.Bound close) {
                ...
                update(i).add(((RangeTombstone) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new RangeTombstone(Slice.make(isReversed ? close : open, isReversed ? open : close), currentDeletion()), 12)));
            ...
            public void close() {
                for (int i = 0; i < repairs.length; i++) {
                    ...
                    MessageOut<Mutation> msg = ((Mutation) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Mutation(repairs[i]), 13)).createMessage(MessagingService.Verb.READ_REPAIR);
    ...
    private UnfilteredPartitionIterator extendWithShortReadProtection(UnfilteredPartitionIterator partitions, InetAddress source, DataLimits.Counter mergedResultCounter) {
        ...
        ShortReadPartitionsProtection protection = ((ShortReadPartitionsProtection) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ShortReadPartitionsProtection(source, singleResultCounter, mergedResultCounter), 8));
        /*
    ...
    private class ShortReadPartitionsProtection extends Transformation<UnfilteredRowIterator> implements MorePartitions<UnfilteredPartitionIterator> {
        ...
        public UnfilteredRowIterator applyToPartition(UnfilteredRowIterator partition) {
             ...
             */
            ShortReadRowsProtection protection = ((ShortReadRowsProtection) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ShortReadRowsProtection(partition.metadata(), partition.partitionKey()), 14));
        ...
        private UnfilteredPartitionIterator executeReadCommand(ReadCommand cmd) {
            ...
            if (StorageProxy.canDoLocalRequest(source)) {
                StageManager.getStage(Stage.READ).maybeExecuteImmediately(((StorageProxy.LocalReadRunnable) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new StorageProxy.LocalReadRunnable(cmd, handler), 15)));
            } else
