...
public abstract class AbstractReadExecutor {
    ...
    private void makeRequests(ReadCommand readCommand, Iterable<InetAddress> endpoints) {
        ...
        if (hasLocalEndpoint) {
            ...
            StageManager.getStage(Stage.READ).maybeExecuteImmediately(((LocalReadRunnable) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new LocalReadRunnable(command, handler), 24)));
        }
    }
    ...
    public static class NeverSpeculatingReadExecutor extends AbstractReadExecutor {
        ...
        public void executeAsync() {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(targetReplicas.size(), 1, ">", 2)))
    ...
    private static class SpeculatingReadExecutor extends AbstractReadExecutor {
        ...
        public void executeAsync() {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(handler.blockfor, initialReplicas.size(), "<", 3))) {
                // We're hitting additional targets for read repair.  Since our "extra" replica is the least-
                // preferred by the snitch, we do an extra data read to start with against a replica more
                // likely to reply; better to let RR fail than the entire query.
                makeDataRequests(initialReplicas.subList(0, 2));
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(initialReplicas.size(), 2, ">", 4)))
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(initialReplicas.size(), 1, ">", 5)))
    ...
    private static class AlwaysSpeculatingReadExecutor extends AbstractReadExecutor {
        ...
        public void executeAsync() {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(targetReplicas.size(), 2, ">", 7)))
