...
public class MigrationCoordinator {
    ...
    protected boolean shouldPullSchema(UUID version) {
        ...
        if (Schema.instance.getVersion().equals(version)) {
            logger.debug("Not pulling schema for version {}, because schema versions match: " + "local={}, remote={}", version, Schema.instance.getVersion(), version);
            return false;
        }
        return true;
    }
    ...
    protected boolean shouldPullImmediately(InetAddress endpoint, UUID version) {
        if (Schema.instance.isEmpty() || runtimeMXBean.getUptime() < MIGRATION_DELAY_IN_MS) {
            ...
            logger.debug("Immediately submitting migration task for {}, " + "schema versions: local={}, remote={}", endpoint, Schema.instance.getVersion(), version);
            return true;
        }
        return false;
    }
