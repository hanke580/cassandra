...
public class MigrationCoordinator {
    ...
    protected boolean shouldPullSchema(UUID version) {
        ...
        if (Schema.instance.isSameVersion(version)) {
            logger.debug("Not pulling schema for version {}, because schema versions match: " + "local/real={}, local/compatible={}, remote={}", version, Schema.schemaVersionToString(Schema.instance.getRealVersion()), Schema.schemaVersionToString(Schema.instance.getAltVersion()), Schema.schemaVersionToString(version));
            return false;
        }
        return true;
    }
    ...
    protected boolean shouldPullImmediately(InetAddress endpoint, UUID version) {
        if (Schema.instance.isEmpty() || runtimeMXBean.getUptime() < MIGRATION_DELAY_IN_MS) {
            ...
            logger.debug("Immediately submitting migration task for {}, " + "schema versions: local/real={}, local/compatible={}, remote={}", endpoint, Schema.schemaVersionToString(Schema.instance.getRealVersion()), Schema.schemaVersionToString(Schema.instance.getAltVersion()), Schema.schemaVersionToString(version));
            return true;
        }
        return false;
    }
    ...
    protected void sendMigrationMessage(Callback callback) {
        ...
        org.zlab.net.tracker.Runtime.record("sendRR", 16, message, callback.endpoint, callback, message.getTimeout(), true);
    }
