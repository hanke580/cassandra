...
public class ActiveRepairService implements IEndpointStateChangeSubscriber, IFailureDetectionEventListener, ActiveRepairServiceMBean {
    ...
    public static class RepairCommandExecutorHandle {
        ...
        private static final ThreadPoolExecutor repairCommandExecutor = initializeExecutor(DatabaseDescriptor.getRepairCommandPoolSize(), DatabaseDescriptor.getRepairCommandPoolFullStrategy());
    }
    ...
    public ActiveRepairService(IFailureDetector failureDetector, Gossiper gossiper) {
        ...
        this.repairStatusByCmd = CacheBuilder.newBuilder().expireAfterWrite(Long.getLong("cassandra.parent_repair_status_expiry_seconds", TimeUnit.SECONDS.convert(1, TimeUnit.DAYS)), TimeUnit.SECONDS).// using weight wouldn't work so well, since it doesn't reflect mutation of cached data
        ...
        maximumSize(Long.getLong("cassandra.parent_repair_status_cache_size", 100_000)).build();
    ...
    public UUID prepareForRepair(UUID parentRepairSession, InetAddressAndPort coordinator, Set<InetAddressAndPort> endpoints, RepairOption options, boolean isForcedRepair, List<ColumnFamilyStore> columnFamilyStores) {
        ...
        for (InetAddressAndPort neighbour : endpoints) {
            if (FailureDetector.instance.isAlive(neighbour)) {
                ...
                org.zlab.net.tracker.Runtime.record("sendWithCallback", 15, msg, neighbour, callback);
            } else {
        ...
        for (InetAddressAndPort endpoint : endpoints) {
            try {
                if (FailureDetector.instance.isAlive(endpoint)) {
                    ...
                    org.zlab.net.tracker.Runtime.record("sendWithCallback", 16, msg, endpoint, loggingCallback);
                }
            } catch (Exception exc) {
