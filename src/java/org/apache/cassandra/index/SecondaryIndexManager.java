...
public class SecondaryIndexManager implements IndexRegistry {
    ...
    private static final ExecutorService asyncExecutor = new JMXEnabledThreadPoolExecutor(1, StageManager.KEEPALIVE, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new NamedThreadFactory("SecondaryIndexManagement"), "internal");
    ...
    public void buildAllIndexesBlocking(Collection<SSTableReader> sstables) {
        buildIndexesBlocking(sstables, indexes.values().stream().filter(Index::shouldBuildBlocking).collect(Collectors.toSet()));
    }
    ...
    public void flushAllNonCFSBackedIndexesBlocking() {
        executeAllBlocking(indexes.values().stream().filter(index -> !index.getBackingTable().isPresent()), Index::getBlockingFlushTask);
    }
    ...
    public void indexPartition(DecoratedKey key, Set<Index> indexes, int pageSize) {
        ...
        if (!indexes.isEmpty()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(cmd, 10, key, indexes, pageSize);
            int nowInSec = cmd.nowInSec();
            boolean readStatic = false;
            SinglePartitionPager pager = ((SinglePartitionPager) org.zlab.ocov.tracker.Runtime.update(new SinglePartitionPager(cmd, null, Server.CURRENT_VERSION), 11, key, indexes, pageSize));
    ...
    public int calculateIndexingPageSize() {
        ...
        logger.trace("Calculated page size {} for indexing {}.{} ({}/{}/{}/{})", pageSize, baseCfs.metadata.ksName, baseCfs.metadata.cfName, meanPartitionSize, meanCellsPerPartition, meanRowsPerPartition, meanRowSize);
        return pageSize;
    }
    ...
    public UpdateTransaction newUpdateTransaction(PartitionUpdate update, OpOrder.Group opGroup, int nowInSec) {
        ...
        Index.Indexer[] indexers = indexes.values().stream().map(i -> i.indexerFor(update.partitionKey(), update.columns(), nowInSec, opGroup, IndexTransaction.Type.UPDATE)).filter(Objects::nonNull).toArray(Index.Indexer[]::new);
    ...
    private static final class IndexGCTransaction implements CompactionTransaction {
        ...
        private IndexGCTransaction(DecoratedKey key, PartitionColumns columns, int versions, int nowInSec, Collection<Index> indexes) {
            this.key = key;
    ...
    private static final class CleanupGCTransaction implements CleanupTransaction {
        ...
        private CleanupGCTransaction(DecoratedKey key, PartitionColumns columns, int nowInSec, Collection<Index> indexes) {
            this.key = key;
            this.columns = columns;
            this.indexes = indexes;
            this.nowInSec = nowInSec;
