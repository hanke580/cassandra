...
public abstract class CassandraIndexSearcher implements Index.Searcher {
    ...
    UnfilteredPartitionIterator search(ReadOrderGroup orderGroup) {
    ...
    private UnfilteredRowIterator queryIndex(DecoratedKey indexKey, ReadCommand command, ReadOrderGroup orderGroup) {
        ...
        return SinglePartitionReadCommand.create(indexCfm, command.nowInSec(), indexKey, ColumnFilter.all(indexCfm), filter).queryMemtableAndDisk(indexCfs, orderGroup.indexReadOpOrderGroup());
    }
    ...
    private ClusteringIndexFilter makeIndexFilter(ReadCommand command) {
        if (command instanceof SinglePartitionReadCommand) {
            ...
            if (filter instanceof ClusteringIndexNamesFilter) {
                ...
                return ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(clusterings.build(), filter.isReversed()), 1, command));
            } else {
                ...
                return ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ClusteringIndexSliceFilter(builder.build(), filter.isReversed()), 3));
            }
        } else {
            ...
            if (range.left instanceof DecoratedKey) {
                ...
                if (range.right instanceof DecoratedKey) {
                    ...
                    slice = Slice.make(makeIndexBound(((DecoratedKey) range.left).getKey(), Slice.Bound.BOTTOM), Slice.Bound.TOP);
                }
            }
            return ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new ClusteringIndexSliceFilter(Slices.with(index.getIndexComparator(), slice), false), 4));
        }
    }
