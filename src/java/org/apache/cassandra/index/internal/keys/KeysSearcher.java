...
public class KeysSearcher extends CassandraIndexSearcher {
        ...
        return new UnfilteredPartitionIterator() {
            ...
            private boolean prepareNext() {
                while (next == null && indexHits.hasNext()) {
                    ...
                    SinglePartitionReadCommand dataCmd = SinglePartitionReadCommand.create(isForThrift(), index.baseCfs.metadata, command.nowInSec(), extendedFilter, command.rowFilter(), DataLimits.NONE, key, command.clusteringIndexFilter(key), null);
                    ...
                    dataIter = filterIfStale(dataCmd.queryMemtableAndDisk(index.baseCfs, orderGroup.baseReadOpOrderGroup()), hit, indexKey.getKey(), orderGroup.writeOpOrderGroup(), isForThrift(), command.nowInSec());
                    if (dataIter != null) {
                        if (dataIter.isEmpty())
                            dataIter.close();
                        else
    ...
    private ColumnFilter getExtendedFilter(ColumnFilter initialFilter) {
        ...
        org.zlab.ocov.tracker.Runtime.update(builder, 12, initialFilter);
    ...
    private UnfilteredRowIterator filterIfStale(UnfilteredRowIterator iterator, Row indexHit, ByteBuffer indexedValue, OpOrder.Group writeOp, boolean isForThrift, int nowInSec) {
        if (isForThrift) {
            ...
            iterator.close();
            Row data = result.getRow(((Clustering) org.zlab.ocov.tracker.Runtime.update(new Clustering(index.getIndexedColumn().name.bytes), 13, iterator, indexHit, indexedValue, writeOp, isForThrift, nowInSec)));
            org.zlab.ocov.tracker.Runtime.update(result, 14, iterator, indexHit, indexedValue, writeOp, isForThrift, nowInSec);
            if (data == null)
                return null;
            ...
            if (baseData == null || !baseData.isLive(nowInSec) || index.getIndexedColumn().type.compare(indexedValue, baseData.value()) != 0) {
                ...
                index.deleteStaleEntry(index.getIndexCfs().decorateKey(indexedValue), new Clustering(index.getIndexedColumn().name.bytes), new DeletionTime(indexHit.primaryKeyLivenessInfo().timestamp(), nowInSec), writeOp);
                return null;
            } else {
                if (command.columnFilter().includes(index.getIndexedColumn()))
                ...
                for (ColumnDefinition selected : command.columnFilter().fetchedColumns()) slices.add(Slice.make(comparator, selected.name.bytes));
                return result.unfilteredIterator(ColumnFilter.all(command.metadata()), slices.build(), false);
            }
        } else {
            if (!iterator.metadata().isCompactTable()) {
                logger.warn("Non-composite index was used on the table '{}' during the query. Starting from Cassandra 4.0, only " + "composite indexes will be supported. If compact flags were dropped for this table, drop and re-create " + "the index.", iterator.metadata().cfName);
            }
            Row data = iterator.staticRow();
            if (index.isStale(data, indexedValue, nowInSec)) {
                // Index is stale, remove the index entry and ignore
                index.deleteStaleEntry(index.getIndexCfs().decorateKey(indexedValue), makeIndexClustering(iterator.partitionKey().getKey(), Clustering.EMPTY), new DeletionTime(indexHit.primaryKeyLivenessInfo().timestamp(), nowInSec), writeOp);
                iterator.close();
                return null;
            } else {
                return iterator;
