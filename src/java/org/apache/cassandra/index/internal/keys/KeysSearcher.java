...
public class KeysSearcher extends CassandraIndexSearcher {
    ...
    private ColumnFilter getExtendedFilter(ColumnFilter initialFilter) {
        ...
        org.zlab.ocov.tracker.Runtime.update(builder, 0, initialFilter);
    ...
    private UnfilteredRowIterator filterIfStale(UnfilteredRowIterator iterator, Row indexHit, ByteBuffer indexedValue, OpOrder.Group writeOp, boolean isForThrift, int nowInSec) {
        if (isForThrift) {
            ...
            iterator.close();
            Row data = result.getRow(((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(index.getIndexedColumn().name.bytes), 1)));
            if (data == null)
                return null;
            ...
            if (baseData == null || !baseData.isLive(nowInSec) || index.getIndexedColumn().type.compare(indexedValue, baseData.value()) != 0) {
                ...
                index.deleteStaleEntry(index.getIndexCfs().decorateKey(indexedValue), ((Clustering) org.zlab.ocov.tracker.Runtime.monitorCreationContext(new Clustering(index.getIndexedColumn().name.bytes), 2)), new DeletionTime(indexHit.primaryKeyLivenessInfo().timestamp(), nowInSec), writeOp);
                return null;
            } else {
                if (command.columnFilter().includes(index.getIndexedColumn()))
