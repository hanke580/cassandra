...
public final class LegacyHintsMigrator {
    ...
    private void migrateLegacyHints(UUID hostId, ByteBuffer buffer) {
        String query = String.format("SELECT target_id, hint_id, message_version, mutation, ttl(mutation) AS ttl, writeTime(mutation) AS write_time " + "FROM %s.%s " + "WHERE target_id = ?", SystemKeyspace.NAME, SystemKeyspace.LEGACY_HINTS);
    ...
    private void migrateLegacyHintsInternal(UUID hostId, Iterator<UntypedResultSet.Row> iterator, ByteBuffer buffer) {
        HintsDescriptor descriptor = ((HintsDescriptor) org.zlab.ocov.tracker.Runtime.update(new HintsDescriptor(hostId, System.currentTimeMillis()), 4, hostId, iterator, buffer));
        try (HintsWriter writer = HintsWriter.create(hintsDirectory, descriptor)) {
            try (HintsWriter.Session session = writer.newSession(buffer)) {
                org.zlab.ocov.tracker.Runtime.update(session, 5, hostId, iterator, buffer);
                while (iterator.hasNext()) {
                    Hint hint = convertLegacyHint(iterator.next());
                    org.zlab.ocov.tracker.Runtime.update(hint, 6, hostId, iterator, buffer);
                    if (hint != null)
                        session.append(hint);
    ...
    private static Hint convertLegacyHint(UntypedResultSet.Row row) {
        ...
        org.zlab.ocov.tracker.Runtime.update(mutation, 7, row);
        if (mutation == null)
            return null;
        // milliseconds, not micros, for the hints table
    ...
    private static Mutation deserializeLegacyMutation(UntypedResultSet.Row row) {
        try (DataInputBuffer dib = new DataInputBuffer(row.getBlob("mutation"), true)) {
            ...
            logger.error("Failed to migrate a hint for {} from legacy {}.{} table: {}", row.getUUID("target_id"), SystemKeyspace.NAME, SystemKeyspace.LEGACY_HINTS, e);
            return null;
        } catch (MarshalException e) {
            logger.warn("Failed to validate a hint for {} (table id {}) from legacy {}.{} table - skipping: {})", row.getUUID("target_id"), SystemKeyspace.NAME, SystemKeyspace.LEGACY_HINTS, e);
            return null;
        }
    }
    ...
    private static void deleteLegacyHintsPartition(UUID hostId) {
        ...
        Mutation mutation = ((Mutation) org.zlab.ocov.tracker.Runtime.update(new Mutation(((org.apache.cassandra.db.partitions.PartitionUpdate) org.zlab.ocov.tracker.Runtime.update(PartitionUpdate.fullPartitionDelete(SystemKeyspace.LegacyHints, UUIDType.instance.decompose(hostId), System.currentTimeMillis(), FBUtilities.nowInSeconds()), 8, hostId))), 9, hostId));
