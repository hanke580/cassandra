...
public class CassandraServer implements Cassandra.Iface {
            ...
            if (predicate.column_names != null) {
                if (metadata.isSuper()) {
                    if (parent.isSetSuper_column()) {
                        ...
                        for (ByteBuffer bb : predicate.column_names) clusterings.add(new Clustering(bb));
    ...
    public ColumnOrSuperColumn get(ByteBuffer key, ColumnPath column_path, ConsistencyLevel consistency_level) throws InvalidRequestException, NotFoundException, UnavailableException, TimedOutException {
        if (startSessionIfRequested()) {
            Map<String, String> traceParameters = ImmutableMap.of("key", ByteBufferUtil.bytesToHex(key), "column_path", column_path.toString(), "consistency_level", consistency_level.name());
        ...
        try {
            ...
            if (metadata.isSuper()) {
                ...
                filter = new ClusteringIndexNamesFilter(FBUtilities.singleton(new Clustering(column_path.super_column), metadata.comparator), false);
            } else {
    ...
    public int get_count(ByteBuffer key, ColumnParent column_parent, SlicePredicate predicate, ConsistencyLevel consistency_level) throws InvalidRequestException, UnavailableException, TimedOutException {
        ...
        try {
            ...
            DataLimits limits = getLimits(1, metadata.isSuper() && !column_parent.isSetSuper_column(), predicate);
            DecoratedKey dk = metadata.decorateKey(key);
            return QueryPagers.countPaged(metadata, dk, columnFilter, filter, limits, ThriftConversion.fromThrift(consistency_level), cState, pageSize, nowInSec, true);
    ...
    public Map<ByteBuffer, Integer> multiget_count(List<ByteBuffer> keys, ColumnParent column_parent, SlicePredicate predicate, ConsistencyLevel consistency_level) throws InvalidRequestException, UnavailableException, TimedOutException {
        ...
        try {
            ...
            Map<ByteBuffer, List<ColumnOrSuperColumn>> columnFamiliesMap = multigetSliceInternal(keyspace, keys, column_parent, FBUtilities.nowInSeconds(), predicate, consistency_level, cState);
    ...
    public CASResult cas(ByteBuffer key, String column_family, List<Column> expected, List<Column> updates, ConsistencyLevel serial_consistency_level, ConsistencyLevel commit_consistency_level) throws InvalidRequestException, UnavailableException, TimedOutException {
        ...
        try {
            ...
            try (RowIterator result = StorageProxy.cas(cState.getKeyspace(), column_family, dk, new ThriftCASRequest(toLegacyCells(metadata, expected, nowInSec), partitionUpdates, nowInSec), ThriftConversion.fromThrift(serial_consistency_level), ThriftConversion.fromThrift(commit_consistency_level), cState)) {
                return result == null ? new CASResult(true) : new CASResult(false).setCurrent_values(thriftifyColumnsAsColumns(metadata, LegacyLayout.fromRowIterator(result).right));
            }
        } catch (UnknownColumnException e) {
            throw new InvalidRequestException(e.getMessage());
    ...
    private void sortAndMerge(CFMetaData metadata, List<LegacyLayout.LegacyCell> cells, int nowInSec) {
        ...
        for (int current = 1; current < cells.size(); current++) {
            ...
            if (cmp == 0) {
                ...
                if (metadata.isCounter()) {
                    res = Conflicts.resolveCounter(pc.timestamp, pc.isLive(nowInSec), pc.value, cc.timestamp, cc.isLive(nowInSec), cc.value);
                } else {
                    res = Conflicts.resolveRegular(pc.timestamp, pc.isLive(nowInSec), pc.localDeletionTime, pc.value, cc.timestamp, cc.isLive(nowInSec), cc.localDeletionTime, cc.value);
                }
                switch(res) {
                    case LEFT_WINS:
        ...
        for (int i = cells.size() - 1; i > previous; i--) cells.remove(i);
    }
    ...
    private void deleteColumnOrSuperColumn(LegacyLayout.LegacyDeletionInfo delInfo, List<LegacyLayout.LegacyCell> cells, CFMetaData cfm, Deletion del, int nowInSec) throws InvalidRequestException {
    ...
    public List<KeySlice> get_range_slices(ColumnParent column_parent, SlicePredicate predicate, KeyRange range, ConsistencyLevel consistency_level) throws InvalidRequestException, UnavailableException, TException, TimedOutException {
        if (startSessionIfRequested()) {
            Map<String, String> traceParameters = ImmutableMap.of("column_parent", column_parent.toString(), "predicate", predicate.toString(), "range", range.toString(), "consistency_level", consistency_level.name());
        ...
        try {
            ...
            try {
                ...
                PartitionRangeReadCommand cmd = PartitionRangeReadCommand.create(true, metadata, nowInSec, columns, ThriftConversion.rowFilterFromThrift(metadata, range.row_filter), limits, new DataRange(bounds, filter));
    ...
    public List<KeySlice> get_paged_slice(String column_family, KeyRange range, ByteBuffer start_column, ConsistencyLevel consistency_level) throws InvalidRequestException, UnavailableException, TimedOutException, TException {
        ...
        try {
            ...
            try {
                ...
                PartitionRangeReadCommand cmd = PartitionRangeReadCommand.create(true, metadata, nowInSec, ColumnFilter.all(metadata), RowFilter.NONE, limits, new DataRange(bounds, filter).forPaging(bounds, metadata.comparator, pageFrom, true));
                try (PartitionIterator results = StorageProxy.getRangeSlice(cmd, consistencyLevel)) {
    ...
    public List<KeySlice> get_indexed_slices(ColumnParent column_parent, IndexClause index_clause, SlicePredicate column_predicate, ConsistencyLevel consistency_level) throws InvalidRequestException, UnavailableException, TimedOutException, TException {
        ...
        try {
            ...
            PartitionRangeReadCommand cmd = PartitionRangeReadCommand.create(true, metadata, nowInSec, columns, ThriftConversion.rowFilterFromThrift(metadata, index_clause.expressions), limits, new DataRange(bounds, filter));
    ...
    public String system_add_column_family(CfDef cf_def) throws TException {
        ...
        try {
            ...
            // explicitly ignore any id set by client (Hector likes to set zero)
            cf_def.unsetId();
    ...
    public String system_add_keyspace(KsDef ks_def) throws InvalidRequestException, SchemaDisagreementException, TException {
        ...
        try {
            ...
            for (CfDef cf_def : ks_def.cf_defs) {
                // explicitly ignore any id set by client (same as system_add_column_family)
                cf_def.unsetId();
                CFMetaData cfm = ThriftConversion.fromThrift(cf_def);
                if (!cfm.getTriggers().isEmpty())
                    state().ensureIsSuper("Only superusers are allowed to add triggers.");
            ...
            switch(compression) {
                case GZIP:
                    ...
                    while (true) {
                        ...
                        int lenWrite = 0;
                        while ((lenWrite = decompressor.inflate(outBuffer)) != 0) decompressed.write(outBuffer, 0, lenWrite);
    ...
    public CqlResult execute_cql3_query(ByteBuffer query, Compression compression, ConsistencyLevel cLevel) throws TException {
        try {
            ...
            ThriftClientState cState = state();
            return ClientState.getCQLQueryHandler().process(queryString, cState.getQueryState(), QueryOptions.fromThrift(ThriftConversion.fromThrift(cLevel), Collections.<ByteBuffer>emptyList()), null).toThriftResult();
        } catch (RequestExecutionException e) {
            throw ThriftConversion.rethrow(e);
        } catch (RequestValidationException e) {
            throw ThriftConversion.toThrift(e);
    ...
    public CqlResult execute_prepared_cql3_query(int itemId, List<ByteBuffer> bindVariables, ConsistencyLevel cLevel) throws TException {
        ...
        try {
            ...
            return ClientState.getCQLQueryHandler().processPrepared(prepared.statement, cState.getQueryState(), QueryOptions.fromThrift(ThriftConversion.fromThrift(cLevel), bindVariables), null).toThriftResult();
        } catch (RequestExecutionException e) {
            throw ThriftConversion.rethrow(e);
        } catch (RequestValidationException e) {
            throw ThriftConversion.toThrift(e);
    ...
    private static class ThriftCASRequest implements CASRequest {
        ...
        public SinglePartitionReadCommand readCommand(int nowInSec) {
            if (expected.isEmpty()) {
                ...
                ClusteringIndexSliceFilter filter = ((ClusteringIndexSliceFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexSliceFilter(Slices.ALL, false), 186, nowInSec));
                DataLimits limits = DataLimits.thriftLimits(1, 1);
                return ((org.apache.cassandra.db.SinglePartitionReadCommand) org.zlab.ocov.tracker.Runtime.update(SinglePartitionReadCommand.create(true, metadata, nowInSec, ColumnFilter.all(metadata), RowFilter.NONE, limits, key, filter), 187, nowInSec));
            }
            ...
            ClusteringIndexNamesFilter filter = ((ClusteringIndexNamesFilter) org.zlab.ocov.tracker.Runtime.update(new ClusteringIndexNamesFilter(clusterings.build(), false), 188, nowInSec));
        ...
        public boolean appliesTo(FilteredPartition current) {
            if (expected.isEmpty()) {
                return ((org.apache.cassandra.db.partitions.FilteredPartition) org.zlab.ocov.tracker.Runtime.update(current, 189, current)).isEmpty();
            ...
            for (Row e : expectedPartition) {
                ...
                for (ColumnData expectedData : e) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(searchIter, 190, current);
                    if (currentData == null)
                        return false;
                    if (column.isSimple()) {
