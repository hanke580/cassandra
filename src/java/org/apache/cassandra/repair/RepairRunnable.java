    ...
    protected void runMayThrow() throws Exception {
        ...
        if (options.isTraced()) {
            ...
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.listeners, "this.listeners").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
            for (ProgressListener listener : listeners) {
                if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                    if (!isSerializeLoggingActive.get()) {
                        isSerializeLoggingActive.set(true);
                        serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(listeners, listener, "listener").toJsonString());
                        isSerializeLoggingActive.set(false);
                    }
                }
