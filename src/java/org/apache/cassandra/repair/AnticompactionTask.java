    ...
    public void run() {
        if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
            if (!isSerializeLoggingActive.get()) {
                ...
                serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.neighbor, "this.neighbor").toJsonString());
        ...
        if (FailureDetector.instance.isAlive(neighbor)) {
            if (org.zlab.dinv.logger.SerializeMonitor.isSerializing) {
                if (!isSerializeLoggingActive.get()) {
                    isSerializeLoggingActive.set(true);
                    serialize_logger.info(org.zlab.dinv.logger.LogEntry.constructLogEntry(this, this.successfulRanges, "this.successfulRanges").toJsonString());
                    isSerializeLoggingActive.set(false);
                }
            }
